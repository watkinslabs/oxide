...
ACTIVE NOW: B450-device-del-registry-drop-after-teardown; IN AUDIT.

Current active item: `device_del` drops device from registry after
remove/devtmpfs teardown.

Next gate after B450: prove source ordering, hosted driver-model regressions,
runtime arch relevance, PR merge, then fresh `origin/main` before claiming B451.
...
| ACTIVE | B450-device-del-registry-drop-after-teardown | `device_del` drops device from registry after remove/devtmpfs teardown. |
