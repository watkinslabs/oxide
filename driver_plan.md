...
ACTIVE NOW: `B389-vsock-close-releases-state` — CLAIMED.

Current active item: `>>> ACTIVE >>> B389-vsock-close-releases-state`.

Current B389 gate: prove or fix AF_VSOCK close/drop cleanup so listener,
backlog, and connection table state are released, with hosted tests and
...
| >>> ACTIVE >>> CLAIMED | B389-vsock-close-releases-state | AF_VSOCK close releases listener/backlog/connection state. |
