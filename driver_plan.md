...
ACTIVE NOW: `B370-virtio-net-no-boot-ipv4-policy` — IN AUDIT.

Current active item: `>>> ACTIVE >>> B370-virtio-net-no-boot-ipv4-policy`.

Current B370 gate: prove old boot-probe default IPv4 policy is removed from virtio-net, add focused regression if missing, run virtio-net tests, x86_64/aarch64 driver-path proof, push PR, merge, then sync local `main`.
...
| >>> ACTIVE >>> IN AUDIT | B370-virtio-net-no-boot-ipv4-policy | Virtio-net old boot-probe default IPv4 policy removed. |
