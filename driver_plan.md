...
ACTIVE NOW: `B388-vsock-listener-backlogs-owner-port` — CLAIMED.

Current active item: `>>> ACTIVE >>> B388-vsock-listener-backlogs-owner-port`.

Current B388 gate: prove or fix AF_VSOCK listener backlog state so it is keyed
by `(owner, port)`, with hosted tests and x86_64/aarch64 runtime proof before
merge.
...
| >>> ACTIVE >>> CLAIMED | B388-vsock-listener-backlogs-owner-port | Listener backlogs keyed by `(owner, port)`. |
