...
ACTIVE NOW: `B385-virtio-vsock-rx-bh-installed` — CLAIMED.

Current active item: `>>> ACTIVE >>> B385-virtio-vsock-rx-bh-installed`.

Current B385 gate: prove or fix virtio-vsock `VsockRx` bottom-half teardown so
only the installed transport controls handler lifetime, with hosted tests and
...
| >>> ACTIVE >>> CLAIMED | B385-virtio-vsock-rx-bh-installed | Virtio-vsock clears `VsockRx` bottom half only for installed transport. |
