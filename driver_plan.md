...
ACTIVE NOW: B425-no-flat-driverentry-probeall; IN AUDIT.

Current active item: Old flat `DriverEntry` / `probe_all(bdf)` live driver path removal.
...
| ACTIVE | B425-no-flat-driverentry-probeall | Remove old flat `DriverEntry` / `probe_all(bdf)` live driver path. |
