...
ACTIVE NOW: `B371-virtio-net-install-remove-keyed` — IN AUDIT.

Current active item: `>>> ACTIVE >>> B371-virtio-net-install-remove-keyed`.

Current B371 gate: prove virtio-net install/remove are keyed to the owning child key from PCI child dispatch through driver state, add focused regression if missing, run virtio-net tests, x86_64/aarch64 driver-path proof, push PR, merge, then sync local `main`.
...
| >>> ACTIVE >>> IN AUDIT | B371-virtio-net-install-remove-keyed | Virtio-net install/remove keyed to owning child key. |
