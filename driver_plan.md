...
ACTIVE NOW: `B352-drm-atomic-empty-state` — IN AUDIT.

Current active item: `>>> ACTIVE >>> B352-drm-atomic-empty-state`.

Current B352 gate: audit current `DRM_IOCTL_MODE_ATOMIC` empty-state gating,
add missing regressions, run hosted DRM tests, line-cap check, and
...
| >>> ACTIVE >>> IN AUDIT | B352-drm-atomic-empty-state | DRM MODE_ATOMIC accepts only internally gated empty state; real atomic property/object tables missing. |
