...
ACTIVE NOW: `B349-drm-page-flip-file-events` — CLAIMED, audit starting.

Current active item: `>>> ACTIVE >>> B349-drm-page-flip-file-events`.

Current B349 gate: audit DRM PAGE_FLIP event ownership, poll/read delivery,
and per-open-file isolation. Required proof is hosted regression, full DRM
tests, line-cap check, fast x86_64/aarch64 driver-path proof, pre-push boot
smoke, PR merge, and post-merge main sync.
...
| CLAIMED | B349-drm-page-flip-file-events | DRM PAGE_FLIP events are per card open file and poll/read correctly. |
