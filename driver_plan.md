...
ACTIVE NOW: `B354-drm-get-cap-supported-only` — IN AUDIT.

Current active item: `>>> ACTIVE >>> B354-drm-get-cap-supported-only`.

Current B354 gate: audit DRM GET_CAP advertised capability values, add hosted
regressions for unsupported PRIME/syncobj/async/page-flip-target/modifiers/cursor
caps, then x86_64/aarch64 driver-path proof.
...
| >>> ACTIVE >>> IN AUDIT | B354-drm-get-cap-supported-only | DRM GET_CAP stops advertising unsupported PRIME/syncobj/async/page-flip-target/modifiers/cursor caps. |
