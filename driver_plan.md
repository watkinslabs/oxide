...
ACTIVE NOW: `B343-scanout-backing-bdf-keyed` — VERIFIED, commit/PR merge pending.

Current active item: `>>> ACTIVE >>> B343-scanout-backing-bdf-keyed`.

Current B343 gate: scanout backing runtime ownership is keyed by virtio child
key, not PCI BDF; commit/PR merge pending.
...
| VERIFIED | B343-scanout-backing-bdf-keyed | Scanout backing runtime state is keyed by virtio child key instead of PCI BDF; source audit, hosted key-vs-BDF regression, full virtio-gpu and DRM tests, fast x86_64/aarch64 driver-path proof, line-cap check, and PR merge pending. |
