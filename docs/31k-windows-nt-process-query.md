...
FROZEN 2026-09-05. Dep:`01`,`02`,`13`,`31b`,`31d`,`31f`,`31h`,`52`,`53`. Provides: current-process and process-handle information for the NT runtime.

## 1 Contract

- `NtQueryInformationProcess` is appended at NT service ID `21`; existing selectors remain stable.
- `ProcessBasicInformation`, `ProcessVmCounters`, `ProcessAffinityMask`, `ProcessWow64Information`, `ProcessImageFileName`, `ProcessImageFileNameWin32`, `ProcessHandleCount`, and `ProcessCommandLineInformation` are implemented; other classes remain explicitly unsupported.
...
- `ProcessCommandLineInformation` reads the task-owned normalized process-parameter command line, returns a 64-bit `UNICODE_STRING` plus terminator, reports the complete required length before short-buffer failure, and never copies from a malformed source string.
