...
| G13 DONE | `net/` — inet.rs (htons/htonl/inet_pton/inet_ntop, oracle vs host), socket.rs (sockaddr_in/in6/storage/msghdr/iovec size-matched + socket/bind/connect/sendto/recvfrom/… wrappers, per-arch nrs; socketpair round-trip smoke), addrinfo.rs (getaddrinfo/getnameinfo/gai_strerror numeric+localhost+/etc/hosts + `/etc/services` service names + DNS A/AAAA through the `/etc/resolv.conf` nameserver). FOLLOW-UP: nsswitch/search-domain policy, multiple nameservers, full resolver retry/sort behavior. | tcp/inet6 smokes |
