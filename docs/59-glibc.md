...
2. **Loader data-symbol resolution (the real blocker)**: with the aliases exported and a single-reference usability test (`extern char **__environ; __environ[0]…`, `__progname`, `__tzname`, `__signgam` after lgamma — each referenced ALONE so no dual-copy-reloc), our ld.so leaves them at their INITIAL value (environ=0, progname empty, signgam stuck at 1) while the host populates them. So our loader does not perform R_*_COPY copy-reloc redirection (copy the initial bytes AND make the executable's copy interpose libc's reference) for data symbols — libc's startup writes land in libc's own storage, the executable reads its un-redirected copy. This affects ANY copy-relocated libc data symbol in an executable (the `__` aliases just exposed it); the fix is a `crates/user/ldso` feature (handle R_X86_64_COPY / R_AARCH64_COPY: memcpy + bind the symbol to the exe copy for all objects), not a libc change. Deferred to a focused ldso task. (GOT-accessed data and shared-lib refs are unaffected; the dominant `environ` path works because callers reach it via libc internals / GOT, not a copy-relocated executable reference.)

### 9.5 Work order (next PRs)

resolver/NSS_r → fts → ldso R_*_COPY data-symbol redirection (unblocks the §9.4 `__` aliases AND copy-relocated libc data in executables) → `_FloatN` aliases (export via naked-fn jmp thunks, like clone — asm-`.set` is localized) → C23 math → (RPC/crypt only if vendor-linked). DONE this pass: pthread surface, C11 threads, modern syscalls, locale `_l`, account-db, eaccess/sigisemptyset/__fpclassify/gets, in6addr, mbrtoc32, wcwidth/wcswidth, clone(2), dl_iterate_phdr — the entire libc-function surface the vendor binaries reference. Conformance 144/144.
