...
- `fatal!` and `klog::announce` are the only exceptions.
  ...
  - The `klog::announce` family announces an **irreversible machine transition** or a rare destructive operational decision whose victim must remain explainable after it is gone. Reboot, power-off, halt, kexec relocation and an OOM kill qualify. Gating these inverts the rule's own rationale: the cost argument is about paths taken millions of times, while these happen only after ordinary execution is ending or has already failed. Emitted at warning priority so a default level filter keeps it. `announce_bytes` is the allocation-free built-line form used when the answer carries an identity such as the OOM victim pid.
  - `announce` is bounded so it cannot become a waiver: the family is legal only for a terminal machine transition, an operator-requested answer, or a destructive victim decision such as OOM. It is not a general "important message" channel — ordinary paths that return to normal execution take the gated form. Spec-lint permits this one family and no other.
