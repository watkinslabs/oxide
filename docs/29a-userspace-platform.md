...
- Changed: §1-§6, §8-§14 — userspace is upstream Fedora glibc userland installed from RPMs and composed by the sibling `../images` repo. This repo builds no libc, no loader, no app, and publishes no sysroot; there is no in-tree cross-compile workflow, no `xtask user`, no `userspace/apps/`. Spec `59` deleted; `59§` references in the R04 block are flattened to plain text so `xref` resolves.
- Why: `crates/user/*` (own libc, `ld.so`, NSS, PAM, RPM, pkg), the `xtask glibc`/`sysroot`/`ldso`/`folded` commands, the `userspace/` probe tree, and `vendor/cross` are deleted. R04 replaced one build-our-own libc with another; both are gone. The ABI target is unchanged — Fedora `-gnu` binaries — only the supplier changed.
- Affected code: none — the deletions already landed.
- Test contract change: §12 replaces "we cross-compile X and it runs" with "the Fedora RPM binary runs"; ABI goldens + symbol-version-set checks are void with the libc they tested.

## Revision 2026-06-14 (R04)

- Changed: libc decision flips musl → **glibc ABI**, implemented in Rust as `crates/user/glibc` (oxide-libc) per spec 59. §2 userspace target triples → `*-unknown-linux-gnu`; §3 "Vendored musl fork" → oxide-libc emitting `libc.so.6`; §4 loader → `ld-linux-x86-64.so.2`/`ld-linux-aarch64.so.1` (`crates/user/ldso`). §3 UAPI-headers source unchanged. Spec 59 is the authoritative libc spec; §2-4 here are superseded by it.
- Why: endgame = Fedora RPM + from-source GNOME; Fedora is glibc. musl cannot run unmodified Fedora `-gnu` binaries. "100% Linux compat" (`03`) at the userspace ABI = glibc ABI.
- Affected code: `crates/user/glibc`, `crates/user/ldso`; `xtask glibc`; musl fork + ld-oxide retired at spec 59 G19 (kept buildable until then).
- Test contract change: differential oracle vs host glibc + ABI goldens + symbol-version-set check supersede musl-parity acceptance.
