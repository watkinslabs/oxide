...
- Changed: libc decision flips musl → **glibc ABI**, implemented in Rust as `crates/user/glibc` (oxide-libc) per `59`. §2 userspace target triples → `*-unknown-linux-gnu`; §3 "Vendored musl fork" → oxide-libc emitting `libc.so.6`; §4 loader → `ld-linux-x86-64.so.2`/`ld-linux-aarch64.so.1` (`crates/user/ldso`). §3 UAPI-headers source unchanged. `59` is now the authoritative libc spec; §2-4 here are superseded by it.
...
- Affected code: `crates/user/glibc`, `crates/user/ldso`; `xtask glibc`; musl fork + ld-oxide retired at `59§6` G19 (kept buildable until then).
- Test contract change: differential oracle vs host glibc + ABI goldens + symbol-version-set check (`59§7`) supersede musl-parity acceptance.
