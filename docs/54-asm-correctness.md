...
| 1.9 | both | The idle park must ADMIT INTERRUPTS as part of the park — `sti; hlt` on x86_64, `msr daifclr, #2` before `wfi` on aarch64 (`hal::CpuOps::safe_halt`, Linux `raw_safe_halt`). The idle path is reached with interrupts MASKED, so a park written as the bare architectural halt stops the core where nothing maskable can reach it again: no tick, no reschedule IPI, no device. `sti` and `hlt` must be ONE asm block — the interrupt shadow covers exactly the next instruction, and separate statements lose the wakeup that lands between them. Corollary: the cross-CPU stall scan excuses an idle-parked CPU, so this failure reports NOTHING. | B2245 — a wedged guest with CPU 1's heartbeat frozen 79 s, `[CPU-STALL]` silent, and both NMI samples showing the mask closed. |
