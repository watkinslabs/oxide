...
- `read_current_process` and `write_current_process` accept only the current
  address-space owner; read destinations are writable-probed before source
  access, write sources are readable-probed before writable destinations, and
  source or destination faults return `STATUS_PARTIAL_COPY` with the
