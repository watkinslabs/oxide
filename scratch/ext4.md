...
|---|---|---|---|---|
| E4-01 | high | Owned concurrent block reads are not safe for ext4. The earlier attempt was reverted after boot-time SIGBUS/data failures; ext4 still uses the serialized compatibility path. | E4-02 | concurrent read stress, byte-for-byte image validation, no boot regression, both arches |
| E4-02 | high | Metadata and frame-cache ownership does not yet provide a single lifetime-safe path for asynchronous ext4 reads and fills. | baseline only | one cache/request ownership model, injected completion/error tests, no stale-frame publication |
| E4-03 | high | Intermittent ext4-backed file fills return EIO and userspace reports SIGBUS while reading dconf. The precise failing owner is not yet proven. | E4-02 | provenance identifies the failing boundary; focused reproducer passes repeatedly; live GNOME read survives |
| E4-04 | high | Writes to the small ext4 `/home` image can return EIO while GNOME writes dconf. Allocation, extent update, writeback, journal, and capacity paths need a controlled reproducer. | E4-02, E4-05 | focused write/persistence test, remount persistence, live GNOME dconf write |
| E4-05 | high | Journal checkpointing retains only a conservative one-transaction handoff. Full retained-transaction accounting and batching remain absent, leaving write amplification. | E4-01, E4-02 | crash/replay matrix, checkpoint-list tests, repeated boot/perf evidence |
| E4-06 | medium | `journal_async_commit` is admitted but not implemented as a live JBD2 mode. | E4-01, E4-05 | option validation, async commit ordering, crash/replay tests, both arches |
...
| E4-10 | medium | `dioread_nolock`, `nodioread_nolock`, and `dioread_lock` are admitted without direct-I/O semantics. | E4-01, E4-02 | Linux-shaped direct-I/O read/write behavior or an explicit capability refusal; no accept-and-drop |
...
5. Add crash points before/after descriptor, data, commit, journal-superblock, home-write, and clean-publication operations.
...
2. Complete group scan criteria and best-group selection after the order indexes are consulted.
