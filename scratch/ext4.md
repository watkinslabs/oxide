...
| E4-07 | medium | Exact physical preallocation ownership, failed-claim rollback, contiguous locality-tail consumption with Linux-style size re-bucketing, Linux's `fls(len)-2` average-fragment buckets, the bounded three-order best-available scan fallback, bounded multiblock extent selection, and inode-PA release on truncate, final orphan eviction, and mount teardown are covered. Remaining locality lifecycle validation is still open. | E4-01, E4-02, E4-05 | allocator model tests, fragmentation/failure tests, e2fsck-clean images, measured workload |
