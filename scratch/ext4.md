...
| E4-10 | IN-PROGRESS | Ext4 now owns synchronous `O_DIRECT` reads/writes through extent mapping and the block device, with alignment errors, buffered-write drain, cache invalidation, journaled allocation/size publication, and Linux-shaped `dioread_nolock`/`dioread_lock` policy. `data=journal` disables the lockless mode and O_DIRECT admission, matching Linux. The real `mini.img` harness now covers direct extent reads, alignment rejection, remount persistence, and unwritten-extent zero/convert behavior. Inline-data/legacy-indirect files and DAX remain refused because their real owners are not implemented. | E4-01, E4-02 | Add mapped-page coherency and full syscall-level O_DIRECT coverage; then close the remaining layout-specific gaps |
