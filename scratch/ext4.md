...
| E4-08 | medium | Indexed-directory lookup and inode construction reuse the type-probe image and resident VFS inode identity; special inode construction now also reuses parsed `i_blocks` instead of rereading the inode table. Ordinary pathname walks start in Linux RCU mode and fall back at blocking boundaries. The Linux-shaped rwsem reader fast path removes the wait-lock round trip for uncontended shared readers, delayed-allocation buffered writes no longer reread the inode table before returning, and linear-directory byte matching now avoids per-entry object and closure work while borrowing the shared metadata block. Linear scans now retain Linux's per-directory last-successful-block hint. Create/mkdir/symlink/mknod transactions now durably publish the journal record before returning and defer home-block checkpointing to the background owner, matching the reference journal-handle boundary and shortening the VFS inode-lock hold. Two controlled repeats with the hint put `newfstatat` at 11,743/11,393 ns (15x/14x host), versus the prior 18,622/18,188 ns (24x/23x); both reached GNOME Shell. Two deferred-checkpoint repeats reduced boot CPU time to 6,059/6,181 ms versus 6,377/6,448 ms controls, with inode-writer hold at 2,623/2,342 ms. Aggregate closure is still open at ~15x host; the remaining owner is the blocking ext4 lookup path and residual mutation work. | E4-01, E4-02 | named phase owner; repeated controlled comparison closes the ratio |
