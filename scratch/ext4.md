...
| E4-08 | medium | Indexed-directory lookup and inode construction reuse the type-probe image and resident VFS inode identity. Ordinary pathname walks start in Linux RCU mode and fall back at blocking boundaries. The Linux-shaped rwsem reader fast path removes the wait-lock round trip for uncontended shared readers, delayed-allocation buffered writes no longer reread the inode table before returning, and linear-directory byte matching now avoids per-entry object and closure work while borrowing the shared metadata block. Linear scans now retain Linux's per-directory last-successful-block hint. Two controlled repeats with the hint put `newfstatat` at 11,743/11,393 ns (15x/14x host), versus the prior 18,622/18,188 ns (24x/23x); both reached GNOME Shell. A fresh attribution run shows 3,010 actual inode mutation locks held 2,096 ms total (~696 us each), while the generic writer counter includes unrelated file-position locks. The remaining owner is mutation hold time plus the blocking ext4 lookup path; aggregate closure is still open. | E4-01, E4-02 | named phase owner; repeated controlled comparison closes the ratio |
