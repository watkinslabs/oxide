...
| E4-08 | medium | Indexed-directory lookup and inode construction reuse the type-probe image and resident VFS inode identity. Ordinary pathname walks start in Linux RCU mode and fall back at blocking boundaries. The Linux-shaped rwsem reader fast path removes the wait-lock round trip for uncontended shared readers, but GNOME's blocking misses still encounter queued writers. The repeat attribution shows parent-lock ~406 us/miss, writer hold ~180 us, and `newfstatat` 18,627 ns (24x host); this is a phase-level improvement, not an aggregate ratio closure. The remaining owner is writer contention plus the blocking ext4 lookup path, not htree selection or root setup. | E4-01, E4-02 | named phase owner; repeated controlled comparison closes the ratio |
