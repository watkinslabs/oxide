...
| E4-08 | medium | Indexed-directory lookup and inode construction reuse the type-probe image and resident VFS inode identity. Ordinary pathname walks start in Linux RCU mode and fall back at blocking boundaries. The Linux-shaped rwsem reader fast path removes the wait-lock round trip for uncontended shared readers, delayed-allocation buffered writes no longer reread the inode table before returning, and linear-directory byte matching now avoids per-entry object and closure work while borrowing the shared metadata block. Two controlled repeats put the linear phase at ~161–163 us and `newfstatat` at 18,622/18,188 ns (24x/23x host); GNOME Shell started both times. A fresh attribution run shows 3,010 actual inode mutation locks held 2,096 ms total (~696 us each), while the generic writer counter includes unrelated file-position locks. These are phase-level improvements, not aggregate closure. The remaining owner is mutation hold time plus the blocking ext4 lookup path. | E4-01, E4-02 | named phase owner; repeated controlled comparison closes the ratio |
