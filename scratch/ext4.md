...
| E4-08 | medium | Indexed-directory lookup and inode construction reuse the type-probe image and resident VFS inode identity. Ordinary pathname walks start in Linux RCU mode and fall back at blocking boundaries. The Linux-shaped rwsem reader fast path removes the wait-lock round trip for uncontended shared readers, and delayed-allocation buffered writes no longer reread the inode table before returning. The latest attribution shows parent-lock ~331 us/miss, writer hold ~157 us, and `newfstatat` 18,734 ns (24x host); these are phase-level improvements, not an aggregate ratio closure. The remaining owner is queued-writer contention plus the blocking ext4 lookup path, not htree selection or root setup. | E4-01, E4-02 | named phase owner; repeated controlled comparison closes the ratio |
