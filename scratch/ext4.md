...
| E4-10 | IN-PROGRESS | Ext4 owns synchronous `O_DIRECT` and polled io_uring I/O through canonical extent/indirect mapping, allocation, cache invalidation, and durability owners. Inline-data and legacy-indirect mutation are covered. DAX now has Linux-shaped mount policy, shared-only mmap/page-mkwrite, direct byte I/O, virtio shared-memory transport handoff, and a kernel-target virtio-pmem block/DAX provider with config fallback and probe-failure cleanup. Remaining evidence is provider flush completion under the real image, DAX image coverage, and final GNOME/runtime closure. | E4-01, E4-02 | DAX flush and real-image harness evidence, both-arch target checks, final boot/runtime verification |
| E4-11 | DONE | Bitmap prefetch, ext4 external-xattr mbcache sharing/refcounting, and Linux orphan-file slots are wired and covered; `nombcache`/`no_mbcache` disables sharing. | E4-02, E4-07 | EA-inode xattr values remain a separate refused feature |
...
request format, block registration, DAX region publication, and DMA/frame
...
E4-03/E4-10 runtime update (B2995): the current merged main passes a real
GNOME userspace probe on x86_64 and AArch64 with SMP=1. The probe requires
active `gdm.service` and a running `gnome-shell`, then the harness also proves
serial RX. Basic boot and both architecture checks remain green. Whole-boot
performance closure is still separate and requires a comparable instrumented
sample.
