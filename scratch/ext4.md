...
| E4-08 | medium | Indexed-directory lookup and inode construction now reuse the type-probe image and resident VFS inode identity. Ordinary pathname walks now start in the Linux RCU mode and fall back at blocking boundaries; the controlled phase trace reduced parent-lock wait from 11,438 ms / 668,822 ns to 5,146 ms / 296,094 ns (~55%), while `newfstatat` moved only from 20,005 ns to 19,311 ns (25x host), within harness variance, so the ratio remains open. | E4-01, E4-02 | phase profile names remaining owner; repeated controlled comparison closes the ratio |
