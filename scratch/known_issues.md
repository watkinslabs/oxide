...
| OPEN | MISSING | med | `IP_NODEFRAG` is not a raw receive toggle: it only bypasses the IPv4 netfilter defragmentation hook, which this kernel does not implement. | no IPv4 netfilter defragmentation hook | unowned |
...
| Status | Class | Sev | Issue | Evidence | Owner |
|---|---|---|---|---|---|
| OPEN | MISSING | low | `IpInDiscards` still reads zero because no packet-buffer drop owner has been implemented. It needs its event named at the point it occurs. | `crates/kernel/net/src/mib.rs` | — |
