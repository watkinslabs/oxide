...
|---|---|---|---|---|
| `FIOSETOWN` (0x8901), `SIOCSPGRP` (0x8902), `FIOGETOWN` (0x8903), `SIOCGPGRP` (0x8904) | `sock_ioctl` async owner / fasync | Missing | no command route in `016_ioctl` | socket and pipe owner, process group, permission, signal delivery, 32-bit compat |
...
| `SIOCINQ`/`FIONREAD`, `SIOCOUTQ`/`TIOCOUTQ`, `SIOCOUTQNSD` (0x894b) | socket queue accounting | `SIOCINQ`/`SIOCOUTQ` implemented; `SIOCOUTQNSD` missing | `016_ioctl/common.rs`, `net/sock/io.rs`, queue-count tests | stream/datagram/seqpacket queue semantics, pending vs transmitted TCP bytes, faults |
...
| `SIOCGIFBR` (0x8940), `SIOCSIFBR` (0x8941) | bridge | Partial: native raw vectors implement version, bridge enumeration, create/delete; per-bridge private owner implements member lifecycle, bridge/port info, bridge/port priority, port path cost, bridge timing config, STP enable/disable, port list, FDB query, FDB ageing; canonical owner handles IEEE 802.1D Configuration/TCN BPDUs, root/designated roles, topology-change ageing, acknowledgement, and port-state timers | `siocgif/bridge.rs`, canonical `stack/bridge.rs` FDB/port owner, `stack/bridge_stp.rs`, `stack/bridge_stp_bpdu.rs` | native+compat and guest differential |
...
| `SIOCDARP` (0x8953), `SIOCGARP` (0x8954), `SIOCSARP` (0x8955) | ARP / neighbour | Partial: native 68-byte `arpreq` lookup, dynamic/permanent Ethernet binding mutation, and delete route to the canonical per-interface owner; proxy forms explicitly unsupported and invalid non-proxy netmask flags rejected | `siocgif/arp_ioctl.rs`, `stack/arp_neighbor.rs`, `stack/ethernet.rs`, virtio neighbour path | native+compat probes, guest differential, proxy semantics, exact Linux errno/order |
...
| `SIOCBRADDBR` (0x89a0), `SIOCBRDELBR` (0x89a1), `SIOCBRADDIF` (0x89a2), `SIOCBRDELIF` (0x89a3) | bridge lifecycle | Implemented through canonical RTNL bridge owner | `siocgif/bridge.rs`, `stack/bridge.rs` | native+compat lifecycle/error-order and guest differential |
