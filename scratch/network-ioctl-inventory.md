...
|---|---|---|---|---|
| `FIOSETOWN` (0x8901), `SIOCSPGRP` (0x8902), `FIOGETOWN` (0x8903), `SIOCGPGRP` (0x8904) | `sock_ioctl` async owner / fasync | Socket route implemented through canonical `File::f_setown`/`f_getown`; fasync SIGIO owner state is shared with `fcntl` | `016_ioctl/common.rs`, `016_ioctl/core.rs`, hosted shape test, retained x86 `t_sockioctl` owner-alias match | real socket owner/process-group, SIGIO delivery, 32-bit compat |
