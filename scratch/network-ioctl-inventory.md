...
| `SIOCGIFBR` (0x8940), `SIOCSIFBR` (0x8941) | bridge | Partial: native raw vectors implement version, bridge enumeration, create/delete; per-bridge private owner implements member lifecycle, bridge/port info, bridge/port priority, port path cost, bridge timing config, no-STP control, port list, FDB query, FDB ageing; canonical owner has IEEE 802.1D Configuration BPDU codec foundation | `siocgif/bridge.rs`, canonical `stack/bridge.rs` FDB/port owner, `stack/bridge_stp_bpdu.rs` | enabled STP admission/root/roles/states/timers, native+compat and guest differential |
