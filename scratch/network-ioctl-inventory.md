...
| `SIOCDARP` (0x8953), `SIOCGARP` (0x8954), `SIOCSARP` (0x8955) | ARP / neighbour | `arpreq` ABI remains missing; canonical per-interface cache now follows live interface generations, physical ARP ingress learns senders and replies for an owned primary address | `netdev.rs`, `netdev/ingress.rs`, `stack_forward.rs`, `linux_netdev/core.rs` | neighbour lookup/mutation, capability, address family, namespace, error order, transmit resolution and pending-neighbour lifecycle |
