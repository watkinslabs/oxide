...
| `SIOCGIFBR` (0x8940), `SIOCSIFBR` (0x8941) | bridge | Partial: native raw vectors implement version, bridge enumeration, create/delete; per-bridge private owner implements member lifecycle, bridge/port info, bridge/port priority, port path cost, bridge timing config, no-STP control, port list, FDB query, FDB ageing | `siocgif/bridge.rs`, canonical `stack/bridge.rs` FDB/port owner | enabled STP state/timers, native+compat and guest differential |
