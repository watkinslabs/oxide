...
| `SIOCGIFBR` (0x8940), `SIOCSIFBR` (0x8941) | bridge | Partial: native raw vectors implement version, bridge enumeration, create/delete; per-bridge private owner implements member lifecycle, bridge/port info, bridge/port priority, port path cost, bridge timing config, STP enable/disable, port list, FDB query, FDB ageing; canonical owner emits/adopts IEEE 802.1D Configuration BPDUs, selects roots, and runs port-state timers | `siocgif/bridge.rs`, canonical `stack/bridge.rs` FDB/port owner, `stack/bridge_stp.rs`, `stack/bridge_stp_bpdu.rs` | topology-change BPDU/ack semantics, native+compat and guest differential |
