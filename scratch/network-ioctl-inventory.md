...
| `SIOCDARP` (0x8953), `SIOCGARP` (0x8954), `SIOCSARP` (0x8955) | ARP / neighbour | Partial: native 68-byte `arpreq` lookup, dynamic/permanent Ethernet binding mutation, and delete route to the canonical per-interface owner; proxy/netmask forms explicitly unsupported | `siocgif/arp_ioctl.rs`, `stack/arp_neighbor.rs`, `stack/ethernet.rs`, virtio neighbour path | native+compat probes, guest differential, proxy/netmask semantics, exact Linux errno/order |
