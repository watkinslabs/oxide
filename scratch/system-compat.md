...
| `T2-c` | `PARTIAL` | FAT/VFAT | A FAT volume is readable end to end: boot sector, geometry with the cluster-count width rule, the allocation table including twelve-bit shared-byte entries, directory listing with long names, path resolution, and file reads at any offset across cluster boundaries — all exercised together against a synthetic image built by an independent writer. What remains is the `FileSystemType` registration that lets `mount -t vfat` reach it, and the write path. | `crates/kernel/fatfs` (66 hosted tests); no `FileSystemType` registered | F1165-fat-volume-read |
