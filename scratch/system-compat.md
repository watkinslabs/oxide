...
| `T2-c` | `HAVE` (read-only) | FAT/VFAT | `mount -t vfat` and `-t msdos` reach a real filesystem: boot sector, geometry with the cluster-count width rule, the allocation table, directories with long names, path resolution, and file reads, published through the VFS as inodes with stable identity. The EFI system partition and an ordinary USB stick are readable. Remaining: no write path, so the mount is forced read-only; no code-page translation for 8.3 names; and `msdos` is registered as an alias of the long-name type rather than as the short-name-only filesystem it is upstream. | `crates/kernel/fatfs` (80 hosted tests, including mounts over a block device whose block size differs from the volume's sector size); registered in `syscalls::fsmount_common::registry` | F1166-fat-mount |
