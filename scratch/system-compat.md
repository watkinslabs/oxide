...
| `T2-c` | `HAVE` (read/write files; no create/delete) | FAT/VFAT | `mount -t vfat` mounts read-write. Reading is complete; writing covers a file's bytes, growth by chain extension, truncation, and the record's size and first cluster, with every copy of the table updated and the dirty flag set at mount and cleared at unmount. A volume left dirty mounts read-write with a warning, as the reference does. Remaining: creating, deleting and renaming names, timestamps, and code-page translation for 8.3 names — each filed. | `crates/kernel/fatfs` (117 hosted tests) | F1168-fat-writable-mount |
