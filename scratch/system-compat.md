...
| `BLOCKED` | A boot reaches a login prompt and stays there (aarch64) | Never reaches one: `systemd-journald` start times out at ~56 s, is killed at ~147 s and restarts, in 3 of 3 boots. The recorded stack overflow did NOT recur in those 3 boots; its structural cause (task path + exception reserve > 16 KiB) is measured and half-gated | B2241 |
