...
| `T2-b` | `PARTIAL` | Loop devices | The driver exists and publishes `/dev/loop0`..`loop7` under the reference's fixed major: window arithmetic, block-size rules, status and configure validation with its refusal order, the pre-64-bit conversions, the control-node index rules, and the block device itself whose reads and writes reach a file. What remains is the wiring — `/dev/loop-control` as a character device, the `LOOP_*` ioctl entry points in the syscall shim, and the boot-time registration call — so nothing can reach it from userspace yet. | `crates/drivers/drv-loop`, 58 hosted tests; no `/dev/loop-control` node and no `LOOP_*` arm in `crates/kernel/syscalls/src/016_ioctl` | F1160-loop-block-device |
| `T2-c` | `NOT FOUND` | FAT/VFAT | No FAT owner. The EFI system partition cannot be mounted, so `/boot/efi` fails on any UEFI install, and no ordinary USB stick mounts | source-tree audit under `crates/kernel` | — |
