...
| `T2-c` | `PARTIAL` | FAT/VFAT | Volume layer and directory layer: boot-sector validation in the reference's refusal order, geometry with the cluster-count rule that derives FAT12 from FAT16, and the 32-byte records including long-name assembly — reversed slots, ordinals, the alias checksum, and the fall-back-to-short-name rule a mismatch triggers. What remains is the cluster-chain walk, the read and write paths, and the `FileSystemType` registration, so no volume mounts yet. | `crates/kernel/fatfs` (38 hosted tests); no `FileSystemType` registered | F1163-fat-directory-entries |
