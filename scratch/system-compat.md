...
|---|---|---|---|---|---|
| `T1-a` | `NOT FOUND` | Modesetting driver for a physical GPU | The DRM framework exists, but every driver bound to it is virtual or a firmware framebuffer: virtio-gpu, bochs, simplefb. On a physical machine nothing drives a display beyond whatever mode the firmware left | `crates/drivers/{drm,drv-virtio-gpu,drv-bochs,drv-simplefb}`; no `i915`/`amdgpu`/`nouveau`/`vc4` owner in `crates/drivers` | — |
...
| `T2-b` | `NOT FOUND` | Loop devices | No `loop`/`loop-control` owner: `LOOP_SET_FD` appears nowhere. Mounting a disk image or an ISO is impossible, from the desktop or the shell | source-tree audit under `crates/kernel` and `crates/drivers` | — |
