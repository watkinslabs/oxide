...
| `T2-c` | `PARTIAL` (read mounts; write layer landing) | FAT/VFAT | Reading is complete and mountable (`mount -t vfat`). The write layer's table half is in: cluster allocation with a wrapping search and a hint, transactional failure that claims nothing, chain linking and truncation ordered so an interrupted write reads as a short chain rather than a dangling link, the twelve-bit shared-byte merge, the FAT32 reserved-bit preserve, the dirty flag at its per-width offset, and the start of every table copy. Remaining before a volume is writable end to end: writing those buffers back through the device, directory-entry updates, and the mount dropping its forced read-only. | `crates/kernel/fatfs` (104 hosted tests) | F1167-fat-write-allocation |
