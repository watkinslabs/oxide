...
| `T2-c` | `PARTIAL` | FAT/VFAT | Volume, directory and allocation-table layers: boot-sector validation in the reference's refusal order, geometry with the cluster-count rule deriving FAT12 from FAT16, the 32-byte records with long-name assembly, and the table itself — the shared-byte 12-bit entries, the 28-bit FAT32 entry, the fold of bad-cluster marks into end-of-chain, and a chain walk that refuses a loop, a link past the volume and a free entry mid-chain. What remains is the read and write paths and the `FileSystemType` registration, so no volume mounts yet. | `crates/kernel/fatfs` (52 hosted tests); no `FileSystemType` registered | F1164-fat-cluster-chain |
