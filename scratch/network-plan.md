...
| N24 network ioctl (16) | `IN-PROGRESS` — `F700-bridge-ioctl-owner` claimed | namespace/capability routing, ifreq uaccess, interface owner operations | full socket/interface plus driver/file ioctl surface, compat, exact error order, runtime differential | establish the canonical bridge-device owner and its raw-pointer/ifreq socket ABI paths; D352 inventory remains authoritative for other owner families |
