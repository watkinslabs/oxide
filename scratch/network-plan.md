...
| N26 VSOCK | `PARTIAL` | atomic lifecycle, waits, SIGPIPE, core `SOL_VSOCK` options; B1265 defines exact virtio SEQPACKET wire/feature ABI, B1266 separates immutable protocol personality and current virtio DGRAM contracts, and B1267 owns record RX/TX through file I/O and `recvmsg`/`sendmsg`, including EOM/EOR, truncation, copy-fault discard, record admission, partial-emission reset, and negotiated virtio feature gating | complete option ABI, guest blocked I/O/accept, Linux differential | add record-specific owner tests, then execute guest blocked-I/O/accept and Linux differential matrices |
