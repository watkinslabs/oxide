...
| N14 `listen` (50) | `PARTIAL` | B1072 normalized VSOCK backlog propagation; current-tree B1268 evaluates VSOCK `Listen` admission before protocol publication and clamps against the socket-retained namespace's live `somaxconn`; AF_UNIX datagram and AF_PACKET now preserve Linux `sock_no_listen`/`EOPNOTSUPP` rather than leaking generic `EINVAL` | fd/type/backlog ordering, SYN/accept queues, reuseport, UNIX/VSOCK parity, security, differential | complete backlog/reuseport family matrix with N20 |
