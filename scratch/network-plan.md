...
| N29 `socket` (41) | `IN-PROGRESS` `[CLAIMED B1300-unix-unbound-socket 2026-07-20]` | reproduced source audit: AF_UNIX STREAM/SEQPACKET construction relabeled `TcpInit`, so `bind_file` had no Unix receive-queue owner | Linux `unix_create1`-equivalent unbound owner, stable SCM/GC receive queue through bind/connect/release, constructor and lifecycle tests, target builds | B1300 creates the real AF_UNIX unbound endpoint; do not alter N16 socketpair or N19 security scope without an independent contract failure |
