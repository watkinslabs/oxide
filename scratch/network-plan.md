...
| N26 VSOCK | `PARTIAL` | atomic lifecycle, waits, SIGPIPE, core `SOL_VSOCK` options; B1265 defines exact virtio SEQPACKET wire/feature ABI, B1266 separates immutable protocol personality and current virtio DGRAM contracts, and B1267 owns complete-record RX through `recvmsg`, plus record TX admission, fragmentation, EOM/EOR emission, and reset-on-partial-emission in the VSOCK data-plane owner | wire record TX/RX into file I/O and `sendmsg`, negotiated-feature admission, complete option ABI, guest blocked I/O/accept, Linux differential | finish socket work-layer routing, then negotiate and gate virtio SEQPACKET before socket creation |
