...
| N26 VSOCK | `PARTIAL` | atomic lifecycle, waits, SIGPIPE, core `SOL_VSOCK` options; B1265 introduces exact virtio SEQPACKET wire/feature constants without advertising unsupported socket behavior | complete DGRAM/SEQPACKET transport ownership, option ABI, guest blocked I/O/accept, Linux differential | implement typed DGRAM/SEQPACKET owners and virtio transport capability negotiation before admitting either socket type |
