...
| N29 `socket` (41) | `IN-PROGRESS` `[CLAIMED B1349-net-row41-socket-corpus 2026-07-23]` | B1300 merged the AF_UNIX unbound endpoint owner. B1349 adds the row-41 corpus `t_socket` and closes three reproduced Linux divergences: `unix_create` accepts protocol `PF_UNIX`; `unix_create` rewrites `SOCK_RAW` to the datagram personality so `SO_TYPE` reports `SOCK_DGRAM`; `__sock_create` range-checks the family before the type, so an out-of-range family outranks an out-of-range type while an in-range unregistered family does not | AF_PACKET/AF_VSOCK creation cases, namespace and privileged-capability probes, ARM guest record | continue the row-41 corpus with privileged and packet/vsock families |
| N-connect `connect` (42) | `PARTIAL` `[B1354 t_connect]` | B1354 adds the row-42 corpus `t_connect`; source audit of `042_connect.rs` + `sock::ops::connect` confirms parity: EBADF/ENOTSOCK, ECONNREFUSED to a closed port, short-addrlen EINVAL, wrong-family EAFNOSUPPORT, UDP default-peer connect/reconnect and AF_UNSPEC disconnect (`Disc::Udp` clears peer; `Disc::Bad` -> EINVAL), EISCONN on connected-pair reconnect — all match the Linux oracle (`t_connect` PASS x86_64, aarch64 compile PASS). connect's owner is `#[cfg(oxide-kernel)]`, so kernel-level tests need the hosted-cfg refactor tracked separately; remaining: blocking/EINPROGRESS, AF_PACKET/NETLINK, guest frame. |
...
| N10 `recvmsg` (47) | `PARTIAL` `[B1362 namelen parity]` | B1362 fixes a real Linux divergence: the recvmsg import read `msg_namelen` as a bare `u32` with no validation, so a negative `msg_namelen` (with a name buffer) consumed the datagram and returned bytes where Linux `__copy_msghdr` returns EINVAL before the receive; an oversized value is now clamped to `sockaddr_storage` (128). Gated on `msg_name != NULL` (a NULL name ignores the length, matching the oracle). Verified on the oracle (recvmsg name!=NULL namelen=-1 → EINVAL; name=NULL → success; namelen=200 → clamped, out=real length) and hosted `recv_user` tests (164/164). sendmsg `msg_namelen > 128` fixed in B1363: both send name-copy paths (`import_name_with` + `copy_sockaddr`) now clamp to sockaddr_storage and send (Linux `__copy_msghdr`), matching the oracle (namelen=130 → send; namelen<0 → EINVAL; NULL name → ignored). The address parser reads only the family's struct from the clamped prefix, so the clamp is safe. Both send and recv now match `__copy_msghdr` fully. audit disproved the MSG_CMSG_CLOEXEC claim — the oracle echoes it in msg_flags, so Oxide matches. `prior N10:` | B1067 cmsg-copyout fault propagation; current-tree B1268 admits INET `MSG_ERRQUEUE` before it consumes an extended error, VSOCK `MSG_OOB` before its unsupported-operation result, and direct INET-OOB/TCP/AF_UNIX receive owners before family-specific handling; B1289/`e3511eb91` rejects native `MSG_CMSG_COMPAT` before fd lookup or msghdr import, with hosted invalid-fd/msghdr order and `MSG_CMSG_CLOEXEC` preservation fixtures | extended errors/control, OOB, VSOCK, compat, security, syscall-context differential | continue the Linux receive transaction audit and add missing corpus cases |
| N11 `recvmmsg` (299) | `PARTIAL` `[CLAIMED B1282-mmsg-ordering-corpus 2026-07-20]` | expanded current-tree `t_mmsg` x86 guest frame: timeout-before-fd, available-prefix, partial fault, and nonblocking timeout ordering | compat layout, restart/SA_RESTART, blocking-timeout/error ordering, cross-protocol errors, ARM differential | audit and retain the Linux-derived mmsg ordering corpus; add blocking/restart, control, and cross-family probes |
| N12 `shutdown` (48) | `PARTIAL` `[B1353 t_shutdown]` | B1353 adds the row-48 corpus `t_shutdown` and two kernel-level hosted tests, proving parity: EBADF before `how` validation, invalid-`how` EINVAL after security admission, unconnected TCP/UDP latch-then-ENOTCONN, listener SHUT_RDWR success, idempotent double SHUT_WR, connected-pair SHUT_WR delivering peer EOF — all match the Linux oracle (`t_shutdown` PASS x86_64, aarch64 compile PASS; `sock::shutdown` hosted 7/7). B1069 connected-UDP `SHUT_RD`; `21da15611` classifies AF_NETLINK as a socket, applies shutdown security admission, then returns Linux `sock_no_shutdown`/`EOPNOTSUPP`; `0d38a58c7` moves INET and VSOCK invalid-`how` validation behind canonical shutdown admission; `5eaaaf98a`/`e3870fb6c` classify AF_PACKET as Linux `sock_no_shutdown`, including invalid-`how` ordering; current-tree B1268 gives AF_UNIX listeners Linux shutdown ownership: read shutdown refuses new connects, retains queued accepts, reports terminal accept `EINVAL` only after draining, and publishes RDHUP/HUP readiness without closing the listener; INET shutdown now atomically cancels `SYN_SENT` for either direction, tears down TCP listeners only for read shutdown, and latches unconnected UDP/raw/TCP shutdown before Linux `ENOTCONN` | all remaining families, half-close wake/data/error rules, security, differential | complete full Linux `__sys_shutdown` family matrix and runtime differential |
| N13 `bind` (49) | `PARTIAL` `[B1356 real fixes]` | B1356 closes three reproduced Linux divergences (all confirmed on the host+raw-syscall oracle): (1) no per-family minimum addrlen check — `bind(inet_fd, sockaddr_in, 8)` succeeded; now v4 requires >=16 and v6 requires SIN6_LEN_RFC2133=24 before the family check, EINVAL otherwise. (2) sufficient-length family mismatch returned EINVAL; Linux returns EAFNOSUPPORT (a too-short recognized-family request stays EINVAL via its own length check first — the exact asymmetry the oracle shows). (3) AF_UNSPEC was rejected outright; Linux `__inet_bind` accepts AF_UNSPEC as a v4 INADDR_ANY bind (EAFNOSUPPORT for a non-zero address, and no exception for v6 sockets). Also fixes a latent over-strict `require_sockaddr_in6` (was 28, Linux min 24) shared with connect, via a length-aware `read_sockaddr_in6_len` that reads the optional `sin6_scope_id` only when present. Verified against the full 13-case `t_bind` oracle matrix (PASS x86_64 ABI, aarch64 compile, both kernel builds). B1070 full sockaddr readable-range validation; `182d416b5` gives AF_NETLINK a canonical namespace/protocol/port-ID owner, validates its sockaddr, applies bind admission, and rejects live explicit-ID collisions before group publication; current-tree B1268 passes AF_INET6 TCP `sin6_scope_id` through the same resolved interface owner as UDP, admits AF_VSOCK bind from that socket's retained namespace, and applies Linux VSOCK and INET reserved-port policy with the network namespace owner's user-namespace-scoped `CAP_NET_BIND_SERVICE` before reservation; canonical socket bind admission now runs before AF_UNIX node creation and AF_PACKET mutation, with one consumed admission token rather than duplicate checks | reuse/TIME_WAIT, remaining family parity, syscall-context ordering, runtime differential | Linux bind audit and family/reuse corpus |
| N14 `listen` (50) | `PARTIAL` `[B1355 real fix]` | B1355 fixes a real Linux divergence: datagram and raw inet `listen()` returned EINVAL (fell into `listen_tcp`'s bad-state path) where Linux `inet_dgram_ops`/`inet_sockraw_ops` use `sock_no_listen` -> EOPNOTSUPP. The owner now rejects `Udp`/`Raw4`/`Raw6`/`Packet` with EOPNOTSUPP before the TCP path; only stream kinds reach `listen_tcp` (whose EINVAL is Linux `inet_listen`'s bad-`sk_state`). Verified on the host oracle (`udp/udp6/unix_dgram listen` -> errno 95) and the `t_listen` corpus (PASS x86_64, aarch64 compile PASS, both kernel builds). B1072 normalized VSOCK backlog propagation; current-tree B1268 evaluates VSOCK `Listen` admission before protocol publication and clamps against the socket-retained namespace's live `somaxconn`; AF_UNIX datagram preserves its Linux `unix_listen` `EOPNOTSUPP`, and AF_PACKET preserves `sock_no_listen`/`EOPNOTSUPP`, rather than leaking generic `EINVAL` | fd/type/backlog ordering, SYN/accept queues, reuseport, UNIX/VSOCK parity, security, differential | complete backlog/reuseport family matrix with N20 |
| N15 `getsockname`/`getpeername` (51/52) | `PARTIAL` `[B1352 t_sockname]` | B1352 adds the row-51/52 corpus `t_sockname` and proves current parity: EBADF/ENOTSOCK/ENOTCONN classification, unbound `family=AF_INET`+zero-port, IPv4 16-byte / IPv6 28-byte / AF_UNIX 2-byte lengths, value-before-length truncation writing the actual length back, and connected-pair `client_local==server_peer` / peer==listener identity all match the Linux oracle byte-for-byte (`xtask glibc-test --tests t_sockname` PASS x86_64, aarch64 compile PASS). B1071 value-before-length copyout; `378b934d5` classifies AF_NETLINK peer queries; `cbf99ffdf` gives Netlink one socket-owned connected destination used by `connect`, destination-less `sendmsg`, and `getpeername`; `f816d3db6` reports its bound multicast mask through `getsockname`; current-tree B1268 preserves Linux AF_PACKET `packet_getname(peer)` `EOPNOTSUPP`, emits its local `sockaddr_ll` from packet/interface ownership, reads native IPv6 local/peer state with Linux interface-scope encoding, preserves accepted TCP local/peer names from the canonical transport entry, and reports retained bound/connected AF_UNIX datagram addresses rather than only the family or `ENOTCONN`; B1290/`c742ceb27` restores Linux `move_addr_to_user` full-length writeback before sockaddr-copy `EFAULT` (focused hosted 4/4, net-common hosted 4/4, x86 build, clean integrated ARM build at `c742ceb27`) | other family/disconnected states, scope IDs, complete uaccess ordering, security, differential | continue the full Linux name-query audit and add differential coverage |
| N16 `socketpair` (53) | `PARTIAL` `[CLAIMED B1291-socketpair-nosupport 2026-07-20]` | B852/B1105/B1106 atomic publication, usercopy, argument ordering; current-tree B1268 defers family/type parsing until after the reserved-pair user copyout, matching Linux `__sys_socketpair` ordering, and maps AF_UNIX `SOCK_RAW` to Linux's datagram socketpair personality | UNIX stream/datagram/seqpacket parity, errno/lifetime/permission/side effects, differential | return Linux `sock_no_socketpair`/`EOPNOTSUPP` after the existing reservation/copyout transaction for valid supported non-AF_UNIX families, then continue the direct corpus |
**IP/IPv6 option validation (2026-07-24, B1368/B1369).** IP_TTL now accepts -1
(use default) per Linux (B1368); getsockopt resolves an unset (-1) IPV6_UNICAST_HOPS
/ IPV6_MULTICAST_HOPS to the effective default (64/1) to match the TX path (B1369).
IP_MULTICAST_TTL, IPV6 hop-limit set-side, IP_TOS, and MTU_DISCOVER validation all
verified correct. **Characterized missing option (needs a focused impl, not a
stub):** IPV6_TCLASS (and IPV6_RECVTCLASS ancillary) — entirely unimplemented, so
set/get return ENOPROTOOPT where Linux accepts and defaults to 0. A faithful
implementation needs an `opts.ipv6_tclass` field, set/get with the -1→default rule,
AND wiring the traffic-class byte through the several `push_ipv6_header_hop` TX
sites (currently hardcoded 0); a set/get-only version would be a hollow stub.
Deferred to a focused session with guest verification of the TX change.

**Socket-option quirks (2026-07-24, B1359/B1360/B1364).** setsockopt error
precedence (B1359), getsockopt unknown-level EOPNOTSUPP (B1360), and
SO_RCVBUF/SO_SNDBUF value doubling + minimum floor (B1364) all fixed and
verified against the oracle. **Characterized open items (sysctl-dependent,
deferred — need sysctl infra + guest verification, not safe to change blindly):**
(1) `rmem_max`/`wmem_max` cap — a SO_*BUF request above the ceiling is not
clamped down (Oxide has no such sysctl; `sk_buf_value` only doubles+floors).
(2) fresh-socket default SO_RCVBUF: Oxide `TCP_RCVBUF_DEFAULT`=16384, Linux
returns rmem_default*2 (131072 on the reference host, tuning-dependent);
SO_SNDBUF default 16384 already matches. Changing the constant would 8x the
AF_PACKET default receive accounting limit, so it needs the tcp_rmem/rmem_default
sysctl owner, not a bare constant bump.

| N17/N18 options (54/55) | `PARTIAL [B1359 setsockopt precedence]` | B1359 fixes two reproduced `setsockopt` error-precedence divergences (confirmed on the oracle): a blanket `optval==0 -> EFAULT` guard ran before the per-option short-optlen and unknown-level checks, so `setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, NULL, 2)` returned EFAULT (Linux EINVAL — short optlen checked first) and `setsockopt(fd, <unknown-level>, x, NULL, 4)` returned EFAULT (Linux ENOPROTOOPT — unknown level never reads optval). The guard is removed and all 23 int/byte option arms route through precedence-correct readers (`read_i32_required`/`read_u8_or_i32_required`: short->EINVAL, bad-ptr->EFAULT); the change is behavior-preserving for every well-formed input (only NULL/bad-optval error codes move). getsockopt (55): B1360 fixes an unknown-LEVEL divergence the audit missed — the catch-all returned ENOPROTOOPT for every unmatched (level,opt), but Linux returns EOPNOTSUPP for an unrecognized level on a non-IPv6 socket (v4/unix) while IPv6 keeps ENOPROTOOPT; an unknown OPTION at a recognized level stays ENOPROTOOPT for all families. Characterized on the oracle (v4 TCP/UDP/unix unknown level → 95; v6 → 92; unknown opt → 92) and verified via `t_getsockopt`. Only the malformed-level path changes (real programs use recognized levels). Verified via the `t_setsockopt` oracle matrix + both kernel builds; syscalls 161/161. Row 44 sendto, 45 recvfrom, 53 socketpair, 55 getsockopt all audited MATCHES. `N17/N18 options prior:` | B1108-B1116 uaccess/readback; multicast/TCP/packet work; current-tree B1268 routes AF_VSOCK and AF_NETLINK option owners plus shared socket-filter and `SO_ERROR` dispatch through retained-namespace `Option` admission before ABI-specific handling, resolves `SO_BINDTODEVICE` readback in that same retained namespace, and names every implemented `setsockopt`/`getsockopt` SOL_SOCKET/IP/IPV6/TCP selector in its owning UAPI module | complete SOL_SOCKET/IP/IPV6/TCP/PACKET/UNIX/NETLINK/VSOCK matrix, LSM/capability, filter ABI, teardown, differential | publish one option inventory, then implement by owner rather than one option at a time |
| N19 security | `PARTIAL [B1294 merged ad47256f8 2026-07-20]` | B1075-B1093/B1237 canonical namespace/operation admission; current-tree B1268 routes AF_VSOCK bind/connect/listen/accept and AF_NETLINK connect/send/receive through retained namespace admission before protocol state transitions or data consumption, admits direct INET TCP/AF_UNIX file read/write owners before queue access, and admits retained raw/VSOCK `MSG_OOB` rejection shared by `sendmsg` and `sendmmsg` | Linux syscall-context enforcement, teardown and runtime allow/deny/counter differential | security policy corpus across all socket operations |
| N20 TCP edges | `IN-PROGRESS` `[CLAIMED B1287-tcp-info-abi 2026-07-20]` | hosted OOB, backlog, reuseport, retransmit, RST, TIME_WAIT, keepalive work; D353 Linux-owner inventory | remaining SYN/cookie, accept/reuseport, urgent, async-error, PMTU and runtime matrices | restore full Linux `TCP_INFO` ABI copyout before retaining target urgent/diagnostic frames |
| N21 teardown | `IN-PROGRESS` `[CLAIMED B1288-raw4-fragment-teardown 2026-07-20]` | transport/raw/packet close, poll wakeups, fragment/NDP isolation | every family across move/remove/final-drop, blocked I/O, real poll/epoll, multicast/routes/neighbors/diagnostics, runtime differential | prove namespace-owned Raw4 fragment queues cannot cross namespaces and final-drop removes their table |
| N22 differential harness | `IN-PROGRESS` `[CLAIMED C134-as-lifetime-provenance 2026-07-21]` | B1253 fixes host-loader startup; expanded current-tree x86 `t_mmsg` guest frame matches host; C118/PR #3683 (`939c9a332`) now fails closed when the selected host oracle cannot complete, so no retained guest frame lacks its Linux control | real Oxide guest comparison for rows 41-55/299/307: return, errno, bytes, flags/cmsg, blocking, side effects, both arches | C119 moves kernel/ISO preparation before the capped QEMU interval; B1283 makes ARM mprotect mutate its explicit address-space root; C120 persists atomic pre-QEMU phase/terminal evidence; C124 replaces serial-flooding base ARM mprotect operation records with aggregate retired-AS checkpoints and retains separately armed detail; C125 makes both SSH and ZRAM readiness fail immediately only when both this run's launcher and QEMU are gone, while recording the terminal harness cause; C126 keeps diagnosis feature-gated while marking AArch64 sshd exec success and its socket/bind/listen/accept4 entry/exit path; B1295 restores Linux `d_add` instantiation for fresh entries, so the existing lookup `iput` leaves the dentry alias and its durable inode hold live; C127's retained per-call trace now requires explicit `debug-getdents-detail`; C128 makes base `debug-getdents` task-owned active stage plus VFS backend/block/progress metadata emitted only on watchdog/task dumps; C129 moves the retained per-syscall sshd trace behind explicit `debug-sshd-detail`, leaving base `debug-sshd` lifecycle-only; C130 adds watchdog-only task-owned syscall-return tail stages after dispatch/diagnostics/timers/rseq/ptrace; C133 makes retained periodic task dumps selectable through both kernel binaries and records an x86 `systemd-udev-trigger` fault in deferred `Dentry::drop` inode release; B1307 owns the Linux-shaped VFS repair; C134 retains compact canonical AS/VMA ownership provenance; its bounded x86 run reaches guest 84s without a page fault but SSH readiness still fails, and disproves lazy-TLB premature mmdrop (hardware root changed before each observed final drop); next diagnostic owner is VmaTree allocation/free provenance after B1308 rmap coordination; ARM guest differential remains open |
| N23 `sendmmsg` (307) | `PARTIAL` `[CLAIMED B1282-mmsg-ordering-corpus 2026-07-20]` | native/compat importers, Linux `UIO_MAXIOV` cap, VSOCK security; current-tree B1268 routes retained raw/VSOCK `MSG_OOB` through the canonical send admission before its Linux unsupported-operation result; expanded x86 `t_mmsg` frame | blocking/signal/restart, control/cross-family cases, compat ABI, ARM differential | audit and retain the Linux-derived mmsg ordering corpus; add blocking, signal, control-message, and cross-family probes |
| N24 network ioctl (16) | `IN-PROGRESS` `[CLAIMED B1286-siocwandev-owner 2026-07-20]` | namespace/capability routing, ifreq uaccess, interface owner operations | full socket/interface plus driver/file ioctl surface, compat, exact error order, runtime differential | route `SIOCWANDEV` through canonical device WAN settings ownership, then continue the command inventory |
| N25 TCP wait | `IN-PROGRESS` B1303 | lock-coupled connect/write waits and hosted race tests; B1303 target corpus covers timeout, signal interruption, ACK writable wake, reset error consumption, and orderly FIN | x86+ARM guest differential records; any mismatch fixed in canonical TCP wait owner | run the expanded target matrix after N22 restores the guest execution channel; do not alter proven wait ownership without a differential failure |
...
- [~] **N16 socketpair row 53**. Claimed by `B1291-socketpair-nosupport`
  (2026-07-20).
...
- [ ] **N20 TCP Linux edge semantics**. B1181 adds deterministic contention
  ...
  B1284 adds the canonical loopback packet fixture for established TCP input
  and output. It captures and validates IPv4/TCP tuple, checksum, ACK, sequence,
  acknowledgment, and payload before each accepted in-window data frame enters
  TCP demux; bounded convergence drains emitted ACKs. It does not cover
  SYN/RST/URG/FIN, options, or window-edge behavior. Retained target urgent-data
  and `TCP_INFO` frames are still required and are not replaced by this fixture.
