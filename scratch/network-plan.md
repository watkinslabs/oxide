...
| N26 VSOCK | `PARTIAL` | atomic lifecycle, waits, SIGPIPE, core `SOL_VSOCK` options; B1266 separates immutable DGRAM/STREAM/SEQPACKET personality from mutable lifecycle state and admits DGRAM only through current virtio's real create/bind/read/shutdown/poll contracts | complete DGRAM peer/address/message owner, SEQPACKET transport ownership, option ABI, guest blocked I/O/accept, Linux differential | route destination and record semantics through typed DGRAM/SEQPACKET owners, then negotiate virtio SEQPACKET only when complete |
