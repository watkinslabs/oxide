...
| N26 VSOCK | `PARTIAL` | atomic lifecycle, waits, SIGPIPE, core `SOL_VSOCK` options; B1265 defines exact virtio SEQPACKET wire/feature ABI, B1266 separates immutable protocol personality and current virtio DGRAM contracts, and B1267 owns complete-record RX through `recvmsg`, including EOM/EOR, peek, truncation, copy-fault discard, and credit retirement | atomic record TX, negotiated-feature admission, file-I/O parity, complete option ABI, guest blocked I/O/accept, Linux differential | make record TX all-or-error at the transport/driver boundary, then negotiate and gate virtio SEQPACKET before socket creation |
