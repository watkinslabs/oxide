...
| N15 `getsockname`/`getpeername` (51/52) | `NEEDS-AUDIT` | B1071 value-before-length copyout; `378b934d5` classifies AF_NETLINK peer queries; `cbf99ffdf` gives Netlink one socket-owned connected destination used by `connect`, destination-less `sendmsg`, and `getpeername`; `f816d3db6` reports its bound multicast mask through `getsockname`; current-tree B1268 preserves Linux AF_PACKET `packet_getname(peer)` `EOPNOTSUPP` and emits its local `sockaddr_ll` from packet/interface ownership rather than a generic INET tuple | other family/disconnected states, scope IDs, complete uaccess ordering, security, differential | full Linux name-query audit before further narrow fixes |
