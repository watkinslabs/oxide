...
| N17/N18 options (54/55) | `PARTIAL` | B1108-B1116 uaccess/readback; multicast/TCP/packet work; current-tree B1268 routes AF_VSOCK set/get option access and shared socket-filter option dispatch through retained-namespace `Option` admission before ABI-specific handling | complete SOL_SOCKET/IP/IPV6/TCP/PACKET/UNIX/NETLINK/VSOCK matrix, LSM/capability, filter ABI, teardown, differential | publish one option inventory, then implement by owner rather than one option at a time |
