...
| N25 TCP wait | IN-PROGRESS (`B1270-tcp-wait-matrix`) | lock-coupled connect/write waits and hosted race tests; active-open CLOSE without `sk_err` now reports Linux `ECONNABORTED` | target scheduler signal/timeout/ACK/RST/close matrix and runtime differential | add target probe matrix and close every remaining execution row |
