...
| N26 VSOCK | `PARTIAL` | atomic lifecycle, waits, SIGPIPE, core `SOL_VSOCK` options; B1265 defines exact virtio SEQPACKET wire/feature ABI and B1266 separates immutable DGRAM/STREAM/SEQPACKET personality from mutable lifecycle state with current virtio DGRAM contracts | complete SEQPACKET transport, record lifecycle, option ABI, guest blocked I/O/accept, Linux differential | implement record TX/RX and transport capability negotiation before admitting SEQPACKET |
