...
| N26 VSOCK | `PARTIAL` | atomic lifecycle, waits, SIGPIPE, core `SOL_VSOCK` options; B1266 separates immutable DGRAM/STREAM/SEQPACKET personality from mutable lifecycle state | complete DGRAM/SEQPACKET transport ownership, option ABI, guest blocked I/O/accept, Linux differential | route every operation through the typed owner before admitting either new socket type |
