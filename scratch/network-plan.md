...
| N22 differential harness | `IN-PROGRESS` `[CLAIMED B1283-arm-mprotect-root 2026-07-20]` | B1253 fixes host-loader startup; expanded current-tree x86 `t_mmsg` guest frame matches host; C118/PR #3683 (`939c9a332`) now fails closed when the selected host oracle cannot complete, so no retained guest frame lacks its Linux control | real Oxide guest comparison for rows 41-55/299/307: return, errno, bytes, flags/cmsg, blocking, side effects, both arches | C119 moves kernel/ISO preparation before the capped QEMU interval; B1283 makes ARM mprotect mutate its explicit address-space root and retains target tracing; ARM guest differential remains open |
