...
| N09 `sendmsg` (46) | `PARTIAL` | B1066 plus socket-owned INET/raw, VSOCK, and retained-file work; `4c62cc001` routes AF_NETLINK unicast through its canonical bound port owner and connected-peer admission; `af69682ce`/`08237bfcf` move imported and coalesced Netlink sends onto that same owner; current-tree B1268 retains the feature-gated uevent send trace on both routes | Linux syscall-context error/copy-fault ordering, security, and runtime differential | build the row-46 differential corpus and audit remaining family/control cases against Linux |
...
| N17/N18 options (54/55) | `PARTIAL` | B1108-B1116 uaccess/readback; multicast/TCP/packet work; current-tree B1268 routes AF_VSOCK set/get option access through its retained-namespace `Option` admission before ABI-specific handling | complete SOL_SOCKET/IP/IPV6/TCP/PACKET/UNIX/NETLINK/VSOCK matrix, LSM/capability, filter ABI, teardown, differential | publish one option inventory, then implement by owner rather than one option at a time |
| N19 security | `PARTIAL` | B1075-B1093/B1237 canonical namespace/operation admission; current-tree B1268 routes AF_VSOCK bind/connect/listen/accept through the retained namespace admission owner before protocol state transitions | Linux syscall-context enforcement, teardown and runtime allow/deny/counter differential | security policy corpus across all socket operations |
