...
| N26 VSOCK | `PARTIAL` | atomic lifecycle, waits, SIGPIPE, core `SOL_VSOCK` options; B1265 defines exact virtio SEQPACKET wire/feature ABI, B1266 separates immutable protocol personality and current virtio DGRAM contracts, and B1267 adds the canonical hidden-until-EOM record RX owner | complete SEQPACKET transport, record lifecycle wiring, option ABI, guest blocked I/O/accept, Linux differential | wire record TX/RX through the connection, driver, and syscall owners, then negotiate virtio SEQPACKET |
