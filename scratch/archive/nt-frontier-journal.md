...
Windows frontier update (2026-09-04): `LdrGetDllPath` now expands `LOAD_LIBRARY_SEARCH_DEFAULT_DIRS` into the application, user, and System32 roots; preserves explicit `LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR`; applies the process defaults to altered-search-path requests; rejects altered mode combined with any directory flag; and rejects drive-relative module names before constructing a load directory. The policy owner has six focused tests and a positive-control regression; the next loader boundary remains full filesystem DLL probing.
