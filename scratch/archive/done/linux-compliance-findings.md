...
Consolidates four read-only subsystem audits run against the reference implementation's
actual behaviour (v7.2.0-rc4). Each source doc carries per-row oxide file:line;
...
The asm-generic syscall numbering reserves 295..402 as unassigned; every one of those ran a real x86
...
|---|---|---|
| **TCP initial sequence numbers were a fixed constant** — `TCP_ISN_INITIAL = 0x1000_0000` stepping `0x1000` from a global counter, identical every boot; the passive/server side opened at literal **0**; ephemeral ports scanned sequentially from a fixed base | Remotely exploitable TCP sequence prediction (RFC 6528): an off-path attacker needed to guess neither the sequence number nor the source port, so blind connection reset and blind data injection against any TCP connection were arithmetic rather than search. Structural cause: `crates/kernel/net` had no `crng` dependency | Linux's secure-sequence construction — `seq_scale(siphash(4-tuple, net_secret))` with a per-boot CSPRNG key. New `crates/shared/siphash` (pinned to the reference implementation's own test vectors); `crates/kernel/net/src/secure_seq.rs` |
...
- **ASLR (`audit-mm.md` §1, `audit-net-sec.md` §A2): all six components implemented in F771.**
  ...
  randomised `mmap_base`. (b) The vDSO likewise has NO dedicated
...
- **B1434's `pkey_alloc` correction was itself wrong for x86_64 — corrected by B1479.** B1434
  ...
  holds on arm64 and NOT on x86_64: the x86 `init_new_context` sets
  ...
  `arch_set_user_pkey_access` with `EINVAL`; the rollback
  ...
  `0xF` on arm64 (`PKEY_DISABLE_READ`/`_EXECUTE` are arm64-only UAPI bits),
...
| 2 | sysfs attributes reject `O_TRUNC` (`i_op->truncate` default `EROFS`) | `echo add > /sys/class/drm/card0/uevent` → "Read-only file system" | `kernfs_iop_setattr` runs `setattr_copy` and ignores size changes |
...
The reference scheduler core, beside `try_to_wake_up`'s `smp_load_acquire(&p->on_cpu)`,
documents the ordering invariant it relies on: a waker must load `p->on_cpu` strictly
after `p->on_rq`, or it can falsely observe `on_cpu == 0`. A task must be running
(`on_cpu == 1`) to remove itself from the runqueue, so `__schedule()`'s switch-to path
stores `on_cpu = 1` before `try_to_wake_up()` loads `on_rq`, and its put-to-sleep path
stores `on_rq = 0` before a waker loads `on_cpu` — the two orderings pair to make the
combined "clear" state unobservable mid-switch.
...
Linux's scheduler core takes the rq lock FIRST and re-reads `p->on_cpu` at the activation
point, waiting the switch out if it is still set: under the rq lock, for each pending
wakelist entry it warns (and waits) if `p->on_cpu` is still set, warns (and corrects) if
the task's recorded CPU no longer matches this runqueue, then activates it.
...
| 4 | `balance.rs` | migrated the leftmost CFS task with no running check | `can_migrate_task` refuses (and counts as `nr_failed_migrations_running`) any task still marked running on its source runqueue |
...
|---|---|---|---|
| 1 | `openat2` `RESOLVE_*` | `O_CREAT` branch built its parent walk from `LookupFlags::default()`, dropping every scoping bit | `build_open_flags` folds all six into ONE `op->lookup_flags`; `path_openat` uses it for both walk phases |
| 2 | `userfaultfd` | `UFFDIO_COPY`/`ZEROPAGE` installed USER\|RW at any VA with no VMA and no registration — an arbitrary-write primitive; no creation gate at all | `validate_dst_vma`, `find_vma_and_prepare_anon`, `userfaultfd_syscall_allowed` |
| 3 | mount flags | `graft_mount` passed a hardcoded `0` as `mnt_flags`; `MNT_LOCK_*` did not exist; `require_sys_admin()` was a flat effective-set test | `path_mount`, `lock_mnt_tree`, `may_mount`, `mount_capable` |
| 4 | `seccomp` | `RET_TRACE`/`RET_LOG` failed OPEN — a filter denying by tracing was a no-op | `__seccomp_filter` |

### 12.1 Corrections to the source audit

The audit rows were treated as claims to disprove. Four were wrong:

- **userfaultfd's implementation moved subsystems in v7.2.0-rc4** (out of the
  filesystem tree, into mm). Any row citing the old location was not read against this tree.
...
- **A tracerless `SECCOMP_RET_TRACE` does not act as `RET_KILL_THREAD`.**
  `__seccomp_filter` sets the return value to `-ENOSYS` and
