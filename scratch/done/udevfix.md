...
| DONE | B613-inotify-udev-dirs | Prove or fix inotify `IN_CREATE`/`IN_DELETE`/move delivery for `/dev` and `/run/udev` directory mutations. |
| VERIFIED | B614-class-env-audit | Added driver-model uevent env propagation, virtio-input PRODUCT/NAME/UNIQ env, fbdev DEVTYPE=fb, and verified focused tests plus x86_64/aarch64 kernel builds. |
...
5. B614 verified; no remaining rows in this udevfix ledger.
