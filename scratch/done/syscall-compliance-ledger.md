...
| D6 | rename/renameat/renameat2 (82/264/316) ext4 atomic replacement | WIP | B1257 | The syscall layer resolves both parents in the caller namespace, checks sticky/ownership rules through `may_rename`, and forwards backend errors unchanged. A 2026-07-18 x86 GNOME boot nevertheless showed dconf's ordinary same-filesystem temporary-file replacement (`g_rename`) failing with `EIO` on `/home/oxide/.config/dconf/user`. The `EIO` originates below the syscall layer in ext4's journaled `rename_child` path; do not mask it. Next: capture the exact `MountError`, repair the transaction/metadata fault, and add an ext4 image regression for overwrite rename on the home filesystem. |
