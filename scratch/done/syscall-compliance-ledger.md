...
| S7 | sched_setparam(142), sched_setscheduler(144), sched_setattr(314) RT authorization | FIXED — x86 service startup | B1257 | The RT-policy paths incorrectly treated uid 0 as the authority check. Linux uses the caller's effective `CAP_SYS_NICE`; a correctly configured service may drop uid while retaining that capability. Replaced the uid shortcut in the shared RT-policy path and `sched_setparam` with the canonical capability check. `cargo test -p syscalls`, x86 and ARM target builds pass. 2026-07-18 x86 live boot: rtkit dropped privileges, entered `Running`, and no longer logged its prior `Failed to make ourselves RT: Operation not permitted`. |
...
| D5 | chmod/chown/utimes ext4 persist (90/91/92/93/132/235/260/268/280/452) | DONE | B641 #TBD | syscall `notify_change` now routes the apply through the VFS `i_op->setattr` mechanism; ext4 overrides `setattr` to journal mode/owner(+osd2 hi)/times to the on-disk inode. Read side: ext4 inode decoder now parses atime/ctime/mtime(+`i_*time_extra` ns) and iget threads them via `InodeBuilder::times`, so utimes round-trips. Hosted `setattr_persist_image` proves chmod+chown(>16-bit)+utimes(ns) survive a remount. |
| D6 | rename/renameat/renameat2 (82/264/316) ext4 atomic replacement | FIXED — x86 GNOME boot | B1257 | The syscall layer already resolved both parents in the caller namespace and forwarded backend errors. The live dconf `g_rename` `EIO` came from ext4 truncation of the overwritten destination: truncation treated raw `ee_len` as a block count, but unwritten extents encode their state in its high bit. It freed past the extent’s real end, hit an already-clear bitmap bit, and aborted rename. Truncation now uses `Extent::real_len()` everywhere and preserves the unwritten encoding when partially retained. Hosted regressions: unwritten-extent truncate frees exactly its real coverage; rename-overwrite suite passes. 2026-07-18 x86 GNOME boot: dconf completed its recovery/replacement path without rename `EIO`; `gnome-session-initialized.target` remained active. |
