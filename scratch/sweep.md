...
| CLAIMED B1518-nvme-mqes-queue-depth | B1518-nvme-mqes-queue-depth | NVMe never clamps queue size to `CAP.MQES` | `drv-nvme/src/regs.rs:57` | `Q_ENTRIES = 32` goes straight into AQA and CREATE_IO_*Q QSIZE (NVMe §3.1.8). Masked because QEMU reports a large MQES; a real controller with MQES < 31 gets an invalid queue. |
