...
| CLAIMED | B1531-net-hosted-sched-feature | `net` debug features do not compose in a hosted `--all-features` build | An optional B1530 probe found `cargo check -p net --all-targets --all-features` reaches `sched::live` through `debug-dbus`/`debug-scmfd`. Audit whether `net/hosted` must propagate the dependency's hosted implementation without changing real-kernel feature selection. |
