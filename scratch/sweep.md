...
| CLAIMED B1522-gic-ppi-level-config | B1522-gic-ppi-level-config | level-sensitive PPIs silently stay edge | `arch-irq/src/gic/regs.rs:73` | `lines.rs::enable_intid` skips PPI trigger config and `enable_intid_level` delegates PPIs to it, so `GICR_ICFGR1` is never programmed. |
