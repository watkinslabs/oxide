...
| CLAIMED | B1528-delayed-work-test-ownership | Delayed-work tests reset one shared production table concurrently | High-parallel B1527 verification exposed `live::delayed_work::tests`: `a_full_table_refuses_rather_than_dropping_silently` failed after 37 otherwise-clean branch runs, while unmodified `main` immediately failed `a_due_item_is_handed_to_the_workqueue_exactly_once`. Each test calls `reset()` on shared `PENDING`/`EARLIEST_NS` state while sibling tests enqueue and tick it. Give only this four-test group explicit ownership; keep the rest of the scheduler suite parallel. |
