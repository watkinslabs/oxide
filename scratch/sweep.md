...
| CLAIMED | B1527-preempt-reset-ownership | Scheduler BH reset races parallel tests | `sched::bh::tests::bh_disable_enable_balances_and_marks_in_interrupt` can panic `preempt_count_sub underflow`. `preempt::_test_reset()` clears every simulated CPU's count and reschedule anchor, including slots concurrently owned by other hosted-test threads. Scope the convenience reset to the caller's simulated CPU and prove ownership with a barrier-controlled regression; do not serialize the whole scheduler suite. |
