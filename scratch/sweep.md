...
| CLAIMED B1525-dr7-watchpoint-fields | B1525-dr7-watchpoint-fields | DR7 field literals open-coded next to the named constants | `hal-x86_64` `set_data_watchpoint` | The `DR7_*` set exists and is now correctly arch/feature-gated, but the watchpoint arming path still builds the register from literals. |
