...
| CLAIMED | B1532-xfs-path-test-isolation | 21 XFS quota integration tests replay the production module's unit tests | The 70-file audit found 207 direct `#[path]` edges, 66 of which resolve `cfg(test)` in the included source. The largest family points 21 binaries at `179_quotactl_xfs.rs`; its 18-test child manifest therefore runs 378 phantom copies. Split a path-includable core from the unit-test-owning production wrapper. |
