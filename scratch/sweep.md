...
| CLAIMED B1520-debugfs-null-fops | B1520-debugfs-null-fops | debugfs refuses a NULL-fops file instead of substituting a no-op | `modules/src/linux_debugfs.rs:302` | Both create paths return `null_mut()`; Linux `__debugfs_create_file` (`fs/debugfs/inode.c`) substitutes `debugfs_noop_file_operations` and still creates the entry. `NULL_FILE_OPS` is already written and unused. |
