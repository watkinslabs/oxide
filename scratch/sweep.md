...
| CLAIMED B1512-route-metrics-compliance | B1512-route-metrics-compliance | Route metrics are MTU-only instead of Linux-compatible | `netlink/src/rtnetlink_route.rs`, `netlink/src/rtnetlink/{route_ops,route_state}.rs`, `net/src/route.rs`, `docs/25-net.md §10` | B1511 fixed nested traversal but still drops `RTAX_LOCK`, WINDOW, RTT, RTTVAR, SSTHRESH, CWND, ADVMSS, REORDERING, HOPLIMIT, INITCWND, FEATURES, RTO_MIN, INITRWND, QUICKACK, CC_ALGO, and FASTOPEN_NO_COOKIE; skips their payload and value validation; loses them on route dump/notification and alias comparison; and does not apply retained metrics to route/TCP behavior. The routing doc also describes fields absent from `RouteEntry`. Preserve, validate, round-trip, and consume the complete metric set, and correct the documentation. |
...
|---|---|---|---|---|
| OPEN | — | `--rebuild-vendor` is parsed by nothing | `tools/xtask/src/gc.rs:227,255` | `rebuild_vendor` + `all_vendor_pkgs` have no call site; `tools/qemu-mcp/server.py:439` faithfully forwards the flag to `xtask grub`, which ignores it. Every agent that asked for a vendor rebuild has been booting stale vendor artifacts. This is the recorded "stale artifacts mask kernel bugs" failure mode wired into the tool agents are told to use. `cmd_grub` / `cmd_grub_aarch64` must call it before `cmd_rootfs`. |
| OPEN | — | Bridge never answers ARP for its own IPv4 | `net/src/stack/bridge_tx.rs:44` | `bridge_answer_arp` is a complete responder with zero callers. A `br0` holding an address is unreachable from the LAN. Bridge RX should call it before FDB forwarding. |
| OPEN | — | `SA_RESTORER` unchecked on x86_64 | `syscalls/src/signal_dispatch.rs:12` | Linux `arch/x86/kernel/signal_64.c:171` returns `-EFAULT` → `force_sigsegv` when the flag is absent. We pass `p.restorer` to `build_signal_frame` regardless, so a raw `rt_sigaction` caller that omitted it gets a frame returning to 0 — a fault at a bogus PC after the handler instead of dying at delivery. **aarch64 computes the right answer and throws it away**: `aarch64_restorer` honours the flag, then `fs/src/sig_dispatch.rs:106` does `let _ = restorer;` and re-derives from the vDSO (Linux arm64 `signal.c:1481` honours a user `sa_restorer`). Fix both arms together. |
| OPEN | — | `sock_diag` has no `sdiag_family` dispatch | `netlink/src/sock_diag.rs:12,14` | `handle_in` never switches on family; `family_matches` accepts anything and answers every request with an inet dump. Linux `net/core/sock_diag.c` `__sock_diag_cmd` EINVALs `family >= AF_MAX` and ENOENTs an unregistered family. `ss -x` therefore gets an inet reply. |
...
| OPEN | — | AHCI never resets the HBA | `drv-ahci/src/regs.rs:18` | `bring_up` sets only `GHC.AE`; Linux `ahci_reset_controller` drives `GHC.HR` first. Firmware-left state survives into our driver. |
| OPEN | — | AHCI writes 64-bit DMA bases without checking `CAP.S64A` | `drv-ahci/src/regs.rs:8` | `PxCLBU`/`PxFBU` are written unconditionally. On a 32-bit-only HBA that is silent wrong-address DMA. Masked because QEMU sets S64A. |
| OPEN | — | NVMe never clamps queue size to `CAP.MQES` | `drv-nvme/src/regs.rs:57` | `Q_ENTRIES = 32` goes straight into AQA and CREATE_IO_*Q QSIZE (NVMe §3.1.8). Masked because QEMU reports a large MQES; a real controller with MQES < 31 gets an invalid queue. |
| OPEN | — | zram re-compresses slots already at target priority | `drv-zram` `Slot::Raw.priority` / `compression_priority` | Recorded but never read. Linux `zram_drv.c:2405` skips a slot already at ≥ the target priority; we redo the work. |
...
| OPEN | — | debugfs refuses a NULL-fops file instead of substituting a no-op | `modules/src/linux_debugfs.rs:302` | Both create paths return `null_mut()`; Linux `__debugfs_create_file` (`fs/debugfs/inode.c`) substitutes `debugfs_noop_file_operations` and still creates the entry. `NULL_FILE_OPS` is already written and unused. |
