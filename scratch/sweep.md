...
| CLAIMED B1511-rtnetlink-route-metrics | B1511-rtnetlink-route-metrics | `parse_metrics` reads only the first attribute | `netlink/src/rtnetlink_route.rs:38-51` | Every arm of the `while` body returns, so the loop is a disguised `if` and `off` never advances (which is why it needed no `mut`). An `RTA_METRICS` blob whose first nested attr is not `RTAX_MTU` gets `Unsupported` even when `RTAX_MTU` follows. The sibling `parse_nh_attrs` at :57 does it correctly — copy that shape. |
