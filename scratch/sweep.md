...
| CLAIMED | B1530-net-min-feature-warnings | Narrow per-crate builds repeat four `net` unused-binding warnings | The 112-member audit found four unique `net` warnings repeated through dependants: `lifecycle::private_loopbacks`, `SocketMcastGate`/`SocketMcastLease`, and Ethernet ingress `l2_dst`/`metadata`. Audit the exact trace, hosted, test, and kernel consumers before applying owner gates. |
