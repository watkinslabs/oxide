...
| OPEN | — | `net` debug features do not compose in a hosted `--all-features` build | An optional B1530 probe found `cargo check -p net --all-targets --all-features` reaches `sched::live` through `debug-dbus`/`debug-scmfd`, but those features do not enable the dependency's `sched/hosted` implementation. Both real kernel targets are unaffected because `sched::live` is kernel-owned. Audit the feature edges in a separate lane. |
