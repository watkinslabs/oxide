...
| CLAIMED B1519-zram-recompress-priority | B1519-zram-recompress-priority | zram re-compresses slots already at target priority | `drv-zram` `Slot::Raw.priority` / `compression_priority` | Recorded but never read. Linux `zram_drv.c:2405` skips a slot already at ≥ the target priority; we redo the work. |
