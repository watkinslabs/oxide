...
| CLAIMED | B1533-pselect-path-test-isolation | Five poll/select integration tests replay `pselect_ppoll` unit tests | Each path-including binary discovers the same 14 child tests, creating 70 phantom executions. Split the unit-test-owning production wrapper from the core that the five harnesses intentionally compile. |
| OPEN | — | 70 integration tests still compile production modules via `#[path]` | They carry `#![allow(dead_code)]` because the lint measures each test's reach, not the kernel's. After XFS and `pselect_ppoll`, 40 direct edges into 16 targets still resolve `cfg(test)` against the INCLUDING crate; some intentionally expose hosted hooks, while five inline unit-test manifests create phantom coverage. `fs` also declares five `syscalls` trace features it does not own solely to make those foreign gates well-formed. Audit one source family per lane. |
