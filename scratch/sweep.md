...
|---|---|---|---|
| CLAIMED | B1526-listns-cursor-test | `listns` zero-capacity cursor test is order-dependent | `syscalls::s470_listns::tests::unknown_type_precedes_owner_lookup_and_zero_capacity_still_checks_cursor` snapshots the highest live UTS namespace ID, then races concurrent namespace allocation before invoking the syscall. Production correctly sees the new successor and returns 0; the test expects `ENOENT`. Use Linux's highest non-wrapping cursor to make the no-successor premise structural. |
| OPEN | — | Scheduler BH reset races parallel tests | `sched::bh::tests::bh_disable_enable_balances_and_marks_in_interrupt` can panic `preempt_count_sub underflow`. `preempt::_test_reset()` clears every simulated CPU's count and reschedule anchor, including slots concurrently owned by other hosted-test threads. Fix reset ownership in a separate lane; do not serialize the whole scheduler suite. |
