...
| CLAIMED | B1529-sched-min-feature-warnings | Narrow per-crate builds repeat six `sched` unused-import warnings | An exhaustive `cargo check -p <crate> --all-targets` audit of all 112 workspace members found six unique `sched` warnings repeated through dependants. Diagnostic imports belong to `debug-watchdog`; the wait matcher and CPU-clock types belong to the already-gated live/timers modules. Apply those exact owner gates and re-run every minimal package configuration. |
| OPEN | — | Narrow per-crate builds repeat four `net` unused-binding warnings | The same 112-member audit found only four additional source warnings: `lifecycle::private_loopbacks`, `SocketMcastGate`/`SocketMcastLease`, and Ethernet ingress `l2_dst`/`metadata`. Audit their trace/hosted consumers in a separate `net` lane after `sched` is clean. |
