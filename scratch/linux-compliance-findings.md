...
| # | Item | Evidence | Owner |
|---|---|---|---|
| 1 | ~~`[BUG] exit_to_user_mode_loop: work never cleared`~~ **FIXED — `B1476`. `need_resched` was per-CPU; Linux's `TIF_NEED_RESCHED` is per-TASK.** The flag that never cleared is `NEED_RESCHED`, and the defect **predates `B1471`** (the per-CPU `NEED_RESCHED: [Pcpu<AtomicBool>; MAX_CPUS]` array dates to 2026-05-10; `B1471` merged 2026-07-28). `B1471` only made it visible: it added a loop that RE-READS the flag after each pass plus a bound that complains. Mechanism: the timer tick sets "this CPU wants a reschedule" unconditionally (`arch-irq` `lapic/dispatch.rs`, `gic/dispatch.rs`); the work loop consumes it, `schedule()` switches away, and every tick that lands while the task is descheduled re-sets the SAME per-CPU word — so the task is handed the CPU and immediately yields again, one pass per intervening tick, until `MAX_PASSES`. Linux cannot do this: `__resched_curr` stamps `rq->curr`'s thread_info (`kernel/sched/core.c`) and `__schedule` clears it on `prev` (`clear_tsk_need_resched(prev)`, same file, label `picked:`), so a resumed task reads its OWN, already-clear flag. Fix: `Task::need_resched` + `preempt::resched` (`set/clear/test_tsk_need_resched`, `__resched_curr`-shaped remote routing through `rq(cpu)->curr`), the clear added at `__schedule`'s pick, and the dead `Runqueue::need_resched` field removed. 4 hosted tests in `sched/src/preempt/resched/tests.rs`; 3 of them fail against the pre-fix per-CPU algorithm (verified by substituting it back). The `[BUG]` marker and the pass bound are unchanged — the bound was right, the flag was wrong | `B1476` |
| 2 | **`[CPU-STALL]` = a CPU inside a long IF=0 kernel section, plus a TLB-shootdown sender spinning on it.** DIAGNOSED + protocol hardened in `B1476`; the ROOT CAUSE is architectural and open (see §10.1 below) | Every observed `[CPU-STALL]` in the campaign logs sits within ~20 ms of a `[TLB-MISSED-FLUSH] pending=<mask>` — the two are one event. `arch-irq/src/tlb.rs` `shootdown()` spun a blind 1e9 iterations (≈10–18 s, which IS the reported 10/16/17/18 s) waiting for a target's 0x42 ACK, then declared the flush missed. Both CPUs stop ticking for that window: the target because it cannot answer, the sender because it is spinning with IRQs masked. `rflags=0x6` / `0x96` on every `[NMI-BT]` — IF clear in all of them. Fixed in `B1476`: ACK is per-ROUND (a target still inside `service()` when a round is torn down could ACK the NEXT round having flushed the PREVIOUS VA — the owner then frees a frame the peer still caches, a silent stale-TLB UAF that no log line named); a target whose logical id has no hardware id clears its bit instead of guaranteeing the spin; and the blind cap is replaced by Linux's `csd_lock_wait_toolong` escalation (`kernel/smp.c`) — `[TLB-STUCK]` naming the waiter, the pending mask, the VA and the round, then re-send the IPI and NMI-backtrace each stuck CPU, repeating, and NEVER abandoning the round. `[CPU-STALL]` now also prints the wedged CPU's LIVE `rq->curr` tid / last syscall / `preempt_count` / `resched`, since the heartbeat fields are `age_ns` stale by construction and `[NMI-BT]`'s rip is unusable without the exact ELF | `B1476` |
| 2a | **ROOT CAUSE of #2, OPEN: x86_64 syscalls and faults run with IF=0 end to end.** `IA32_FMASK` masks IF (`hal-x86_64/src/syscall.rs` `SFMASK_BITS`, written at `wrmsr(IA32_FMASK, …)`) and `oxide_syscall_entry` never `sti`s; every IDT vector is an interrupt gate (`idt.rs` `GATE_INT64_KERNEL = 0x8e`) and `oxide_fault_common` never `sti`s; `schedule()` deliberately CARRIES each task's IF across the switch (`live/schedule/switch.rs` `irq_save_disable`). Linux does the opposite in both paths — `syscall_enter_from_user_mode()` calls `local_irq_enable()` immediately (`include/linux/entry-common.h`), and `do_user_addr_fault()` does the same under `WARN_ON_ONCE(!(regs->flags & X86_EFLAGS_IF))` (`arch/x86/mm/fault.c`) — and requires IRQs ENABLED at any cross-CPU call sender (`kernel/smp.c` `smp_call_function_many_cond`: `lockdep_assert_irqs_enabled()`, "Can deadlock when called with interrupts disabled"). So on this port ANY long kernel section freezes its CPU's tick and blocks every cross-CPU ACK; `[CPU-STALL]`, the missed flushes, and the standing desktop latency are all downstream of it. The incremental approach is already in flight (`dde0f0bb8` per-CPU IRQ stack "foundation for IRQs-on-in-kernel", `36d08b42b` virtio-blk busy-poll with IRQs enabled) — only three `IrqGate::save_enable()` call sites exist in the whole tree. Needs its own lane; it is not a one-PR change, because the switch path's IF-carrying design exists precisely to protect process-context locks the timer ISR takes without `irqsave` | sched/hal |
