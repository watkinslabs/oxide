...
| 3 | **Signals are only delivered at the syscall-return tail** — `take_lowest_pending()` has one call site and none on the IRQ/exception return path, either arch. | A compute-bound loop issuing no syscalls is **unkillable**: `kill -9`, `^C`, `SIGALRM` never arrive. Sibling SIGKILL from `zap_other_threads` also lands only at each sibling's next syscall return. | `B1471-signal-on-irq-return` in flight |
