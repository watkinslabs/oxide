...
| 87 | `unlink` | no `is_local_mountpoint`→`EBUSY`. **Unlink-while-open FIXED (B1479):** `Mount::unlink` now `orphan_add`s at `links_count==0` and frees nothing; `Ext4SuperOps::evict_inode` truncates + frees at the last `iput`, so an open fd keeps its blocks and its data | ext4 | `fs/ext4/namei.c:3299 ext4_orphan_add`; `fs/inode.c:1975` | `ext4/src/ialloc.rs unlink`; `rootfs/evict.rs` | S |
...
| 165 | `mount` | the `MS_*` word never reaches a fresh or bind graft — `mount -o noexec,nosuid,ro` silently produces a rw/suid/exec mount; options string dropped by every fs ctor but 4; propagation branch discards its error | syscalls | `fs/namespace.c path_mount`, `:2983`, `:2862-2909` | `165_mount.rs:42,237,288-293,341` | L |
...
| 263 | `unlinkat` | inherits row 87's mountpoint-`EBUSY`. Row 87's ext4 unlink-while-open block free is FIXED (B1479) — same code path | ext4 | `fs/namei.c:5504`; `fs/ext4/namei.c` | `ext4/src/ialloc.rs unlink`; `rootfs/evict.rs` | S |
...
| 317 | `seccomp` | the filter IS evaluated per syscall, but 9 behaviours are absent — NNP/`CAP_SYS_ADMIN` gate, all flags discarded (TSYNC, NEW_LISTENER), `RET_TRACE`/`RET_LOG` fail OPEN, `KILL_PROCESS`→`KILL_THREAD`, no `TRAP` siginfo, `instruction_pointer` 0, no cBPF verifier, no user-notif, `seccomp_mode` not copied on fork | security | `kernel/seccomp.c:691,1025,1966,1276` | `security/src/seccomp.rs:301,314,347,384` | L |
...
| 323 | `userfaultfd` | only MISSING is wired (WP registration succeeds and never delivers; MINOR/MOVE/POISON absent); `uffdio_api` 16 bytes vs 24 with `ioctls` never written; range-ioctl bitmap off by one; COPY/ZEROPAGE install into `current_mm()` not `ctx->mm`, no `EEXIST`, and fall back to USER\|RW with NO VMA covering dst; no `userfaultfd_syscall_allowed` gate | fs+mm-vmm | `mm/userfaultfd.c:4481-4503`; `uapi/linux/userfaultfd.h:50-57,162` | `fs/src/userfaultfd/ioctl.rs:34-40,113-132` | L |
...
| 329 | `pkey_mprotect` | FIXED (B1479). Shares `do_mprotect_pkey` with slot 10, so the pkey refusal now follows the address/len/prot checks; the key is tested against this mm's real `pkey_allocation_map`, so x86 without OSPKE refuses key 0 (`execute_only_pkey` is 0) and arm64 accepts it | syscalls | `mm/mprotect.c:836-876`; `arch/x86/include/asm/pkeys.h:53-72` | `010_mprotect.rs`; `syscalls/src/pkey.rs` | — |
| 330 | `pkey_alloc` | FIXED (B1479), correcting B1434. x86_64: `EINVAL` once per mm then `ENOSPC` (empty map + no `arch_pkeys_enabled` guard + a rollback that fails because key 0 == `execute_only_pkey`); aarch64: `ENOSPC` from the first call. `PKEY_ACCESS_MASK` is also per-arch (arm64 adds `PKEY_DISABLE_READ`/`_EXECUTE`) | syscalls | `mm/mprotect.c:999-1027`; `arch/x86/kernel/fpu/xstate.c`; `arch/x86/include/asm/mmu_context.h:157-162` | `syscalls/src/pkey.rs`; `mm-vmm/src/address_space/pkeys.rs` | — |
| 331 | `pkey_free` | FIXED (B1479). `mm_pkey_free` against this mm's real map: arm64 `pkey_free(0)` succeeds once (its `init_new_context` reserves `BIT(0)` unconditionally) then `EINVAL`; x86 without OSPKE is `EINVAL` for every key | syscalls | `arch/arm64/include/asm/pkeys.h:49-60` | `331_pkey_free.rs`; `mm-vmm/src/address_space/pkeys.rs` | — |
...
| 432 | `fsmount` | flags/attrs complete; `require_sys_admin()` is a flat effective-set test, not `ns_capable(fc->user_ns, …)`; `MOUNT_ATTR_NODEV`/`NOSYMFOLLOW` have zero enforcement consumers | syscalls+vfs | `fs/namespace.c do_fsmount`; `fs/namei.c pick_link` | `432_fsmount.rs:29`; `vfs/src/mount/mnt_flags.rs` | S |
...
| 437 | `openat2` | all five `RESOLVE_*` are genuinely enforced in the walker — **but bypassed on the create path**: the `O_CREAT` branch calls `resolve_parent_at`, which drops `extra`, so `openat2(dirfd, "../../etc/passwd", O_CREAT, {RESOLVE_BENEATH})` creates outside the scope | syscalls | `fs/namei.c:4320,4843` | `257_openat.rs:265-273`; `pathresolve/at.rs:98-100` | S |
