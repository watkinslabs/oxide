...
Next gate: PTMX/TTY open routes by resolved char-device identity; netns ioctl uses detached dentries; `open_dentry(path,...)`, public `devfs::lookup/tree`, and `vfs::mount::{resolve_mount,is_readonly_path,lookup}` are gone; cgroup/inotify notify by inode identity; AF_UNIX pathname sockets now bind/connect/sendto/unlink by resolved socket inode key `(fsid,ino)` instead of registry string with canonicalize fallback, while abstract sockets remain netns name-keyed; datagram binds also unbind on close by stored identity; devfs hot-unplug no longer resolves rendered `/dev/<name>` through global VFS and instead returns removed kernfs inode identities then prunes dcache aliases by inode; sysfs dynamic invalidation now drops cached dentries by walking from sysfs's own mounted superblock root, not by global `/sys/...` re-resolution; cgroupfs no longer hides a global `/sys/fs/cgroup` lookup in its mount helper, and boot passes the already-resolved mountpoint dentry into `cgroup::mount_at`. Evidence: `cargo test -p net --lib -- --nocapture` passed 252/252 including inode-key/symlink-key tests; `cargo test -p devfs --lib -- --nocapture` passed 14/14 including hot-unplug removed-inode identity; `cargo test -p kernfs --lib -- --nocapture` passed 10/10; `cargo test -p sysfs --features modules/hosted --lib -- --nocapture` passed 43/43 including root dcache invalidation; `cargo test -p cgroup --lib -- --nocapture` passed 24/24; `cargo check -p sysfs --features modules/hosted` passed; `cargo check -p kmain` passed for x86_64+aarch64 kernel targets; prior path cleanup evidence remains fs 88/88, vfs lib 96/96, modules misc cdev 1/1, devpts 0/0. `cargo test -p vfs --tests -- --nocapture` still fails pre-existing `executor_msmove_root_then_resolve_binary_and_lib`.
Landlock path-beneath rules now store the opened parent fd as a mounted absolute rule prefix instead of `dentry.name()` basename, so a rule for `/run/udev` covers `/run/udev/data/...` and does not accidentally become unmatched `udev`. Evidence: `cargo test -p security --lib -- --nocapture` passed 36/36 including absolute-prefix and basename-regression tests; `cargo check -p kmain` passed for x86_64+aarch64 kernel targets.
