...
| VERIFIED | B608-uevent-seqnum-sysfs | Add/readable `/sys/kernel/uevent_seqnum` backed by the netlink uevent sequence counter, or document and prove non-requirement in code tests. |
...
| NOT DONE | B611-udev-control-proof | Add focused AF_UNIX/systemd-udevd control socket proof for connect, wake, send, and reply behavior without boot smokes. |
| NOT DONE | B612-inotify-udev-dirs | Prove or fix inotify `IN_CREATE`/`IN_DELETE`/move delivery for `/dev` and `/run/udev` directory mutations. |
| NOT DONE | B613-class-env-audit | Audit and fix class-specific uevent env for input, sound, graphics, misc, and mem devices where Linux udev rules need more than DEVNAME/MAJOR/MINOR. |
