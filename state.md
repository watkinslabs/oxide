...
## BLOCKER #2 FULLY PINNED (UXWRITE + EPADD): dbus-broker accepts polkit but NEVER PROCESSES it
UXWRITE trace (connected-socket writes) for polkit's bus pair ffff8000061a9fa0:
- polkitd wrote end=B n=1 then n=6 (the D-Bus AUTH nul byte + AUTH probe).
- dbus-broker wrote **ZERO** to polkit's pair — while writing **1227 replies (end=A) to OTHER pairs**.
- polkit's pair IS `accepted_by=dbus-broker` (confirmed this boot).
So dbus-broker accepts polkit's connection but **never reads polkit's AUTH / never replies** → polkit's ppoll stuck on empty read queue → no RequestName → 45s timeout.
EPADD trace: dbus-broker adds accepted sockets to its epoll as EPOLLET|EPOLLIN (ev=0x80000015), BUT one EPADD shows `fd=28 ev=0` — a socket added with **NO event interest** (dbus-broker's add-disabled-then-EPOLL_CTL_MOD-to-enable pattern). A fd with events=0 yields `ready = raw & 0 = 0` → never reported; the gen_edge fix can't help (it requires ready!=0). **If polkit's accepted fd is added ev=0 and the MOD to enable EPOLLIN never fires (or is lost), dbus-broker never reads polkit's socket.**
NEXT (last mile of blocker #2): trace dbus-broker's EPOLL_CTL_MOD for polkit's accepted fd — does dbus-broker MOD it to EPOLLIN? If yes but dbus-broker still doesn't read → the MOD/readiness path is broken (does MOD re-arm the edge? scan_once MOD resets et_seen=0 but if the fd is already level-ready, does the next scan report it? a MOD to EPOLLIN on an already-ready fd must fire — verify). If dbus-broker never MODs polkit's fd → why (dbus-broker's per-connection state machine stuck). This is the true last mile; epoll_ctl(MOD) re-arm on an already-ready fd is the prime kernel suspect.

## (earlier) BLOCKER #2 PINNED (POLLFDS with fix): dbus-broker's REPLY doesn't reach polkit's read queue
polkit's stuck ppoll WITH the fix is UNCHANGED from before: `fd=6` (bus socket, ino 534f434b) polled for POLLIN, `rdy=0x4` (POLLOUT only → **read queue EMPTY**), tmo=infinite — waiting forever for a D-Bus reply that never lands. (One sample showed rdy=0x5, so delivery is INTERMITTENT/partial.) So: polkit's connection is accepted (fix #1 worked), but **dbus-broker's replies to polkit do not reliably land in polkit's accepted-socket read queue** → polkit's worker ppoll-stuck → never sends RequestName → 45s timeout.
This is a CONNECTED-socket delivery bug, distinct from the accept edge-loss. dbus-broker (accepted end A) writes reply → should go to the pair's a_to_b queue → polkit (end B) reads a_to_b + its ppoll wakes via end_b_subs. Since rdy stays POLLOUT (a_to_b empty), dbus-broker either (a) isn't WRITING the reply (didn't process polkit's Hello — but the EPOLLET fix should let its read-epoll see polkit's write), or (b) writes to the wrong queue (accept end-A/B wiring), or (c) intermittent (the one rdy=0x5 sample). NEXT: trace dbus-broker's WRITE to polkit's accepted socket (does write_inner push to a_to_b? which pair? does polkit's ppoll wake?) — instrument the connected-socket write/read for the polkit↔dbus-broker pair. THAT delivers the reply → RequestName → greeter.

## (earlier) BLOCKER #2 CHARACTERIZED: polkit still never sends RequestName (post-accept stall)
Re-profiled polkit WITH the fix (debug-syscost, target=polkit): polkit's connection is accepted, but it STILL **never sends RequestName(PolicyKit1)** (`RequestName sent?: False`) and times out (2 restart cycles). It does <800 syscalls total (profiler never hit its dump threshold) → polkit blocks VERY EARLY, before doing much. So the accept fix did NOT change polkit's fundamental stall — polkit is accepted but its post-Hello, pre-RequestName authority-init still hangs (the original ppoll-stuck-on-bus-socket / worker→main eventfd handoff finding). So blocker #2 is polkit's OWN stall, independent of (and downstream of) the now-fixed orphaned-accept.
NEXT for blocker #2: with the accept fixed, re-run the POLLFDS trace on polkit's stuck ppoll — is it STILL blocked on the bus socket (fd=6) with an empty read queue (dbus-broker's reply not arriving), or on the GMainContext eventfd (worker never posting)? If bus-socket-empty: the reply from dbus-broker still isn't delivered — check the connected-socket write→peer-read-queue path (does dbus-broker's write to polkit's accepted socket land + wake polkit's ppoll? the EPOLLET fix covers dbus-broker's READ epoll, but polkit uses ppoll — verify ppoll wakes on the connected socket). If eventfd: the worker→main handoff. This is the last mile.

## (earlier) BUT polkit STILL TIMES OUT — a SECOND blocker remains
