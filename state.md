...
Traced: `po_mnt=140 nr_id=140 root_id=120` — the service rootfs mount (140) is
genuinely SHARED when Linux needs PRIVATE. RULED OUT as the cause: open_tree(CLONE)
(clones are `CloneType::Private`, clone_tree.rs) and detached move_mount (429:77
`commit_tree_hashonly`, doesn't share). The SHARED comes from mount 140's
CREATION: `mount(MS_BIND|MS_REC, source=/proc/self/fd/4, target=/run/systemd/
mount-rootfs)` — a recursive bind grafted under `/run`, which is STILL SHARED, so
the bind inherits shared (Linux `do_add_mount`: dest shared ⇒ new mount joins its
peer group). systemd's `make-rslave /` (MS_REC|MS_SLAVE) SHOULD have made `/run`
slave first — but 140 is still shared, so **the recursive make-slave isn't
reaching `/run`.** **First task: find why.** Suspects: (a) the make-slave runs in
the service's NEW mount ns (CLONE_NEWNS) and our ns isolation / mount-tree copy is
off, so `subtree_ids(root)` in that ns doesn't include `/run`; (b) `subtree_ids`
returns only direct children, not the transitive subtree; (c) the make-rslave
target resolves to the wrong ns's root. Verify: in the failing service's ns,
trace `set_propagation_recursive` — does it enumerate `/run`'s mount and set it
slave? If `/run` is slave at bind time, 140 is private and pivot_root succeeds →
sysinit completes → getty/login. Check CLONE_NEWNS mount-ns copy +
`subtree_ids` (vfs/src/mount/model.rs) + `mount(MS_BIND)` graft propagation
(does our bind honor the dest's slave/private vs share).
