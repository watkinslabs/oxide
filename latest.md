...
Next gate: PTMX/TTY open now routes by resolved char-device identity, netns ioctl uses a detached dentry, and `open_dentry(path,...)` is deleted; cgroup.events inotify notification now passes synthesized inode identity to `fire_modify`, dirent fsnotify hooks take parent inode objects, and inotify keys marks by stable `(fsid, ino)` instead of `Arc` address; verified by `cargo test -p fs --lib -- --nocapture` (88/88), `cargo test -p cgroup --lib -- --nocapture` (24/24), and `cargo check -p kmain` for x86_64+aarch64 kernel targets; `cargo test -p vfs --tests -- --nocapture` still fails the pre-existing `executor_msmove_root_then_resolve_binary_and_lib` case.
