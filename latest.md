...
Next gate: PTMX/TTY open now routes by resolved char-device identity, netns ioctl uses a detached dentry, `open_dentry(path,...)` is deleted, cgroup/inotify notify by inode identity, `vfs::mount::{resolve_mount,is_readonly_path,lookup}` wrappers are gone, and devfs no longer exports `lookup`/`tree` as a public path resolver; remaining absolute text resolution funnels through canonical `resolve_abs` only, with mount ownership through `walk_to_mount`/`mnt_id`; verified by `cargo test -p fs --lib -- --nocapture` (88/88), `cargo test -p cgroup --lib -- --nocapture` (24/24), `cargo test -p vfs --lib -- --nocapture` (96/96), `cargo test -p vfs --test deep_resolve -- --nocapture` (3/3), `cargo test -p vfs --test mount_check_mnt -- --nocapture` (2/2), `cargo test -p vfs --test file_mnt_readonly -- --nocapture` (4/4), `cargo test -p devfs --lib -- --nocapture` (13/13), `cargo test -p modules --lib misc_register_claims_minor_and_cdev -- --nocapture` (1/1), `cargo test -p devpts --lib -- --nocapture` (0/0), and `cargo check -p kmain` for x86_64+aarch64 kernel targets; `cargo test -p vfs --tests -- --nocapture` still fails the pre-existing `executor_msmove_root_then_resolve_binary_and_lib` case.
