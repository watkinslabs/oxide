...
static void sha512(const uint8_t* data, long len, uint8_t out[64]) {
    ...
    // pad: 1, then zeros, then 16-byte BE bit length
    uint8_t buf[256];
    long pad_len = 128 - ((len + 17) % 128);
    if (pad_len == 128) pad_len = 0;
    long total = len + 1 + pad_len + 16;
    if (total > (long)sizeof(buf)) total = sizeof(buf); // login passwords are <128 bytes; cap
    for (long i = 0; i < len; i++) buf[i] = data[i];
    buf[len] = 0x80;
    for (long i = len + 1; i < total - 16; i++) buf[i] = 0;
    uint64_t bitlen = (uint64_t)len * 8;
    for (int i = 0; i < 8; i++) buf[total - 16 + i] = 0;
    for (int i = 0; i < 8; i++) buf[total - 8 + i] = (uint8_t)(bitlen >> (56 - 8*i));
    for (long off = 0; off < total; off += 128) {
        uint64_t w[80];
        for (int i = 0; i < 16; i++) {
            w[i] = 0;
            for (int j = 0; j < 8; j++)
                w[i] = (w[i] << 8) | buf[off + i*8 + j];
        }
        for (int i = 16; i < 80; i++) {
...
// crypt-base64 alphabet (./0-9A-Za-z); encode 64-byte digest into ASCII.
static const char ALPH[64] =
    "./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";

static long crypt_b64(const uint8_t* digest, char* out) {
    long o = 0;
    long i = 0;
    for (; i + 3 <= 64; i += 3) {
    ...
    if (i < 64) {
        uint32_t v = 0;
        for (long k = 0; i + k < 64; k++) v |= ((uint32_t)digest[i + k]) << (8 * k);
        out[o++] = ALPH[v & 63];
        out[o++] = ALPH[(v >> 6) & 63];
    }
    return o;
...
// Mirror of crypt::sha512::sha512crypt: sha512(salt|password|salt)
// then crypt-base64. Returns length written to out.
static long sha512crypt_simple(const char* password, const char* salt, char* out) {
    uint8_t buf[256], digest[64];
    long sl = mlen(salt), pl = mlen(password);
    if (sl + pl + sl > (long)sizeof(buf)) return 0;
    long o = 0;
    for (long i = 0; i < sl; i++) buf[o++] = (uint8_t)salt[i];
    for (long i = 0; i < pl; i++) buf[o++] = (uint8_t)password[i];
    for (long i = 0; i < sl; i++) buf[o++] = (uint8_t)salt[i];
    sha512(buf, o, digest);
    return crypt_b64(digest, out);
}
