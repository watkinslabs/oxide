...
fn decode_response(frame: &[u8]) -> Result<Response, Error> {
    ...
    let response = match code {
        ...
        3 if frame.len() == 2 => Response::Failure(match frame[1] { 1 => Error::InvalidPath, 2 => Error::MissingKey, 3 => Error::MissingValue, 4 => Error::InvalidFile, 5 => Error::Io("remote I/O failure".into()), 6 => Error::Deleted, _ => return Err(Error::InvalidFile) }),
