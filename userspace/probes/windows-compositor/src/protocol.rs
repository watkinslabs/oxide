...
pub enum BridgeCommand {
    ...
    Reparent { hwnd: u32, parent: u32, rect: Rect },
...
fn decode_command(opcode: Opcode, hwnd: u64, p: &[u8]) -> Result<BridgeCommand, TransportError> {
    ...
    Ok(match opcode {
        ...
        Opcode::Reparent => BridgeCommand::Reparent { hwnd:id, parent:u32::try_from(wire::u64_at(p,0).map_err(|_| TransportError::Unsupported)?).map_err(|_| TransportError::Unsupported)?, rect:window_rect_from_wire(&p[8..])? },
