...
fn main() {
    match query() {
        Ok(capability) => {
            ...
            let description = SurfaceDescription { session: 3, window: 41, window_owner: 7, device: 11, queue: 13, resource: 17, device_ready: true, surface_alive: true, present_supported: true, width: capability.max_width.min(1280), height: capability.max_height.min(720), format };
            let mut session = PresentSession::create(capability, description).expect("native capability cannot admit its own WSI contract");
            session.acquire(SurfaceHandoff { session: 3, window: 41, window_owner: 7, device: 11, queue: 13, resource: 17 }).expect("WSI acquire contract failed");
