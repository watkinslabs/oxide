 ...
 * WITHOUT CAP_NET_RAW. The manifest pins this probe to uid=unprivileged on both
 * sides, so the host oracle and the guest run under the same privilege.
 *
 * Pinned here: which types the level admits and their exact widths
 * (IPV6_PKTINFO takes at least an in6_pktinfo, IPV6_HOPLIMIT/IPV6_TCLASS take
 * exactly an int and bound it to -1..255, IPV6_DONTFRAG takes exactly an int
 * and bounds it to 0..1), the extension-header types refused for the missing
 * capability, a routing header whose type is rejected before any capability is
 * consulted, an unknown type at this level, a level the socket does not own
 * (stepped over), and an unknown SOL_SOCKET type.
 *
 * `t_cmsgv6_priv` is the same case list under uid=root. */
#define _GNU_SOURCE
#include <errno.h>
