...
#define BATCH_CAP 1024U
#define BATCH_OVER (BATCH_CAP + 1U)
...
static void send_cap(void) {
    int sv[2];
    long page = sysconf(_SC_PAGESIZE);
    size_t used = BATCH_CAP * sizeof(struct mmsghdr);
    ...
    errno = 0;
    int got = sendmmsg(sv[0], msgs, BATCH_OVER, 0);
...
static void recv_over_cap(void) {
    ...
    struct iovec out_iov = {sent, 1}, in_iov = {received, sizeof received};
    struct mmsghdr *in = calloc(BATCH_OVER, sizeof(*in));
    ...
    in[0].msg_hdr.msg_iov = &in_iov; in[0].msg_hdr.msg_iovlen = 1;
    errno = 0;
    int got = recvmmsg(sv[1], in, BATCH_OVER, MSG_DONTWAIT, NULL);
    printf("recv_cap rc=%d errno=%d len=%u data=%d\n", got, errno, in[0].msg_len, received[0] == 'z');
    free(in); close(sv[0]); close(sv[1]);
}

int main(void) {
    base_batch(); timeout_before_fd(); send_cap(); prefix_faults(); recv_over_cap();
    return 0;
}
