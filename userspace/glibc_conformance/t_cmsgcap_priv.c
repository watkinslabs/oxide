 ...
 * answered WITH CAP_NET_RAW/CAP_NET_ADMIN. The manifest pins this probe to
 * uid=root on both sides, so the host oracle runs privileged too and the
 * comparison is byte-exact.
 *
 * Same case list as `t_cmsgcap`; the pair is the refused/admitted differential
 * for one surface. Privilege moves SO_MARK from the capability refusal to its
 * width answer and admits SO_PRIORITY above the interactive band, while the
 * width-first and socket-state answers are unchanged.
 *
 * Keep the case list identical to `t_cmsgcap`: the value of the pair is that
