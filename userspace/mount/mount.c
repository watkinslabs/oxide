// /bin/mount — POSIX mount(8) reader. With no args, prints the
// contents of /proc/mounts (one mount per line). Mount/unmount
// (with args) is not supported: v1 doesn't expose mount(2) yet.
...
void
_start(void) {
    long fd = sc4(SYS_openat, AT_FDCWD, (long)"/proc/mounts", O_RDONLY, 0);
