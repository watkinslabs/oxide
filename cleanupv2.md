...
|------|---------|------|--------|--------|-----|
| 1.1 | PAM `PAM_SESSION_ERR` — `user@979.service` step PAM | 1 | **ADVANCED (was mislabeled "BLOCKED, no causative syscall").** Two real causes found via boot capture: (a) **keyring** — `keyctl(LINK)` didn't resolve the child special-id, so pam_keyinit's `keyctl(LINK, USER_KEYRING, SESSION_KEYRING)` → ENOKEY ("Failed to link user keyring into session keyring" ×15). **FIXED + boot-verified 15→0** (B529 #2587), which advanced the boot so gnome-shell + the greeter now start. (b) **Login blocker = INTERMITTENT capability race.** `user@979.service` PAM session calls `mount(2)` → **EPERM** at `165_mount.rs:176` (CAP_SYS_ADMIN gate). Proven intermittent across 3 boots and **directly correlated with login**: boot with EPERM firing (×3) → NO greeter; boot with EPERM absent (×0) → **GNOME greeter appears** (gnome-shell starts, reaches graphical). So this ONE intermittent EPERM is the desktop gate. The `[mnt-cap]` trace (B531 #2590) shows PID1 mounts have `sysadmin=1`; the failing mount intermittently sees the caller WITHOUT CAP_SYS_ADMIN → a **cap race** during fork/exec/PAM-setup (caps sometimes wrong), same family as the documented intermittent-wedge/refcount bugs. NOT deterministically reproducible by single boot (Lesson 3). Next: a hosted/deterministic repro of the fork→exec→setresXid→has_cap sequence, or a runtime detector that traps when a root task's CAP_SYS_ADMIN vanishes. Do NOT hack-remove the mount check. **path_id ENOENT + hwdb no-match are present every boot regardless → non-fatal NOISE, not blockers (greeter appears despite them).** | keyring: B529 #2587 (done); cap-race: open (intermittent) | #2587 |
