...
- **Separate real bug found in the hunt:** `keyctl(KEYCTL_JOIN_SESSION_KEYRING)` returns a CONSTANT global serial (`1`) every call (fake single keyring) instead of a fresh per-session keyring — a genuine Linux-divergence worth fixing on its own merits (pam_keyinit is `session optional` so it isn't the 1.1 cause, but it's a real compat gap).
