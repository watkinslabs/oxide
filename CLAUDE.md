...
11. **DECODE the corrupt `[KALLOC]` free-list header BYTES before theorizing — the ~90%-boot heap corruptor is a VIRTIO USED-RING UAF, not a data-buffer UAF.** The recurring incidental values (`node_size`/`bad_next` = `1<<32`, `2<<32`, `3<<32`, `0x7fffffff<<32`, `0xaaaaaa`, …) are NOT random: laid out little-endian, a corrupt 16-byte header like `bad_next=0x300000000 node_size=0x200000000` decodes to `flags=0, idx=0, ring[0]={id:3,len:0}, ring[1]={id:2,len:0}` — **exactly a `virtio` used ring** (`struct vring_used { u16 flags; u16 idx; vring_used_elem ring[]; }`, each elem `{u32 id; u32 len}`). The device DMA-writes completed descriptor IDs (small: 2,3,…) into a used-ring frame that was **freed and recycled into the kalloc heap** (freed raw frame → buddy → `kalloc_grow` → KHEAP; the device's late used-ring write lands on a free-list header). This is why B1339/B1340/B1341 (data-buffer quiescence) never moved the rate — the victim is the **virtqueue RING frame**, not a data buffer. Raw ring/DMA frames have **refcount 0** (fan-out Finding 2), so the B1340 in-use guard cannot protect them. The only ring-frame free paths are `virtio_transport/msix.rs`: (a) `reset_failed_probe`→`reset_device` (SAFE — `reset_device` spins on status→0, confirmed quiescence) and (b) **`release_transport_record` (published-device unpublish) frees `vring_frames` relying on a bare SAFETY-comment assumption "Child remove resets/quiesces the device before unpublishing" — but it does NOT call `reset_device` itself, and its callers (`unpublish_transport_mmio`/`unpublish_transport`) don't visibly reset.** That assumed-but-unverified quiescence is the prime suspect; the alternative is a buddy double-hand of a live refcount-0 ring frame to `kalloc_grow`. **Fix direction:** make `release_transport_record` defensively `virtio::reset_device(cfg_va)` BEFORE `mappings.unmap_all()`/`free_one_frame` (thread `cfg_va` into `TransportRecord`), mirroring the failed-probe path — never free a device-DMA'd frame on a caller-assumed quiesce. **First confirm** which device unpublishes mid-boot (trace `release_transport_record`) before/with the fix. `[ZRAM-SYSFS] disksize=` clustering is the detection point (heaviest alloc burst), not the corruption point.
