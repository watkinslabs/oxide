...
WINE_PROFILE="${OXIDE_WINE_PROFILE:-release}"
case "$WINE_PROFILE" in
    release) PROFILE_CFLAGS="-O2 -g" ;;
    debug) PROFILE_CFLAGS="-O0 -g -DOXIDE_WINE_DIAGNOSTICS=1" ;;
    *) echo "build-wine-runtime: OXIDE_WINE_PROFILE must be release or debug" >&2; exit 1 ;;
esac
...
OUT="${OXIDE_WINE_OUT:-$REPO_ROOT/target/artifacts/wine/x86_64-$WINE_PROFILE}"
...
SRC="$WORK/wine-$WINE_VERSION-$WINE_PROFILE"
BUILD="$WORK/build-$WINE_VERSION-$WINE_PROFILE"
HEADERS="$WORK/mingw64-headers-$WINE_PROFILE"
...
if [ ! -f "$BUILD/Makefile" ]; then
    mkdir -p "$BUILD"
    ( cd "$BUILD" && CFLAGS="$PROFILE_CFLAGS" CXXFLAGS="$PROFILE_CFLAGS" x86_64_CFLAGS="-isystem $INCLUDE $PROFILE_CFLAGS" x86_64_CXXFLAGS="-isystem $INCLUDE $PROFILE_CFLAGS" \
        "$SRC/configure" --enable-win64 --with-mingw=clang --disable-tests --disable-win16 CC=gcc )
fi

make -C "$BUILD" -j"$JOBS"

DESTDIR="$WORK/dest-$WINE_VERSION-$WINE_PROFILE"
rm -rf "$DESTDIR"
...
printf '%s\n' "$WINE_PROFILE" > "$OUT/wine-profile"
printf '%s\n' "${INPUT_KEY%% *}" > "$OUT/wine-build-id"
...
echo "build-wine-runtime: wine=$WINE_VERSION profile=$WINE_PROFILE modules=$modules unixlibs=$unixlibs out=$OUT"
