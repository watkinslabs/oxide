...
mkdir -p "$device_root" "$driver_root/nvme" "$driver_root/e1000" "$driver_root/igc" "$driver_root/r8169" "$driver_root/other"
...
device 0000:00:09.0 0x1d6a 0x04c0 0x02000000
device 0000:00:0a.0 0x8086 0x15f3 0x02000000 igc
device 0000:00:0b.0 0x8086 0x125c 0x02000000
device 0000:00:0c.0 0x8086 0x15f2 0x02000000 other
...
printf '%s\n' "$out" | grep -Fx 'OXIDE_HARDWARE_AUDIT|v1|driver-assessment|NEEDS-DRIVER|bdf=0000:00:09.0|driver=atlantic|reason=linux-atlantic-aqc113-family'
...
printf '%s\n' "$out" | grep -Fx 'OXIDE_HARDWARE_AUDIT|v1|driver-assessment|WRONG-DRIVER|bdf=0000:00:0c.0|expected=igc|driver=other'
