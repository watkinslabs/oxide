...
LOG="$(mktemp /tmp/oxide-uart-rebind-${ARCH}-XXXXXX.log)"
PIDFILE="$(mktemp /tmp/oxide-uart-rebind-${ARCH}-XXXXXX.pid)"
UART="$(mktemp -u /tmp/oxide-uart-rebind-${ARCH}-uart-XXXXXX.sock)"
UART_BRIDGE_PID=""
...
echo "uart-rebind-smoke: arch=$ARCH timeout=${TIMEOUT}s uart=$UART log=$LOG"
OXIDE_DRIVER_PATH_SMOKE=1 OXIDE_UART_REBIND_SMOKE=1 OXIDE_QEMU_HEADLESS=1 OXIDE_QEMU_UART_SOCK="$UART" OXIDE_QEMU_KVM="${OXIDE_QEMU_KVM:-1}" \
...
[ -S "$UART" ] || { echo "uart-rebind-smoke: FAIL - UART socket absent" >&2; exit 1; }
python3 - "$UART" "$LOG" <<'PY' &
import socket, sys
...
while [ "$(date +%s)" -lt "$deadline" ]; do
    pid="$(cat "$PIDFILE" 2>/dev/null || true)"
    if [ -n "$pid" ] && ! kill -0 "$pid" 2>/dev/null; then
        echo "uart-rebind-smoke: FAIL - qemu exited before verdict" >&2
        tail -n 80 "$LOG" >&2
        exit 1
    fi
...
echo "uart-rebind-smoke: FAIL - timeout waiting for verdict" >&2
tail -n 80 "$LOG" >&2
exit 1
