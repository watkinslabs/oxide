#!/usr/bin/env bash
# Run the SOL_NETLINK / SOL_SOCKET-on-netlink-fd probe on Linux (the ORACLE,
# as root — the privilege-ladder cases need CAP_NET_ADMIN/CAP_NET_RAW/
# CAP_NET_BROADCAST) and compare it byte-for-byte with one Oxide boot.
set -euo pipefail
...
PROBE_WORKSPACE="$ROOT/userspace/probes"
PROBE="$PROBE_WORKSPACE/target/x86_64-unknown-linux-gnu/release/sockopt_diff"
RUN_ROOT="${SOCKOPT_DIFF_LOG_DIR:-$ROOT/target/smoke/sockopt-diff}"
mkdir -p "$RUN_ROOT"
...
PIDFILE="$(mktemp "/tmp/oxide-sockopt-diff-${ARCH}-XXXXXX.pid")"
UART="$(mktemp -u "/tmp/oxide-sockopt-diff-${ARCH}-XXXXXX.sock")"
UART_BRIDGE_PID=""
...
fail() {
    echo "sockopt-diff-smoke: FAIL - $*" >&2
    echo "sockopt-diff-smoke: logs=$RUN_DIR" >&2
    return 1
}
...
echo "sockopt-diff-smoke: arch=$ARCH timeout=${TIMEOUT}s logs=$RUN_DIR"
...
if ! ( cd "$PROBE_WORKSPACE" && cargo build --release --target x86_64-unknown-linux-gnu -p sockopt_diff ) >"$HOST_BUILD_LOG" 2>&1; then
...
OXIDE_SOCKOPT_DIFF_SMOKE=1 OXIDE_QEMU_HEADLESS=1 \
    OXIDE_QEMU_UART_SOCK="$UART" OXIDE_QEMU_KVM="${OXIDE_QEMU_KVM:-1}" \
    setsid bash -c "exec make '$MT' > '$BOOT_LOG' 2>&1 < /dev/null" &
echo $! > "$PIDFILE"

# Bounded by the run's own timeout, not a fixed 60s: a cold-tree `make
# qemu-arm` spends minutes building before QEMU exists.
...
while [ "$(date +%s)" -lt "$deadline" ]; do
    ...
    if grep -aiqE 'sockopt-diff-smoke\.service.*(fail|error)|Failed to (start|run).*sockopt.diff' "$UART_LOG" 2>/dev/null; then
        grep -aE 'sockopt-diff-smoke|meta\|complete' "$UART_LOG" >&2 || true
...
echo "sockopt-diff-smoke: PASS - $ARCH exact Linux differential"
echo "sockopt-diff-smoke: logs=$RUN_DIR"
