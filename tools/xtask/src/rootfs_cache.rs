...
fn rootfs_mode() -> String {
    ...
    push_env_mode(&mut mode, "uart-rebind", "OXIDE_UART_REBIND_SMOKE");
    push_env_mode(&mut mode, "ps2-rebind", "OXIDE_PS2_REBIND_SMOKE");
    ...
    push_env_mode(&mut mode, "sysblock", "OXIDE_SYSBLOCK_SMOKE");
    push_env_mode(&mut mode, "sysbus-bind", "OXIDE_SYSBUS_BIND_SMOKE");
...
pub(crate) fn input_hash(repo: &Path, arch: &str) -> String {
    ...
    h.write(b"\0virtio-blk-multidev-smoke=");
    if std::env::var_os("OXIDE_VIRTIO_BLK_MULTIDEV_SMOKE").is_some() { h.write(b"1"); }
    h.write(b"\0sysblock-smoke=");
    if std::env::var_os("OXIDE_SYSBLOCK_SMOKE").is_some() { h.write(b"1"); }
    h.write(b"\0sysbus-bind-smoke=");
    if std::env::var_os("OXIDE_SYSBUS_BIND_SMOKE").is_some() { h.write(b"1"); }
    ...
    h.write(b"\0uart-rebind-smoke=");
    if std::env::var_os("OXIDE_UART_REBIND_SMOKE").is_some() { h.write(b"1"); }
    h.write(b"\0ps2-rebind-smoke=");
    if std::env::var_os("OXIDE_PS2_REBIND_SMOKE").is_some() { h.write(b"1"); }
