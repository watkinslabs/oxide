...
mod rootfs_glibc;
mod rootfs_disks;
use crate::cmds::{cmd_doc_check, cmd_kernel, cmd_spec_lint, cmd_test, parse_arg, run};
...
fn main() -> ExitCode {
    ...
    let res = match cmd {
        ...
        "test"      => cmd_test(rest),
        "conformance" => conformance::cmd_conformance(rest),
...
fn usage() -> ExitCode {
    eprintln!("usage: xtask <kernel|image|grub|rootfs|artifacts|test|conformance|gc|path|spec-lint|doc-check|stats> [args]");
    ExitCode::from(2)
}
