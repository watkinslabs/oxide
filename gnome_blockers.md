...
| G6 | gdm starts + logind creates seat0, but **no greeter session** (no gnome-shell / gdm-session-worker / `New session cN`) | **OPEN — current frontier** | — | With G1+G5 fixed and the full package set (`@base-graphical`/workstation env — `@gnome-desktop` alone lacks the Wayland/session plumbing), boot reaches graphical.target, gdm.service + logind start, `New seat seat0` — but gdm launches no greeter. Most likely **seat0 not marked graphical** (the master-of-seat / `CanGraphical` gate — udev must `TAG+="master-of-seat"` on card0 via 71-seat.rules so logind sets CAN_GRAPHICAL=1; docs/60 §11 "FAIL today"), so gdm's local-display-factory waits forever. Needs a debug-session + card0/udev-seat capture. NOTE: a NetworkManager `ENOPROTOOPT` ("Failed to process RTNL link / update monitor information: Protocol not available") is a separate netlink-sockopt gap, not the display blocker. **BLOCKED on clean iteration:** a concurrent agent is churning the shared oxide-images image + stealing /dev/kvm; stable boots need it stopped. |
