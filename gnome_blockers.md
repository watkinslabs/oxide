...
- **G9 (current greeter frontier): gdm.service starts but forks NOTHING** and emits no debug output — stuck in early gdm-daemon startup before the display factory. Needs a per-process syscall trace / disasm of gdm startup.
- **Boot state now:** Startup finished ✓, detect-virt=kvm ✓, seat0 graphical ✓, logind+accounts-daemon+audit ✓. Everything up to the gdm greeter-display works; gdm itself doesn't advance.
