...
Current marker: B527-virtio-child-profile-declarations-proof audits row 196,
Child drivers export transport profile declarations with feature masks, queue
requirements, IRQ callback policy.

## B527 Current

| Branch | Status | Evidence |
|---|---|---|
| B527-virtio-child-profile-declarations-proof | IN AUDIT | Fresh `main` at merge commit `243362b7` after B526 PR #2623 merge and branch cleanup. Post-merge fresh-main smokes passed: x86_64 reached `oxide login:` in 30s; aarch64 reached `oxide login:` in 36s. `metadata/index.md` advanced B 527 -> 528. Source audit starting for child-owned `VirtioTransportProfile` declarations: feature masks, queue requirements, and IRQ callback policy must come from child drivers, not pci-boot transport policy. |
