...
fn a_standing_rseq_registration_never_keeps_the_loop_spinning() {
    ...
    let w = work_flags(false, 0, 0, false, false, true, false);
...
fn notify_resume_is_likewise_never_a_reason_to_loop_forever() {
    ...
    let w = work_flags(false, 0, 0, false, true, false, false);
...
fn kernel_mode_return_never_runs_the_loop() {
    ...
    let w = work_flags(true, bit(Signum::Sigterm), 0, false, true, true, false);
...
fn no_work_means_no_loop_even_from_user() {
    let w = work_flags(false, 0, 0, false, false, false, false);
    assert_eq!(w, 0);
    assert!(!has_work(w));
    assert!(!runs_on_return(true, w));
}
...
#[test]
fn a_freezer_request_enters_only_the_user_return_loop() {
    let w = work_flags(false, 0, 0, false, false, false, true);
    assert_eq!(w, work::FREEZE);
    assert!(runs_on_return(true, w));
    assert!(!runs_on_return(false, w),
        "a kernel-mode IRQ return is not a lock-free freezer checkpoint");
}

#[test]
fn each_work_item_sets_exactly_its_own_bit() {
    assert_eq!(work_flags(true, 0, 0, false, false, false, false), work::NEED_RESCHED);
    assert_eq!(work_flags(false, bit(Signum::Sigusr1), 0, false, false, false, false), work::SIGPENDING);
    assert_eq!(work_flags(false, 0, 0, true, false, false, false), work::NOTIFY_SIGNAL);
    assert_eq!(work_flags(false, 0, 0, false, true, false, false), work::NOTIFY_RESUME);
    assert_eq!(work_flags(false, 0, 0, false, false, true, false), work::RSEQ);
    assert_eq!(work_flags(false, 0, 0, false, false, false, true), work::FREEZE);
    assert_eq!(work_flags(true, bit(Signum::Sigusr1), 0, true, true, true, true), work::MASK);
}
...
fn blocked_signal_is_not_work() {
    ...
    assert_eq!(work_flags(false, s, s, false, false, false, false), 0);
...
fn sigkill_is_work_even_when_blocked() {
    ...
    assert_eq!(work_flags(false, k, !0, false, false, false, false), work::SIGPENDING);
}
...
fn alarm_posted_by_the_tick_is_work_on_the_next_return() {
    ...
    let w = work_flags(false, bit(Signum::Sigalrm), 0, false, false, false, false);
    assert!(runs_on_return(true, w));
}
