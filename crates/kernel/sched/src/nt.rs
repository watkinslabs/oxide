//! Native Windows fixed-priority dispatcher queue.
//!
//! NT levels are deliberately not POSIX RT priorities.  The dispatcher has
//! 32 FIFO ready buckets, selects the highest non-empty level, and rotates a
//! level only when its quantum is relinquished.
...
pub(crate) const NT_LEVELS: usize = 32;
...
impl Drop for NtRunqueue {
    fn drop(&mut self) { while self.pick_highest().is_some() {} }
}
...
mod tests {
    ...
    fn highest_level_preempts_lower_levels() {
        ...
        let low = task(1, 4);
        let high = task(2, 27);
        assert!(q.enqueue(low, false));
        assert!(q.enqueue(high, false));
    ...
    fn equal_level_is_fifo_until_explicit_rotation() {
        let mut q = NtRunqueue::new();
        let first = task(1, 12);
        let second = task(2, 12);
        assert!(q.enqueue(Arc::clone(&first), false));
        assert!(q.enqueue(Arc::clone(&second), false));
        assert_eq!(q.pick_highest().unwrap().tid, 1);
