...
pub(super) fn save_outgoing(task: &Task) {
    ...
    task.security.fault_frame.store(frame, Ordering::Release);
    task.security.fault_rsp.store(rsp, Ordering::Release);
    task.security.fault_rip.store(rip, Ordering::Release);
}
...
pub(super) fn restore_incoming(task: &Task) {
    hal_x86_64::restore_current_fault_frame(
        task.security.fault_frame.load(Ordering::Acquire),
        task.security.fault_rsp.load(Ordering::Acquire),
        task.security.fault_rip.load(Ordering::Acquire),
    );
}
