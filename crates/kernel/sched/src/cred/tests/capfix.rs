...
fn dropping_every_root_id_clears_permitted_effective_and_ambient() {
    let task = privileged();
    task.security.creds.cap_ambient.store(Creds::CAP_FULL, Ordering::Release);
    assert_eq!(setuid_on(&task, 1000), 0);
    assert_eq!(task.security.creds.cap_permitted.load(Ordering::Acquire), 0);
    assert_eq!(task.security.creds.cap_effective.load(Ordering::Acquire), 0);
    assert_eq!(task.security.creds.cap_ambient.load(Ordering::Acquire), 0);
}

#[test]
fn secure_keep_caps_preserves_permitted_but_still_clears_ambient() {
    let task = privileged();
    task.security.creds.securebits.store(SECBIT_KEEP_CAPS, Ordering::Release);
    task.security.creds.cap_ambient.store(Creds::CAP_FULL, Ordering::Release);
    assert_eq!(setuid_on(&task, 1000), 0);
    assert_eq!(task.security.creds.cap_permitted.load(Ordering::Acquire), Creds::CAP_FULL);
    assert_eq!(task.security.creds.cap_effective.load(Ordering::Acquire), 0,
...
fn no_setuid_fixup_securebit_suppresses_the_whole_juggle() {
    let task = privileged();
    task.security.creds.securebits.store(SECBIT_NO_SETUID_FIXUP, Ordering::Release);
    assert_eq!(setuid_on(&task, 1000), 0);
    assert_eq!(task.security.creds.cap_permitted.load(Ordering::Acquire), Creds::CAP_FULL);
...
fn a_privilege_drop_downgrades_dumpability_and_clears_the_parent_death_signal() {
    let task = privileged();
    task.security.pdeathsig.store(9, Ordering::Release);
    task.security.dumpable.store(SUID_DUMP_USER, Ordering::Release);
    ...
    assert_eq!(setuid_on(&task, 1000), 0);
    assert_eq!(task.security.pdeathsig.load(Ordering::Acquire), 0);
}
...
fn an_identity_preserving_call_leaves_the_parent_death_signal_alone() {
    ...
    assert_eq!(task.security.pdeathsig.load(Ordering::Acquire), 9);
}
