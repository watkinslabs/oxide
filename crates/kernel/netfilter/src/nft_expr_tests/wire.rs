// Wire-format parsing and the register/verdict behaviour of the expressions
// that predate the full set. Kept as one file because each test builds its
// own attribute blob end to end.
...
use alloc::vec::Vec;
    ...
    pub(crate) fn nla(buf: &mut Vec<u8>, ty: u16, payload: &[u8]) {
    ...
    pub(crate) fn nla_u32_be(buf: &mut Vec<u8>, ty: u16, v: u32) {
        nla(buf, ty, &v.to_be_bytes());
    }

    pub(crate) fn nla_str(buf: &mut Vec<u8>, ty: u16, s: &str) {
        let mut p = s.as_bytes().to_vec();
        p.push(0);
        nla(buf, ty, &p);
    }

    pub(crate) fn nla_nested(buf: &mut Vec<u8>, ty: u16, inner: &[u8]) {
        nla(buf, ty | 0x8000, inner);
    }

    pub(crate) fn build_immediate_drop() -> Vec<u8> {
    ...
    pub(crate) fn ipv4_pkt_with_src(src: [u8; 4]) -> Vec<u8> {
    ...
    fn counter_increments_on_reach() {
        ...
        let counter_data = Vec::new(); // counter expr has no body in v1
        let mut c_expr = Vec::new();
