...
pub struct VtdPageTable { hhdm_offset: u64, coherent: bool, address_width: u8, root_pa: u64, pages: Vec<u64> }
impl VtdPageTable {
    ...
    pub fn new(hhdm_offset: u64, coherent: bool, address_width: u8) -> Option<Self> {
        ...
        Some(Self { hhdm_offset, coherent, address_width, root_pa, pages: alloc::vec![root_pa] })
    }
    ...
    pub fn map(&mut self, iova: u64, pa: u64, len: u64) -> bool {
        ...
        while current_iova != end {
            let bytes = largest_page_size(current_iova, current_pa, end - current_iova, self.levels());
...
const fn largest_page_size(iova: u64, pa: u64, remaining: u64, levels: usize) -> u64 {
    if levels >= 3 && iova & (HUGE_PAGE_BYTES - 1) == 0 && pa & (HUGE_PAGE_BYTES - 1) == 0 && remaining >= HUGE_PAGE_BYTES { return HUGE_PAGE_BYTES; }
...
#[cfg(test)] mod tests {
    ...
    #[test] fn selects_largest_aligned_vtd_identity_leaves() {
        assert_eq!(largest_page_size(0, 0, HUGE_PAGE_BYTES, 4), HUGE_PAGE_BYTES);
        assert_eq!(largest_page_size(LARGE_PAGE_BYTES, LARGE_PAGE_BYTES, LARGE_PAGE_BYTES, 4), LARGE_PAGE_BYTES);
        assert_eq!(largest_page_size(PAGE_BYTES, PAGE_BYTES, LARGE_PAGE_BYTES, 4), PAGE_BYTES);
        assert_eq!(largest_page_size(0, 0, HUGE_PAGE_BYTES, 2), LARGE_PAGE_BYTES);
    }
}
