...
impl TcpEntry {
    ...
    pub fn new_bound_ip_opts_pacing_ipv6(conn: TcpConn, error: Arc<crate::SocketError>,
        ...
        max_pacing_rate: Arc<::core::sync::atomic::AtomicU64>) -> Self
    {
        Self::new_bound_ip_opts_pacing_ipv6_mark(conn, error, bind, bpf_filter, ip_mtu_discover,
            ipv6_mtu_discover, ipv6_frag_size, passive_listener, min_hop, ip_opts, ipv6_opts,
            max_pacing_rate, Arc::new(::core::sync::atomic::AtomicI32::new(UNMARKED_OPTION)))
    }

    /// Build a transport entry whose route lookups run under the owning
    /// socket's `SO_MARK` cell. # C: O(1)
    #[allow(clippy::too_many_arguments)]
    pub fn new_bound_ip_opts_pacing_ipv6_mark(conn: TcpConn, error: Arc<crate::SocketError>,
        bind: Option<Arc<TcpBindReservation>>, bpf_filter: Arc<crate::bpf_filter::SocketFilter>,
        ip_mtu_discover: Arc<::core::sync::atomic::AtomicI32>,
        ipv6_mtu_discover: Arc<::core::sync::atomic::AtomicI32>,
        ipv6_frag_size: Arc<::core::sync::atomic::AtomicI32>,
        ...
        mark: Arc<::core::sync::atomic::AtomicI32>) -> Self
    {
        ...
        let bind = match (bind, passive_listener.is_some()) {
            (Some(bind), false) => Some(bind),
            (Some(bind), true) => Some(Arc::new(TcpBindReservation::new_owned(
                bind.owner.clone(), conn.local, bind.reuseaddr,
                bind.reuseport, bind.v6only,
