...
fn udp6_autobind_asks_for_the_ipv6_bind_zero_right() {
    ...
    assert_eq!(check_autobind_udp_for(&sock, Some(&allowed)), Ok(()));
    let denied = net_domain(ACCESS_NET_BIND_UDP, &[(53, ACCESS_NET_BIND_UDP)]);
    assert_eq!(check_autobind_udp_for(&sock, Some(&denied)), Err(NetError::Eacces));
}

#[test]
fn udp6_send_and_connect_check_before_allocating_an_endpoint() {
    ...
    let connect_body = &source[connect..send];
    let send_body = &source[send..];
    for body in [connect_body, send_body] {
        let check = body.find("crate::landlock_addr::check_autobind_udp(sock)?;")
            .expect("UDP6 autobind asks for the bind right");
