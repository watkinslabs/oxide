//! AF_VSOCK shutdown security, state transition, and transport publication.

use core::sync::atomic::Ordering;

use super::{VsockSocket, VsockState};
use crate::vsock;
...
impl VsockSocket {
    ...
    pub fn shutdown(&self, how: crate::uapi::ShutdownHow) -> Result<(), crate::NetError> {
        self.check_shutdown_admission()?;
        self.shutdown_admitted(how)
    }

    /// Apply a raw Linux `shutdown(2)` direction after VSOCK's canonical
    /// security admission. Invalid directions are deliberately validated
    /// after admission, matching Linux `__sys_shutdown_sock` ordering.
    /// # C: O(1)
    pub fn shutdown_raw(&self, how: u32) -> Result<(), crate::NetError> {
        self.check_shutdown_admission()?;
        let how = crate::uapi::ShutdownHow::try_from(how).map_err(|_| crate::NetError::Einval)?;
        self.shutdown_admitted(how)
    }
