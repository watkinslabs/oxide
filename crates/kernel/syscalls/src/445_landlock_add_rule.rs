...
pub fn sys_landlock_add_rule(args: &SyscallArgs) -> i64 {
    ...
    match rule_type {
        RULE_PATH_BENEATH => add_path_beneath(&rs, attr),
        RULE_NET_PORT     => add_net_port(&rs, attr),
...
/// `parent_fd`.
/// # C: O(1)
fn add_path_beneath(rs: &Arc<Ruleset>, attr: u64) -> i64 {
    ...
    if let Err(e) = abi::rule_access_ok(allowed, rs.handled_fs) {
    ...
    match rs.add_fs(inode, is_dir, allowed) {
        Ok(()) => 0,
...
/// # C: O(1)
fn add_net_port(rs: &Arc<Ruleset>, attr: u64) -> i64 {
