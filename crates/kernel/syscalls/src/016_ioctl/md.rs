//! Read-only Linux MD array ioctl bridge.
...
/// Answer the MD query subset on a live canonical MD block node. Unknown
/// commands return `None` so the generic ioctl dispatcher owns `ENOTTY`.
/// # C: O(disks + members)
pub(super) fn handle_md_ioctl(file: &File, req: u64, arg: u64) -> Option<i64> {
    if !matches!(req, ::md::uapi::RAID_VERSION | ::md::uapi::GET_ARRAY_INFO | ::md::uapi::GET_DISK_INFO) { return None; }
