...
pub fn handle_fiemap(inode: &InodeRef, req: u64, arg: u64) -> Option<i64> {
    ...
    if count != 0 {
        let bytes = (count as u64).saturating_mul(FIEMAP_EXTENT_SZ);
        if arg.saturating_add(FM_EXTENTS).saturating_add(bytes) > hal::USER_VA_END {
            return Some(-(Errno::Efault.as_i32() as i64));
        }
    }
    ...
    let rv = match inode.fiemap(start, length, &mut emit) {
        Ok(()) => {
            ...
            // fm_mapped_extents into the caller's `struct fiemap`.
            unsafe { core::ptr::write_volatile((arg + FM_MAPPED) as *mut u32, mapped); }
            0
        }
