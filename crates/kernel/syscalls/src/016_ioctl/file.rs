...
pub(super) fn handle_file_ioctl(cur: &sched::Task, file: &vfs::File, req: u64, arg: u64) -> Option<i64> {
    match req {
        uapi::EXT4_IOC_GETVERSION | uapi::FS_IOC_GETVERSION =>
            Some(ioctl_getversion(file, arg)),
        uapi::EXT4_IOC_SETVERSION | uapi::FS_IOC_SETVERSION =>
            Some(ioctl_setversion(file, arg)),
        uapi::FS_IOC_GETFSLABEL => Some(ioctl_getfslabel(file, arg)),
        uapi::FS_IOC_SETFSLABEL => Some(ioctl_setfslabel(cur, file, arg)),
        uapi::FITRIM => Some(ioctl_fitrim(cur, file, arg)),
        uapi::FAT_IOCTL_GET_ATTRIBUTES => Some(ioctl_fat_get_attributes(file, arg)),
        uapi::FAT_IOCTL_SET_ATTRIBUTES => Some(ioctl_fat_set_attributes(cur, file, arg)),
        uapi::VFAT_IOCTL_READDIR_BOTH => Some(ioctl_fat_readdir(file, arg, false)),
        uapi::VFAT_IOCTL_READDIR_SHORT => Some(ioctl_fat_readdir(file, arg, true)),
        _ => None,
    }
}
...
fn ioctl_fat_get_attributes(file: &vfs::File, arg: u64) -> i64 {
    ...
    if let Err(rv) = crate::userbuf::validate_user_buf_writable(arg, uapi::INT_BYTES, 1) { return rv; }
...
fn ioctl_fat_set_attributes(cur: &sched::Task, file: &vfs::File, arg: u64) -> i64 {
    let idmap = vfs::mount::idmap_for(file.mnt_id());
    let cred = crate::pathresolve::current_cred();
    if let Err(rv) = crate::userbuf::validate_user_buf_readable(arg, uapi::INT_BYTES, 1) { return rv; }
...
fn ioctl_setversion(file: &vfs::File, arg: u64) -> i64 {
    ...
    if let Err(rv) = crate::userbuf::validate_user_buf_readable(arg, uapi::INT_BYTES, 1) {
...
fn ioctl_getfslabel(file: &vfs::File, arg: u64) -> i64 {
    ...
    if let Err(rv) = crate::userbuf::validate_user_buf_writable(arg, uapi::EXT4_LABEL_BYTES, 1) { return rv; }
...
fn ioctl_setfslabel(cur: &sched::Task, file: &vfs::File, arg: u64) -> i64 {
    ...
    if let Err(rv) = crate::userbuf::validate_user_buf_readable(arg, uapi::EXT4_LABEL_BYTES, 1) { return rv; }
    let mut buf = [0u8; uapi::EXT4_LABEL_MAX + 1];
    ...
    let mut label = [0u8; uapi::EXT4_LABEL_MAX]; label[..len].copy_from_slice(&buf[..len]);
    let m = file.vfsmount();
    if let Some(ref mnt) = m {
        if let Err(e) = vfs::mount::mnt_want_write(mnt) { return -(e as i64); }
        if mnt.sb().is_readonly() { vfs::mount::mnt_drop_write(mnt); return -(vfs::VfsError::Erofs as i64); }
...
fn ioctl_fitrim(cur: &sched::Task, file: &vfs::File, arg: u64) -> i64 {
    ...
    if let Err(rv) = crate::userbuf::validate_user_buf_readable(arg, uapi::FSTRIM_RANGE_BYTES, 1) { return rv; }
    if let Err(rv) = crate::userbuf::validate_user_buf_writable(arg, uapi::FSTRIM_RANGE_BYTES, 1) { return rv; }
    let range = match user::get_bytes::<{ uapi::FSTRIM_RANGE_BYTES as usize }>(arg) { Ok(b) => b, Err(rv) => return rv };
    ...
    let mut out = [0u8; uapi::FSTRIM_RANGE_BYTES as usize];
