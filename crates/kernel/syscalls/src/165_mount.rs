...
fn sys_mount_impl(args: &SyscallArgs) -> i64 {
    ...
    if flags & MS_REMOUNT != 0 {
        // Identify the mount by the mnt_id the path walk CROSSED INTO (Linux
        // `path->mnt`), not by re-deriving from the resolved dentry: the walk
        // follows the mount at the final component, landing on the mounted-fs
        // ROOT, which is not a mountpoint (and a pseudo-fs s_root is shared) —
        // re-derivation returned EINVAL and broke systemd's RO bind-remount.
    ...
    if flags & MS_MOVE != 0 {
        ...
        // `path->mnt`), not a re-derived dentry: the source resolves THROUGH
        // the moved mount onto its (shared) root, which can't be mapped back.
