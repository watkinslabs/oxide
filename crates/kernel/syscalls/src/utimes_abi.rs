...
// Linux's `do_futimesat`, `do_utimes`, and
// `do_utimes_fd`.
//
...
// One decode here, two thin slots — routing 261 at
...
/// Which object `do_utimes` acts on.
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
...
/// `do_utimes_fd`: the fd form takes no flags at all,
...
/// Decode a `struct timeval[2]` payload (Linux's `do_futimesat`).
///
...
/// `tstimes[i].tv_sec`, so a pre-1970 stamp is legal
...
/// (`timestamp_truncate`), never rejected here.
/// # C: O(1)
...
/// (Linux's `SYSCALL_DEFINE2(utime)`). Both fields land in
...
/// `vfs::Iattr` for `times == NULL` — Linux `ATTR_TOUCH`:
...
mod tests {
    ...
    /// `do_futimesat` never range-checks `tv_sec` (it tests
    ...
    /// `utime(2)` validates nothing at all: both fields
