...
where F: Fn(NtService, SyscallArgs) -> u64, G: Fn(NtService, SyscallArgs) -> u64 {
    let _ = crate::nt_window::caret::paint::begin_for_current(args[0]);
    let Some(hwnd) = u32::try_from(args[0]).ok().filter(|hwnd| *hwnd != 0) else { return 0; };
    let Some((window, _)) = crate::nt_window::window_rect_for_current(hwnd) else { return 0; };
    let Some(width) = window.right.checked_sub(window.left).filter(|value| *value > 0) else { return 0; };
    let Some(height) = window.bottom.checked_sub(window.top).filter(|value| *value > 0) else { return 0; };
    let backing = crate::nt_gdi::acquire_window_dc_for_current(args[0] as u32, width as i32, height as i32);
    if backing == 0 || backing == STATUS_INVALID_PARAMETER { return 0; }
    let hdc = gdi(NtService::CreateCompatibleDc, SyscallArgs { a0: width as u64, a1: height as u64, a2: 0, a3: 0, a4: 0, a5: 0 });
    if hdc == STATUS_INVALID_PARAMETER || hdc == 0 { return 0; }
    let seeded = u32::try_from(hdc).ok().is_some_and(|dc| crate::nt_gdi::seed_paint_for_current(args[0] as u32, dc).is_ok());
    if !seeded {
        let _ = gdi(NtService::DeleteGdiObject, SyscallArgs { a0: hdc, a1: 0, a2: 0, a3: 0, a4: 0, a5: 0 });
        return 0;
    }
    if crate::nt_window::paint::reserve_for_current(hwnd as u64).is_err() {
        let _ = gdi(NtService::DeleteGdiObject, SyscallArgs { a0: hdc, a1: 0, a2: 0, a3: 0, a4: 0, a5: 0 });
        return 0;
    }
    let bound = u32::try_from(args[0]).ok().zip(u32::try_from(hdc).ok()).is_some_and(|(hwnd, dc)|
        crate::nt_window::paintlease::bind_paint_dc_for_current(hwnd, dc).is_ok());
    if !bound {
...
where F: Fn(NtService, SyscallArgs) -> u64, G: Fn(NtService, SyscallArgs) -> u64 {
    ...
    let Ok(hdc) = uaccess::get_user_u64(args[1]) else { return 0; };
