...
pub fn dispatch(call: NtCall) -> u64 {
    ...
    if let Some(result) = gdi_bitmap_raw::kernel::descriptor(ordinal, &args) { return result; }
...
fn multiplexers(ordinal: u64, args: &[u64]) -> Option<u64> {
    ...
    if let Some(result) = input_context::kernel::route(ordinal, args) { return Some(result); }
    if let Some(result) = query_window::kernel::route(ordinal, args) { return Some(result); }
...
pub fn dispatch_raw(ordinal: u64, args: SyscallArgs) -> Option<u64> {
    ...
    if let Some(result) = gdi_bitmap_raw::kernel::route(ordinal, args) { return Some(result); }
