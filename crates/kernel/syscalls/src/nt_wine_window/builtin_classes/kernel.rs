...
pub(crate) fn register_for_current(procs_w: u64) -> usize {
    register_reporting(
        &mut |index| procs_w.checked_add(index as u64 * PROC_ENTRY_BYTES).and_then(|address| uaccess::get_user_u64(address).ok()),
        &mut crate::nt_window::shared_oem_cursor_for_current,
        &mut |builtin, wndproc, cursor| {
        ...
        &mut |name| {
            klog::write_raw(b"[WINDOWS-BUILTIN-CLASSES] skipped=");
