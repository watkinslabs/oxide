...
pub(super) fn snapshot(binding: ClientBinding, handle: u32) -> Result<(abi::DcText, (i64, i64)), ClientError> {
    let bytes = binding.read_text_attr(handle)?;
...
pub(super) fn set_attribute(binding: ClientBinding, handle: u32, attribute: u32, value: u32) -> Result<u32, ClientError> {
    let mut bytes = binding.read_text_attr(handle)?;
    let (current, _) = abi::decode_text_with_origin(&bytes, handle).map_err(|_| ClientError::Codec)?;
