...
pub(crate) fn render_parts(state: &mut GdiManager, hwnd: u32, bar: i32, scroll: ScrollState,
    ...
    render_dc_parts(state,dc,bar,scroll,context,interior)
}

pub(crate) fn render_dc_parts(state:&mut GdiManager,dc:u32,bar:i32,scroll:ScrollState,
    context:NonclientScrollContext,interior:bool)->Result<(u32,ScrollDrawOutcome),GdiError>{
    ...
    let attributes=state.text_state(dc)?;
    if (attributes.width,attributes.height) != (width, height) { return Err(GdiError::InvalidDimensions); }
