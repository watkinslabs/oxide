...
fn native_section_object(call: NtCall) -> Option<NtObjectCall> {
    match call.service {
        ...
        nt::NtService::NtMapViewOfSectionEx => {
            ...
            Some(NtObjectCall::MapViewOfSectionNative {
                section: call.args.a0 as u32, process: call.args.a1,
                base: UserPtr::new(call.args.a2).ok()?, zero_bits: 0, offset,
                size: UserPtr::new(call.args.a4).ok()?, protect: protect as u32,
            })
        }
        _ => None,
    }
...
pub fn dispatch(call: NtCall) -> u64 {
    ...
    if call.service == syscall::nt::NtService::WineUnixCall { return crate::nt_wine_unix::dispatch(call); }
    ...
    if let Some(result) = crate::nt_vulkan::dispatch(call) { return result; }
        ...
        return match object_call {
                ...
                let outcome = loop {
                    ...
                    let outcome = if alertable != 0 {
                        unsafe { sched::live::wait_event_interruptible_until_user_apc(table.waiters(), wait_deadline,
                            timekeeper::monotonic_ns, || cur.nt_apc_queue.has_pending(), &mut all_ready) }
                    } else { unsafe { sched::live::wait_event_interruptible_until(table.waiters(), wait_deadline, timekeeper::monotonic_ns, &mut all_ready) }.into() };
                    if matches!(outcome, sched::NtWaitOutcome::TimedOut)
                        && sched::nt_object::timer_expiry_may_have_woken(deadline, timer_deadline, timekeeper::monotonic_ns()) {
                        if all_ready() { break sched::NtWaitOutcome::Ready; }
                        // The timer may have fired without completing a wait-all,
                        // or it may have been canceled. Recompute its deadline;
                        // never return a timeout for the stale timer deadline.
                        continue;
                    }
                    break outcome;
                };
            ...
            NtObjectCall::QueryThread { thread, class, info, length, return_length } => {
                ...
                if class == THREAD_GROUP_INFORMATION_CLASS {
                    ...
                    if uaccess::copy_to_user(info.as_u64(), &out).is_err() { return STATUS_INVALID_PARAMETER; }
                    if let Some(return_length) = return_length {
                        if uaccess::put_user_u32(return_length.as_u64(), THREAD_GROUP_INFORMATION_BYTES as u32).is_err() { return STATUS_INVALID_PARAMETER; }
                    }
                    return STATUS_SUCCESS;
                }
