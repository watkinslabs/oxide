...
///
/// `comm` is the resolved process name; the caller owns that rule so this
/// stays free of crate-internal dependencies and can be compiled on its own.
/// `exe` is published only from `host_path` — the path this launch actually
/// resolved and opened — because an NT request's image name is not necessarily
/// a host pathname. A child whose image came from somewhere with no host
/// pathname keeps no `exe` at all rather than reporting one that never existed.
/// # C: O(comm.len() + host_path.len())
pub(super) fn publish(child: &sched::Task, comm: &str, host_path: Option<&str>,
                      command_line: &str) {
