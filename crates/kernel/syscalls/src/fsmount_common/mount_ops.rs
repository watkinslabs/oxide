...
fn graft_mount(spec: vfs::fs::MountSpec, target_d: &Arc<Dentry>, parent_hint: Option<u64>) -> i64 {
    if spec.strict {
        match vfs::mount::attach_sb_with_flags_at(Some(target_d.clone()), spec.sb, 0, parent_hint) {
            Ok(()) => { let _ = vfs::mount::propagate_mount(target_d); 0 }
            Err(vfs::VfsError::Eexist) => -(Errno::Ebusy.as_i32() as i64),
            Err(e) => crate::namei_common::errno_from_vfs(e),
        }
    } else {
        let _ = vfs::mount::attach_sb_with_flags_at(Some(target_d.clone()), spec.sb, 0, parent_hint);
