...
use crate::nt_access::{EVENT_MODIFY_STATE, SEMAPHORE_MODIFY_STATE, SYNCHRONIZE};
...
pub fn dispatch(call: NtCall) -> Option<u64> {
    ...
    // The alertable flag is a one-byte BOOLEAN; any nonzero value is true.
    let alertable = crate::nt_obj_sig::boolean(alertable as u64);
    ...
    let signal_access = match signal_object.kind() {
        ...
        sched::nt_object::NtObjectType::Mutant => SYNCHRONIZE,
    ...
    if table.access(wait).unwrap_or(0) & SYNCHRONIZE != SYNCHRONIZE { return Some(STATUS_ACCESS_DENIED); }
    ...
    let outcome = if let Some(event) = wait_object.event() {
        if alertable { unsafe { event.wait_alertable(deadline, timekeeper::monotonic_ns, || cur.nt_apc_queue.request_delivery()) }.into() }
        else { unsafe { event.wait(deadline, timekeeper::monotonic_ns) }.into() }
    } else if let Some(semaphore) = wait_object.semaphore() {
        if alertable { unsafe { semaphore.wait_alertable(deadline, timekeeper::monotonic_ns, || cur.nt_apc_queue.request_delivery()) } }
        else { unsafe { semaphore.wait(deadline, timekeeper::monotonic_ns).into() } }
    } else if let Some(mutant) = wait_object.mutant() {
        if alertable { unsafe { mutant.wait_alertable(cur.tid as u64, deadline, timekeeper::monotonic_ns, || cur.nt_apc_queue.request_delivery()) } }
        else { unsafe { mutant.wait(cur.tid as u64, deadline, timekeeper::monotonic_ns).into() } }
    } else if let Some(timer) = wait_object.timer() {
        if alertable { unsafe { timer.wait_alertable(deadline, timekeeper::monotonic_ns, || cur.nt_apc_queue.request_delivery()) } }
