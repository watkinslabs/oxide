...
pub(crate) fn promote_time_namespace_at_exec(task: &sched::Task) -> Result<(), Errno> {
    ...
    if namespace_identity::NamespaceRef::ptr_eq(
        &snapshot.time, &snapshot.time_for_children) { return Ok(()); }

    nscg::time_ns::freeze(&snapshot.time_for_children).map_err(|_| Errno::Eio)?;
    let promoted = snapshot.time_for_children.clone();
...
mod tests {
    ...
    fn exec_promotes_for_children_and_freezes_offsets() {
        ...
        assert!(task.replace_time_namespace_for_children(target.clone()).is_ok());

        promote_time_namespace_at_exec(&task).unwrap();

        let snapshot = task.namespace_snapshot().unwrap();
        assert!(NamespaceRef::ptr_eq(&snapshot.time, &target));
        assert!(NamespaceRef::ptr_eq(&snapshot.time_for_children, &target));
    ...
    fn exec_keeps_an_already_current_time_namespace() {
        ...
        let snapshot = task.namespace_snapshot().unwrap();
        assert!(NamespaceRef::ptr_eq(&snapshot.time, &initial));
        assert!(NamespaceRef::ptr_eq(&snapshot.time_for_children, &initial));
    }
