...
#[derive(Clone)]
pub(crate) struct RemotePosition {pub(super) target:u64,pub(super) args:[u64;7],pub(super) reply:Option<Arc<Reply>>,pub(super) compositor:bool}
impl RemotePosition {
...
pub(super) fn admit(work:&mut Vec<RemotePosition>,target:u64,args:[u64;7],reply:Option<Arc<Reply>>)->bool {
    ...
    work.push(RemotePosition {target,args,reply,compositor:false});true
}
...
/// Mark owned queued work as display-origin geometry. # C: O(1) amortized
pub(super) fn admit_compositor(work:&mut Vec<RemotePosition>,target:u64,args:[u64;7])->bool {
    if !admit(work,target,args,None){return false;}
    work.last_mut().unwrap().compositor=true;true
}
