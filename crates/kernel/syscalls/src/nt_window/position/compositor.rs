...
pub(crate) fn queue_compositor(state:&mut WindowManager,positions:&mut Vec<RemotePosition>,id:WindowId,next:WindowRect)->bool {
    ...
    work::admit_compositor(positions,record.owner_tid,args,next)
}

/// Compare at consumption, after preceding queued positions have committed. # C: O(1)
pub(super) fn plan(mut args:[u64;7],old:WindowRect,next:WindowRect)->Option<[u64;7]> {
    ...
    if flags&(NOMOVE|NOSIZE)==NOMOVE|NOSIZE{return None;}
    args[6]=flags as u64;Some(args)
}
