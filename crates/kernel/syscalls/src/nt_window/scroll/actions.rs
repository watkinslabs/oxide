//! Consume canonical scrollbar actions through the existing owners.
use super::{pending, SBM_SETSCROLLINFO};
...
pub fn consume_actions<S: ScrollActionSink + ?Sized>(
    ...
    if !action.hide && action.repaint && !sink.repaint_scrollbar(hwnd, bar, action.redraw) { return pending::Outcome::Failed; }
