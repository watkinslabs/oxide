...
/// Child rectangles are parent-client-relative; screen results walk the
/// canonical parent chain. Client results remain local `[0,width]x[0,height]`.
    ...
    let rect = match kind {
        ...
        RectKind::Window => {
            ...
            while let Some(parent) = state.get(current)?.parent {
                ...
                let client_origin = parent_record.client_rect.unwrap_or(parent_rect);
                let offset_x = client_origin.left;
                let offset_y = client_origin.top;
