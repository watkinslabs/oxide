...
/// Validating the whole desktop repaints it rather than discarding coverage.
pub(crate) const DESKTOP_VALIDATE_FLAGS: u32 = RDW_ALLCHILDREN | RDW_INVALIDATE | RDW_FRAME | RDW_ERASE | RDW_ERASENOW;
...
pub(crate) fn decode(ordinal: u64, args: [u64; 3]) -> Option<Request> {
    let erase = args[2] != 0;
    match ordinal {
        VALIDATE_RECT if args[0] == 0 => Some(Request::Redraw { hwnd: 0, rect: 0, region: 0, flags: DESKTOP_VALIDATE_FLAGS }),
