...
// Linux `cp_new_stat` builds one native struct and hands it to one
// `copy_to_user`. Keep the same shape: scalar user writes each perform their
// own access/exception-table setup and made every stat call do 15–18 such
// trips before the caller even saw the record.
...
pub(crate) fn write_new_stat_user(buf: u64, st: &NewStat) -> Result<(), i64> {
    ...
    {
        let mut bytes = [0u8; STAT_BYTES_X86_64 as usize];
        write_x86_64(&mut SliceSink { out: &mut bytes }, st);
        return um::put_bytes(buf, &bytes).map_err(|_| um::EFAULT);
    }
    #[cfg(target_arch = "aarch64")]
    {
        let mut bytes = [0u8; STAT_BYTES_AARCH64 as usize];
        write_aarch64(&mut SliceSink { out: &mut bytes }, st);
        return um::put_bytes(buf, &bytes).map_err(|_| um::EFAULT);
    }
}
