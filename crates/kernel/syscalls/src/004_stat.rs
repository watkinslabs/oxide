...
pub(crate) fn stat_impl(args: &SyscallArgs, follow: bool) -> i64 {
    ...
    let nlink = inode.nlink() as u64;
    let blksize = inode.blksize() as i64;
    ...
    // Real perms via the inode (matches sys_fstat/sys_statx); fall back by
    // file type when the FS exposes no per-inode mode.
    let mode = mode_type | inode.perm()
        .unwrap_or_else(|| crate::namei_common::default_perm_for(file_type)) as u32;
    ...
    unsafe {
        ...
        core::ptr::write_volatile((buf +  16)     as *mut u64, nlink);
        ...
        core::ptr::write_volatile((buf +  56)     as *mut i64, blksize);
    }
    0
}
