...
fn convert(target: u64, source: u64, allocate: u64) -> u64 {
    ...
    for index in 0..length { let mut byte = [0u8; 1]; if uaccess::copy_from_user(&mut byte, source_buffer + index as u64).is_err() { if owned { free(destination); } return INVALID; } let value = if byte[0].is_ascii() { byte[0] as u16 } else { 0xfffd }; wide[index * 2..index * 2 + 2].copy_from_slice(&value.to_le_bytes()); }
