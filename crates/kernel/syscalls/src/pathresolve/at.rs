...
pub fn resolve_parent_at_flags(dirfd: i32, raw: &str, flags: vfs::LookupFlags) -> Result<vfs::VfsPath, i64> {
    ...
    // reference: a symlink as the FINAL component is resolved, so the caller
    // acts on what the link points at rather than on the link's own name. It is
    // the single bit that separates the create family (clear — an existing final
    // component is the caller's to reject) from `open(O_CREAT)` (set — the
    // create lands on the link's target).
    //
    // The follow itself is the WALKER's, not this shim's: `open_last_lookups`
    // resolves the trailing component and `step_into(WALK_TRAILING)` continues
    // the same walk on the link's target. Re-entering the walk here from a
    // rewritten pathname would be a second resolver — one that re-walks the
    // whole path per hop, keeps its own symlink budget, and looks the leaf up
    // through the inode instead of the dcache.
    if flags.confines_to_dirfd() { resolve_confined(dirfd, raw, flags) }
    else { resolve_at_path(dirfd, raw, flags) }
}
