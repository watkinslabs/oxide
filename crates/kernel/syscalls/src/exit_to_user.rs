...
/// Deliver one pending Windows exception into the user dispatcher. The frame
/// build lives in its own module; this loop only owns the decision to run it.
/// # SAFETY: `regs` is the live return-to-user frame of the current task.
#[cfg(target_arch = "x86_64")]
unsafe fn deliver_nt_exception(regs: *mut UserRegs) -> bool {
    // SAFETY: forwarding this pass's live entry frame to the one frame builder.
    unsafe { crate::nt_exception_frame::deliver(regs) }
}
