...
fn xfs_qstatv_reads_version_before_state_snapshot_hosted() {
    ...
    assert_eq!(xfs::dispatch(&sb, xfs::Q_XGETQSTATV, vfs::QuotaType::User, &qidns::QuotaIdCtx::initial(vfs::QuotaType::User, 0), 0), eno(Errno::Efault));
    assert_eq!(
        xfs::dispatch(&sb, xfs::Q_XGETQSTATV, vfs::QuotaType::User, &qidns::QuotaIdCtx::initial(vfs::QuotaType::User, 0), &mut wrong_version as *mut i8 as u64),
        eno(Errno::Einval),
    );
...
fn xfs_qgetqstat_checks_get_state_support_before_copyout_hosted() {
    ...
    assert_eq!(xfs::dispatch(&sb, xfs::Q_XGETQSTAT, vfs::QuotaType::User, &qidns::QuotaIdCtx::initial(vfs::QuotaType::User, 0), 0), eno(Errno::Enosys));
}
...
fn xfs_qstat_state_error_precedes_copyout_hosted() {
    ...
    assert_eq!(xfs::dispatch(&sb, xfs::Q_XGETQSTAT, vfs::QuotaType::User, &qidns::QuotaIdCtx::initial(vfs::QuotaType::User, 0), 0), eno(Errno::Eio));
    assert_eq!(xfs::dispatch(&sb, xfs::Q_XGETQSTATV, vfs::QuotaType::User, &qidns::QuotaIdCtx::initial(vfs::QuotaType::User, 0), &mut outv as *mut FsQuotaStatv as u64), eno(Errno::Eio));
}
...
fn xfs_qstat_zero_flags_return_enosys_hosted() {
    ...
    assert_eq!(xfs::dispatch(&sb, xfs::Q_XGETQSTAT, vfs::QuotaType::User, &qidns::QuotaIdCtx::initial(vfs::QuotaType::User, 0), &mut out as *mut FsQuotaStat as u64), eno(Errno::Enosys));
    assert_eq!(xfs::dispatch(&sb, xfs::Q_XGETQSTATV, vfs::QuotaType::User, &qidns::QuotaIdCtx::initial(vfs::QuotaType::User, 0), &mut outv as *mut FsQuotaStatv as u64), eno(Errno::Enosys));
}
...
fn xfs_qgetqstat_maps_filesystem_state_and_project_group_fallback_hosted() {
    ...
    assert_eq!(xfs::dispatch(&sb, xfs::Q_XGETQSTAT, vfs::QuotaType::User, &qidns::QuotaIdCtx::initial(vfs::QuotaType::User, 0), &mut out as *mut FsQuotaStat as u64), 0);
...
fn xfs_qstatv_maps_all_filesystem_state_slots_hosted() {
    ...
    assert_eq!(xfs::dispatch(&sb, xfs::Q_XGETQSTATV, vfs::QuotaType::User, &qidns::QuotaIdCtx::initial(vfs::QuotaType::User, 0), &mut out as *mut FsQuotaStatv as u64), 0);
    assert_eq!(out.qs_flags, FS_QUOTA_UDQ_ACCT | FS_QUOTA_UDQ_ENFD | FS_QUOTA_PDQ_ACCT | FS_QUOTA_PDQ_ENFD);
