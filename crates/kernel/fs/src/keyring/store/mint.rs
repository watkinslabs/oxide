...
impl Store {
    ...
    {
        self.alloc(ty, desc, payload, uid, gid, perm, quota_bytes, mode, KEY_IS_POSITIVE)
    }
    ...
    pub fn mint_not_in_quota(&mut self, ty: &'static KeyType, desc: &str, uid: u32, gid: u32,
        perm: u32) -> Result<i32, Errno>
    {
        let serial = self.alloc(ty, desc, Vec::new(), uid, gid, perm, 0, Quota::Overrun, KEY_IS_POSITIVE)?;
    ...
    pub fn mint_uninstantiated(&mut self, ty: &'static KeyType, desc: &str, uid: u32, gid: u32,
        perm: u32, quota_bytes: u64) -> Result<i32, Errno>
    {
        let serial = self.alloc(ty, desc, Vec::new(), uid, gid, perm, quota_bytes,
            Quota::InQuota, KEY_IS_UNINSTANTIATED)?;
    ...
    fn alloc(&mut self, ty: &'static KeyType, desc: &str, payload: Vec<u8>, uid: u32, gid: u32,
        perm: u32, quota_bytes: u64, mode: Quota, state: i32) -> Result<i32, Errno>
    {
        ...
        self.keys.insert(serial, Key {
            serial, key_type: ty, description: String::from(desc), payload,
