...
fn add_key_updates_an_existing_key_in_place() {
    ...
    let a = add_key_core(&t, "user", "refresh-me", alloc::vec![1], 0) as i32;
    let b = add_key_core(&t, "user", "refresh-me", alloc::vec![2, 2], 0) as i32;
    assert_eq!(a, b, "same type+description in the same ring updates in place");
    assert_eq!(read_core(&t, a), Ok(alloc::vec![2, 2]), "the payload was replaced");
    let members = members_of(sess).expect("session is a keyring");
...
fn add_key_unknown_type_is_enodev() {
    let t = ctx(1021, 6201);
    assert_eq!(add_key_core(&t, "not-a-real-type", "x", alloc::vec![], 0), err(Errno::Enodev));
}

// An empty description is EINVAL (`key_create_or_update`'s
// `!index_key.description` test); a `keyring` whose name starts with `.` is
// EPERM (reserved for kernel-internal keyrings).
#[test]
fn add_key_rejects_empty_and_dotted_keyring_descriptions() {
    let t = ctx(1022, 6202);
    assert_eq!(add_key_core(&t, "user", "", alloc::vec![], 0), einval());
    assert_eq!(add_key_core(&t, "keyring", ".hidden", alloc::vec![], 0), eperm());
    assert!(add_key_core(&t, "keyring", "visible", alloc::vec![], 0) >= FIRST_SERIAL as i64);
}

// `logon_vet_description` requires a non-empty `subsystem:key` form.
#[test]
fn logon_descriptions_must_carry_a_subsystem_prefix() {
    let t = ctx(1023, 6203);
    assert_eq!(add_key_core(&t, "logon", "nocolon", alloc::vec![1], 0), einval());
    assert_eq!(add_key_core(&t, "logon", ":leading", alloc::vec![1], 0), einval());
    assert_eq!(add_key_core(&t, "logon", "trailing:", alloc::vec![1], 0), einval());
    assert!(add_key_core(&t, "logon", "sub:key", alloc::vec![1], 0) >= FIRST_SERIAL as i64);
}
...
fn logon_payload_is_not_readable() {
    let t = ctx(1025, 6204);
    let k = add_key_core(&t, "logon", "sub:secret", alloc::vec![7, 7], 0) as i32;
    assert_eq!(read_core(&t, k), Err(err(Errno::Eopnotsupp)),
...
fn revoke_makes_later_ops_ekeyrevoked() {
    let t = ctx(1026, 6205);
    let k = add_key_core(&t, "user", "revoke-me", alloc::vec![1], 0) as i32;
    assert_eq!(revoke_core(&t, k), 0);
    assert_eq!(read_core(&t, k), Err(err(Errno::Ekeyrevoked)));
    assert_eq!(update_core(&t, k, alloc::vec![2]), err(Errno::Ekeyrevoked));
...
fn invalidate_removes_the_key_from_every_keyring() {
    let t = ctx(1027, 6206);
    let sess = get_keyring_id(&t, KEY_SPEC_SESSION_KEYRING, true) as i32;
    let k = add_key_core(&t, "user", "invalidate-me", alloc::vec![1], 0) as i32;
    assert!(members_of(sess).expect("keyring").contains(&k));
    assert_eq!(invalidate_core(&t, k), 0);
    assert_eq!(read_core(&t, k), Err(enokey()), "invalidated is ENOKEY, not EKEYREVOKED");
...
fn set_timeout_expires_the_key() {
    let t = ctx(1028, 6207);
    let k = add_key_core(&t, "user", "expire-me", alloc::vec![1], 0) as i32;
    assert_eq!(set_timeout_core(&t, k, 60), 0);
    assert_eq!(read_core(&t, k), Ok(alloc::vec![1]), "still live before the deadline");
    let mut later = ctx(1028, 6207);
    later.now_ns = 61 * 1_000_000_000;
    assert_eq!(read_core(&later, k), Err(err(Errno::Ekeyexpired)));
    // Clearing the timeout brings it back (`secs == 0` → no expiry).
    assert_eq!(set_timeout_core(&later, k, 0), 0);
    assert_eq!(read_core(&later, k), Ok(alloc::vec![1]));
}
...
fn set_timeout_has_no_sysadmin_bypass() {
    ...
    let k = add_key_core(&owner, "user", "timeout-perm", alloc::vec![1], 0) as i32;
...
fn setperm_rejects_bits_outside_the_four_bytes() {
    let t = ctx(1031, 6210);
    let k = add_key_core(&t, "user", "setperm-bits", alloc::vec![], 0) as i32;
...
fn setperm_needs_owner_or_sysadmin_even_with_setattr() {
    ...
    let k = add_key_core(&owner, "user", "setperm-owner", alloc::vec![], 0) as i32;
...
fn chown_privileged_transfers_need_sysadmin() {
    ...
    let k = add_key_core(&owner, "user", "chown-me", alloc::vec![], 0) as i32;
...
fn describe_matches_the_linux_descriptor_format() {
    let t = ctx(1037, 6215);
    let k = add_key_core(&t, "user", "describe-fmt", alloc::vec![], 0) as i32;
...
fn default_perm_matches_key_create_or_update() {
    let t = ctx(1038, 6216);
    let k = add_key_core(&t, "user", "default-perm", alloc::vec![4], 0) as i32;
    let perm = STORE.lock().keys[&k].perm;
    ...
    assert_eq!(read_core(&t, k), Ok(alloc::vec![4]), "the owner reads it by possession");
}
...
fn default_perm_drops_pos_read_for_an_unreadable_type() {
    let t = ctx(1039, 6217);
    let k = add_key_core(&t, "logon", "sub:noread", alloc::vec![1], 0) as i32;
    let perm = STORE.lock().keys[&k].perm;
