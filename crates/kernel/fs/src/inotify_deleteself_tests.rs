...
fn an_overwritten_rename_target_reports_attrib() {
    ...
    add_or_update_watch(&g, inode_key(&victim), victim.fsid(), IN_ATTRIB, false).unwrap();
...
fn delete_self_on_a_directory_carries_no_isdir() {
    ...
    add_or_update_watch(&g, inode_key(&d), d.fsid(), FAN_DELETE_SELF, true).unwrap();
...
fn the_two_legs_are_separately_maskable() {
    ...
    add_or_update_watch(&g, inode_key(&f), f.fsid(), FAN_DELETE_SELF, false).unwrap();
