...
fn ev(wd: i32, mask: u32, cookie: u32, name: &[u8]) -> Event {
    Event { wd, mask, cookie, name: name.to_vec(), obj: None, pid: 0 }
}
...
fn deleting_the_watched_inode_frees_the_wd_and_queues_ignored() {
    ...
    let f = mk(0x7001);
    let wd = add_or_update_watch(&g, inode_key(&f), f.fsid(), FAN_DELETE_SELF).unwrap();
...
fn a_recycled_ino_gets_a_fresh_wd_not_the_dead_one() {
    let g = InotifyData::new(0);
    let old = mk(0x7002);
    let first = add_or_update_watch(&g, inode_key(&old), old.fsid(), FAN_MODIFY).unwrap();

    fire_delete_self(&old);

    // Same fsid + ino: a fresh file that landed on the recycled inode number.
    let new = mk(0x7002);
    let second = add_or_update_watch(&g, inode_key(&new), new.fsid(), FAN_MODIFY).unwrap();
...
fn mount_scope_marks_survive_an_inode_delete() {
    ...
    let f = mk(0x7003);
    assert_eq!(apply_mark(&g, MarkScope::Mount, 0, f.fsid(), FAN_MODIFY, true, false), 0);
    add_or_update_watch(&g, inode_key(&f), f.fsid(), FAN_MODIFY).unwrap();
...
fn a_fanotify_inode_mark_dies_without_an_ignored_record() {
    let g = InotifyData::new_fanotify(0);
    let f = mk(0x7004);
    assert_eq!(apply_mark(&g, MarkScope::Inode, inode_key(&f), f.fsid(), FAN_MODIFY, true, false), 0);
...
fn oneshot_retires_the_watch_so_a_later_rm_watch_is_correctly_einval() {
    let g = InotifyData::new(0);
    let f = mk(0x7005);
    let wd = add_or_update_watch(&g, inode_key(&f), f.fsid(), FAN_MODIFY | IN_ONESHOT).unwrap();
