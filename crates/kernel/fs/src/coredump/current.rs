...
fn snapshot(cur: &sched::Task, signo: i32) -> CoreContext {
    ...
    let vtgid = cur.security.vtgid.load(Ordering::Acquire);
    let vtid = cur.security.vtid.load(Ordering::Acquire);
    ...
    CoreContext {
        ...
        gid: cur.security.creds.rgid.load(Ordering::Acquire),
        dumpable: cur.security.dumpable.load(Ordering::Acquire) as i32,
