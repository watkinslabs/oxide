...
impl EpollData {
    ...
    pub(super) unsafe fn prepare_park(&self, observed_global: u64, deadline_ns: u64) -> bool {
        ...
        unsafe { self.waiters.prepare_to_wait_interruptible_with_deadline_range(deadline_ns, slack_ns); }
        drop(ready);
        true
    }
...
pub(super) fn sweep_stuck_epolls(now_ns: u64) {
    ...
    for ep in epolls_snapshot() {
        ...
        for item in entries {
            ...
            klog::write_raw(b" ino=");
            klog::write_hex_u64(item.file.upgrade().map(|f| f.inode().ino()).unwrap_or(0));
            klog::write_raw(b" src=");
            klog::write_raw(if item.poll_source.is_some() { b"wired" } else { b"none" });
