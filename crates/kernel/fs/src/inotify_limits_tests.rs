...
fn ev(wd: i32, mask: u32) -> Event {
    Event { wd, mask, cookie: 0, name: Vec::new(), obj: None, pid: 0 }
}
...
fn a_new_watch_past_the_per_user_ceiling_is_enospc() {
    let uid = 91_001;
    let g = InotifyData::new_owned(0, false, uid);
    set_max_user_watches(2);
    assert!(add_or_update_watch(&g, 1, 0x10, IN_MODIFY).is_ok());
    assert!(add_or_update_watch(&g, 2, 0x10, IN_MODIFY).is_ok());
    assert_eq!(add_or_update_watch(&g, 3, 0x10, IN_MODIFY),
    ...
    assert!(add_or_update_watch(&g, 1, 0x10, IN_OPEN).is_ok());
    // Removing one frees exactly one slot.
    let wd = g.watches.lock()[0].wd;
    assert_eq!(remove_watch(&g, wd), Ok(()));
    assert!(add_or_update_watch(&g, 3, 0x10, IN_MODIFY).is_ok());
    drop(g);
...
fn closing_a_group_returns_its_instance_and_watch_charges() {
    ...
    let g = InotifyData::new_owned(0, false, uid);
    add_or_update_watch(&g, 1, 0x10, IN_MODIFY).unwrap();
    add_or_update_watch(&g, 2, 0x10, IN_MODIFY).unwrap();
...
fn a_fanotify_group_charges_marks_not_watches() {
    let uid = 91_003;
    let g = InotifyData::new_owned(0, true, uid);
    assert_eq!(apply_mark(&g, MarkScope::Inode, 7, 0x10, FAN_OPEN, true, false), 0);
    ...
    assert_eq!(apply_mark(&g, MarkScope::Inode, 7, 0x10, FAN_OPEN, false, false), 0);
...
fn fan_unlimited_marks_exempts_a_group_from_the_mark_account() {
    let uid = 91_004;
    // FAN_UNLIMITED_MARKS.
    let g = InotifyData::new_owned(0x0000_0020, true, uid);
    assert_eq!(apply_mark(&g, MarkScope::Inode, 7, 0x10, FAN_OPEN, true, false), 0);
...
fn the_queue_depth_is_snapshot_at_group_creation() {
    ...
    let g = InotifyData::new_owned(0, false, 91_005);
...
fn excl_unlink_suppresses_path_events_on_an_unlinked_file_only() {
    let g = InotifyData::new_owned(0, false, 91_006);
    let f = file(600_001);
    let wd = add_or_update_watch(&g, inode_key(&f), f.fsid(), IN_MODIFY | IN_EXCL_UNLINK).unwrap();
...
fn without_excl_unlink_an_unlinked_files_events_still_arrive() {
    let g = InotifyData::new_owned(0, false, 91_007);
    let f = file(600_002);
    let wd = add_or_update_watch(&g, inode_key(&f), f.fsid(), IN_MODIFY).unwrap();
    fire_self_path(&f, IN_MODIFY, true);
    assert_eq!(read_pair(&g), (wd, IN_MODIFY));
    drop(g);
}
...
fn unmount_reports_in_unmount_then_in_ignored_and_frees_the_wd() {
    let g = InotifyData::new_owned(0, false, 91_008);
    // Only IN_MODIFY was requested; IN_UNMOUNT arrives regardless.
    let wd = add_or_update_watch(&g, 600_003, 0xCC, IN_MODIFY).unwrap();
...
fn unmount_leaves_marks_on_other_filesystems_alone() {
    let g = InotifyData::new_owned(0, false, 91_009);
    let a = file(600_004);
    add_or_update_watch(&g, 4_001, 0xAA, IN_MODIFY).unwrap();
    let keep = add_or_update_watch(&g, 4_002, 0xBB, IN_MODIFY).unwrap();
    let _ = a;
...
fn a_fid_mode_group_reports_a_file_handle_and_no_descriptor() {
    // FAN_REPORT_FID.
    let g = InotifyData::new_owned(0x0000_0200, true, 91_010);
    let f = file(0x4242);
    g.enqueue_event(Event { wd: 1, mask: FAN_OPEN, cookie: 0, name: Vec::new(),
                            obj: Some(f.clone()), pid: 77 });
    let mut buf = [0u8; 128];
    let n = g.read_fanotify(&mut buf).unwrap();
    let want = fan_layout::FAN_EVENT_METADATA_LEN + fan_layout::fid_info_len(8, 0);
    ...
    assert_eq!(u16::from_le_bytes([info[2], info[3]]), fan_layout::fid_info_len(8, 0) as u16);
    assert_eq!(i32::from_le_bytes(info[16..20].try_into().unwrap()), fan_layout::FILEID_INO32_GEN);
    assert_eq!(u32::from_le_bytes(info[20..24].try_into().unwrap()), 0x4242, "handle carries the ino");
    drop(g);
}
...
fn a_dir_fid_name_group_reports_the_entry_name() {
    ...
    let g = InotifyData::new_owned(0x0000_0400 | 0x0000_0800, true, 91_011);
    let d = file(0x99);
    g.enqueue_event(Event { wd: 1, mask: FAN_CREATE, cookie: 0, name: b"kid".to_vec(),
                            obj: Some(d.clone()), pid: 5 });
    let mut buf = [0u8; 128];
    let n = g.read_fanotify(&mut buf).unwrap();
    let want = fan_layout::FAN_EVENT_METADATA_LEN + fan_layout::fid_info_len(8, 3);
    assert_eq!(n, want);
    let info = &buf[24..want];
    assert_eq!(info[0], fan_layout::FAN_EVENT_INFO_TYPE_DFID_NAME);
    assert_eq!(&info[28..31], b"kid");
    assert_eq!(info[31], 0, "the name is NUL-terminated");
    drop(g);
}

#[test]
fn a_fanotify_buffer_too_small_for_the_first_event_is_einval() {
    let g = InotifyData::new_owned(0x0000_0200, true, 91_012);
    g.enqueue_event(Event { wd: 1, mask: FAN_OPEN, cookie: 0, name: Vec::new(),
                            obj: Some(file(3)), pid: 0 });
...
fn a_legacy_fanotify_group_still_emits_bare_metadata() {
    let g = InotifyData::new_owned(0, true, 91_013);
    g.enqueue_event(Event { wd: 1, mask: FAN_OPEN, cookie: 0, name: Vec::new(), obj: None, pid: 3 });
    let mut buf = [0u8; 128];
