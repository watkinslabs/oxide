...
impl FileOps for ReadOps {
    fn read(&self, _inode: &Inode, off: u64, buf: &mut [u8]) -> KResult<usize> {
        ...
        READ_OFF.store(off, Ordering::SeqCst);
...
fn reset() {
    ...
    READ_OFF.store(u64::MAX, Ordering::SeqCst);
}
...
fn install_current_with_fdt(fdt: Option<Arc<FdTable>>) -> &'static Task {
    let task = Box::leak(Box::new(Task::new(0x1700, "pread64-test", SchedClass::Normal { weight: 1024 })));
...
fn ebadf_paths_precede_user_buffer_validation_even_zero_length() {
    ...
    assert_eq!(pread64_syscall::sys_pread64(&args(7, 0, 0, 0)), -(Errno::Ebadf.as_i32() as i64));
    assert_eq!(userbuf::VALIDATE_CALLS.load(Ordering::SeqCst), 0);
    reset();
}
...
fn fmode_gates_precede_user_buffer_validation() {
    ...
    install_current_with_fdt(Some(Arc::clone(&fdt)));
    assert_eq!(pread64_syscall::sys_pread64(&args(fd as u64, 0, 1, 0)), -(VfsError::Espipe as i64));
    assert_eq!(userbuf::VALIDATE_CALLS.load(Ordering::SeqCst), 0);

    let fdt = Arc::new(FdTable::new());
    let fd = fdt.alloc(mk_file(FileType::Regular, OpenFlags::O_WRONLY)).unwrap();
    install_current_with_fdt(Some(Arc::clone(&fdt)));
    assert_eq!(pread64_syscall::sys_pread64(&args(fd as u64, 0, 1, 0)), -(VfsError::Ebadf as i64));
    assert_eq!(userbuf::VALIDATE_CALLS.load(Ordering::SeqCst), 0);
    reset();
}
...
fn directory_eisdir_happens_after_user_buffer_validation() {
    ...
    assert_eq!(pread64_syscall::sys_pread64(&args(fd as u64, 0, 1, 0)), -(Errno::Efault.as_i32() as i64));
    assert_eq!(pread64_syscall::sys_pread64(&args(fd as u64, buf.as_mut_ptr() as u64, 1, 0)), -(VfsError::Eisdir as i64));
    assert_eq!(userbuf::VALIDATE_CALLS.load(Ordering::SeqCst), 2);
...
fn zero_length_still_enters_vfs_read_and_accounts_syscall() {
    ...
    assert_eq!(pread64_syscall::sys_pread64(&args(fd as u64, 0, 0, 5)), 0);
    assert_eq!(userbuf::VALIDATE_CALLS.load(Ordering::SeqCst), 0);
    assert_eq!(READ_CALLS.load(Ordering::SeqCst), 1);
    assert_eq!(READ_LEN.load(Ordering::SeqCst), 0);
    assert_eq!(READ_OFF.load(Ordering::SeqCst), 5);
...
fn validates_original_count_then_clamps_backend_count_and_keeps_f_pos() {
    ...
    assert_eq!(pread64_syscall::sys_pread64(&args(fd as u64, buf.as_mut_ptr() as u64, 32, 9)),
    ...
    assert_eq!(READ_OFF.load(Ordering::SeqCst), 9);
    assert_eq!(&buf[..userbuf::TEST_MAX_RW_COUNT], &[9, 10, 11, 12, 13, 14, 15, 16]);
    assert_eq!(task.io_rchar.load(Ordering::SeqCst), userbuf::TEST_MAX_RW_COUNT as u64);
    assert_eq!(task.io_syscr.load(Ordering::SeqCst), 1);
    reset();
}
