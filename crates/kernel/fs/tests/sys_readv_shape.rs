...
impl FileOps for ReadOps {
    fn read(&self, _inode: &Inode, off: u64, buf: &mut [u8]) -> KResult<usize> {
        ...
        READ_LEN_SUM.fetch_add(buf.len(), Ordering::SeqCst);
...
fn reset() {
    ...
    READ_LEN_SUM.store(0, Ordering::SeqCst);
...
fn install_current_with_fdt(fdt: Option<Arc<FdTable>>) -> &'static Task {
    let task = Box::leak(Box::new(Task::new(0x1900, "readv-test", SchedClass::Normal { weight: 1024 })));
...
fn ebadf_and_fmode_precede_iovec_import() {
    ...
    assert_eq!(readv_syscall::sys_readv(&args(7, 0, 1)), -(Errno::Ebadf.as_i32() as i64));
    assert_eq!(userbuf::VALIDATE_IOV_CALLS.load(Ordering::SeqCst), 0);

    let fdt = Arc::new(FdTable::new());
    let fd = fdt.alloc(mk_file(OpenFlags::O_WRONLY)).unwrap();
    let task = install_current_with_fdt(Some(Arc::clone(&fdt)));
    assert_eq!(readv_syscall::sys_readv(&args(fd as u64, 0, 1)), -(Errno::Ebadf.as_i32() as i64));
    assert_eq!(userbuf::VALIDATE_IOV_CALLS.load(Ordering::SeqCst), 0);
...
fn zero_iov_still_checks_file_and_accounts() {
    ...
    assert_eq!(readv_syscall::sys_readv(&args(fd as u64, 0, 0)), 0);
    assert_eq!(userbuf::VALIDATE_IOV_CALLS.load(Ordering::SeqCst), 0);
    assert_eq!(READ_CALLS.load(Ordering::SeqCst), 0);
    assert_eq!(task.io_rchar.load(Ordering::SeqCst), 0);
    assert_eq!(task.io_syscr.load(Ordering::SeqCst), 1);
    reset();
}
...
fn iovcnt_over_limit_after_fd_and_mode() {
    ...
    assert_eq!(readv_syscall::sys_readv(&args(fd as u64, 0, 1025)), -(Errno::Einval.as_i32() as i64));
    assert_eq!(userbuf::VALIDATE_IOV_CALLS.load(Ordering::SeqCst), 0);
    assert_eq!(task.io_syscr.load(Ordering::SeqCst), 1);
    reset();
}
...
fn validates_iovec_array_then_each_destination_buffer() {
    ...
    assert_eq!(readv_syscall::sys_readv(&args(fd as u64, iov.as_ptr() as u64, 1)), -(Errno::Efault.as_i32() as i64));
...
fn max_rw_count_caps_aggregate_iovec_import_and_accounts() {
    ...
    let mut b = [0u8; 6];
    let iov = [
        Iov { base: a.as_mut_ptr() as u64, len: a.len() as u64 },
        Iov { base: b.as_mut_ptr() as u64, len: b.len() as u64 },
    ];

    assert_eq!(readv_syscall::sys_readv(&args(fd as u64, iov.as_ptr() as u64, iov.len() as u64)), 8);
    ...
    assert_eq!(READ_LEN_SUM.load(Ordering::SeqCst), userbuf::MAX_RW_COUNT);
    assert_eq!(READ_OFF_FIRST.load(Ordering::SeqCst), 20);
    assert_eq!(file.pos(), 28);
    assert_eq!(&a, &[20, 21, 22, 23, 24, 25]);
    assert_eq!(&b[..2], &[26, 27]);
    assert_eq!(&b[2..], &[0, 0, 0, 0]);
    assert_eq!(task.io_rchar.load(Ordering::SeqCst), userbuf::MAX_RW_COUNT as u64);
    assert_eq!(task.io_syscr.load(Ordering::SeqCst), 1);
    reset();
}
...
fn backend_error_after_partial_read_returns_partial_count() {
    ...
    assert_eq!(readv_syscall::sys_readv(&args(fd as u64, iov.as_ptr() as u64, iov.len() as u64)), 4);
    assert_eq!(READ_CALLS.load(Ordering::SeqCst), 2);
    assert_eq!(task.io_rchar.load(Ordering::SeqCst), 4);
    assert_eq!(task.io_syscr.load(Ordering::SeqCst), 1);
    reset();
}
