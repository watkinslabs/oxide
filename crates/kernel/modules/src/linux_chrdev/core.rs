...
struct LinuxGemMmapBacking { ops: usize, file: Arc<File>, offset: u64, object: AtomicUsize, opens: AtomicUsize }
...
impl CharDevOps for LinuxCharOps {
    ...
    fn mmap_backing_file(&self, _devt: Devt, file: &Arc<File>, off: u64) -> vfs::KResult<Option<Arc<dyn vmm::FileBacking>>> {
        if self.ops().and_then(|ops| ops.mmap).is_none() { return Ok(None); }
        Ok(Some(Arc::new(LinuxGemMmapBacking { ops: self.ops, file: Arc::clone(file), offset: off, object: AtomicUsize::new(0), opens: AtomicUsize::new(0) })))
    }
