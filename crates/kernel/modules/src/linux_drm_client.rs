...
fn supported(dev: *mut c_void) -> bool {
    // SAFETY: dev was null-checked by the caller before this call; DRM_DEVICE_DRIVER_OFF and
    // DRM_DEVICE_FEATURES_OFF are verified device offsets, and DRM_DRIVER_DUMB_CREATE_OFF is
    // read only after driver is confirmed non-null.
...
fn new_modesets(crtcs: &[CrtcRecord]) -> Option<(usize, Layout, Vec<(usize, Layout)>)> {
    ...
    // SAFETY: modesets_layout was computed and validated by layout() immediately above this call.
    ...
    for (index, crtc) in crtcs.iter().enumerate() {
        ...
        // SAFETY: connectors_layout was computed and validated by layout() immediately above this call.
        ...
        // SAFETY: modesets was sized for DRM_MODESET_SIZE * count with count > crtcs.len(),
        // so index * DRM_MODESET_SIZE + these offsets stays within the allocation.
...
fn free_modesets(modesets: usize, modesets_layout: Layout, connector_arrays: Vec<(usize, Layout)>) {
    // SAFETY: each (connectors, layout) pair is one allocation new_modesets made with that
    // exact layout, now being torn down together.
    for (connectors, layout) in connector_arrays { unsafe { dealloc(connectors as *mut u8, layout); } }
    // SAFETY: modesets/modesets_layout is the exact allocation new_modesets returned to
    // this caller, checked non-zero before the free.
    if modesets != 0 { unsafe { dealloc(modesets as *mut u8, modesets_layout); } }
}

fn detach(record: ClientRecord) {
    // SAFETY: record.client is the live client being detached; DRM_CLIENT_MODESETS_OFF and
    // DRM_CLIENT_FILE_OFF are the verified fields drm_client_init wrote.
    ...
    // SAFETY: record.client is the live client being detached and DRM_CLIENT_LIST_OFF is
    // its verified list-link offset in the device client list.
    ...
    // SAFETY: record.file is the live file this client owns and DRM_FILE_LHEAD_OFF is its
    // verified list-link offset within the device's internal filelist.
    ...
    // SAFETY: record.client is the live client being detached and offset 0 holds the
    // device back-pointer drm_client_init wrote.
    gem::file_release(unsafe { read((record.client as *mut u8).cast::<*mut c_void>()) }, record.file as *mut c_void);
    // SAFETY: record.file was allocated by drm_client_init with the DRM_FILE_SIZE layout
    // and every reference to it was unlinked above.
...
pub(super) extern "C" fn drm_client_init(dev: *mut c_void, client: *mut c_void, name: *const u8, funcs: *const c_void) -> i32 {
    ...
    // SAFETY: file_layout is the fixed DRM_FILE_SIZE layout; alloc_zeroed and the paired
    // dealloc on init failure both use this exact layout and file was null-checked before the free.
    ...
    // SAFETY: client was null-checked above, file is this call's own zeroed allocation
    // just initialized by gem::file_init, and dev was confirmed live under DEVICES.lock()
    // at the call above; every offset is a verified ABI field.
    ...
    // SAFETY: file/file_layout is this call's own alloc_zeroed allocation from above, not
    // yet linked into any ClientRecord, released here because no live device record matched.
...
pub(super) extern "C" fn drm_client_register(client: *mut c_void) {
    ...
    // SAFETY: client is checked non-null just above; offset 0 and DRM_CLIENT_FUNCS_OFF are
    // the device back-pointer and funcs table drm_client_init wrote.
    let (dev, hotplug) = unsafe { (read(client.cast::<*mut c_void>()), read(client.cast::<u8>().add(DRM_CLIENT_FUNCS_OFF).cast::<*const u8>())) }; if dev.is_null() { return; }
    // SAFETY: record is the live client found under the DEVICES lock so DRM_DEVICE_CLIENTLIST_OFF
    // and DRM_CLIENT_LIST_OFF are valid list nodes; hotplug, when non-null, is the driver funcs
    // table and its HOTPLUG_OFF slot is a driver-supplied fn(*mut c_void)->i32 per the DRM
    // client ABI, invoked after the lock is dropped.
...
pub(super) extern "C" fn drm_client_release(client: *mut c_void) {
    if client.is_null() { return; }
    // SAFETY: client is caller-checked non-null; offset 0 and DRM_CLIENT_FUNCS_OFF are the
    // device back-pointer and funcs table drm_client_init wrote, and the FREE_OFF slot is a
    // driver-supplied fn(*mut c_void) per the DRM client ABI.
...
pub(super) fn unregister_device(dev: *mut c_void) {
    // SAFETY: record.client is a live registered client stored in device.clients; its
    // list-link and funcs offsets are verified ABI fields under the DEVICES lock.
    ...
    // SAFETY: unregister is a nonzero function pointer read from the client's own funcs
    // table, whose ABI contract fixes this fn(*mut c_void) signature.
...
mod tests {
    ...
    fn client_lifecycle_uses_the_modeset_gate_and_device_client_list() {
        ...
        // SAFETY: client is this test's own stack buffer and both offsets lie within it after init.
        ...
        // DRM_DEVICE_CLIENTLIST_OFF is the verified device field.
        ...
        // SAFETY: client is this test's own stack buffer and both offsets lie within it after release.
    ...
    fn device_unregister_unlinks_then_calls_the_client_release_path() {
        ...
        // SAFETY: client is this test's own [u64; 12] stack buffer and DRM_CLIENT_FILE_OFF lies within it.
