...
fn default_group_bin_attr_link_and_release_paths_work() {
    ...
    assert_eq!(configfs_register_subsystem(&mut s), 0);
    let inode = tracefs::config_root().lookup_path("sample/child/blob").expect("bin attr");
    let mut buf = [0u8; 8];
    ...
    assert!(tracefs::config_root().lookup_path("sample/child_link").is_some());
    configfs_drop_link(&mut s.group.item, &mut child.item, b"child_link\0".as_ptr() as *const c_char);
    assert!(tracefs::config_root().lookup_path("sample/child_link").is_none());
