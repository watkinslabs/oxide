// Named finite-field DH groups used by the KPP provider.

pub(super) struct FfdheGroup {
    pub(super) prime: &'static [u8],
    pub(super) private_bits: usize,
}

const FFDHE2048: [u8; 256] = [
    ,
];

const FFDHE3072: [u8; 384] = [
    ,
];

const FFDHE4096: [u8; 512] = [
    ,
];

const FFDHE6144: [u8; 768] = [
    ,
];

const FFDHE8192: [u8; 1024] = [
    ,
];

static GROUPS: [(&[u8], FfdheGroup); 5] = [
    (b"ffdhe2048(dh)", FfdheGroup { prime: &FFDHE2048, private_bits: 256 }),
    (b"ffdhe3072(dh)", FfdheGroup { prime: &FFDHE3072, private_bits: 256 }),
    (b"ffdhe4096(dh)", FfdheGroup { prime: &FFDHE4096, private_bits: 512 }),
    (b"ffdhe6144(dh)", FfdheGroup { prime: &FFDHE6144, private_bits: 512 }),
    (b"ffdhe8192(dh)", FfdheGroup { prime: &FFDHE8192, private_bits: 512 }),
];

/// Resolve the supported finite-field group by its crypto algorithm name.
/// # C: O(groups)
pub(super) fn by_name(name: &[u8]) -> Option<&'static FfdheGroup> {
    GROUPS.iter().find(|(n, _)| *n == name).map(|(_, g)| g)
}
