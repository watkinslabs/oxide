...
fn work_delayed_work_tasklet_and_kthread_paths() {
    ...
    let mut w = LinuxWorkStruct { data: AtomicUsize::new(0), func: None };
    init_work(&mut w, Some(work_cb));
    assert_eq!(schedule_work(&mut w), 1);
    let mut dw = LinuxDelayedWork { work: LinuxWorkStruct { data: AtomicUsize::new(0), func: None }, delay: 0, oxide_id: 0 };
