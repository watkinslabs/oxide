...
pub(super) fn sleep_ns(ns: u64) {
    ...
    #[cfg(target_os = "oxide-kernel")]
    {
        ...
        // SAFETY: process context with no timer lock held. The scheduler owns
        // the expiry registration and cancels it on every signal or timeout
        // exit, so no stack address is published to a separate timer queue.
        let _ = unsafe { sched::live::wait_event(&wait, sched::WaitState::Interruptible,
            deadline, now_ns, || false) };
    }
