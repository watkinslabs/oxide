...
mod tests {
    ...
    fn clone_validation_rejects_a_routed_encoder_without_the_peer_bit() {
        ...
        assert_eq!(check_valid_clones(state.as_mut_ptr().cast(), crtc.as_mut_ptr().cast()), -LINUX_EINVAL); unsafe { write(first.as_mut_ptr().add(DRM_ENCODER_POSSIBLE_CLONES_OFF).cast::<u32>(), 3); } assert_eq!(check_valid_clones(state.as_mut_ptr().cast(), crtc.as_mut_ptr().cast()), 0); DEVICES.lock().clear();
    }
}
