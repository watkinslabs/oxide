...
const DRM_CONNECTOR_PROPERTIES_OFF: usize = 432;
...
pub(super) extern "C" fn drm_connector_attach_encoder(connector: *mut c_void, encoder: *mut c_void) -> i32 {
    ...
    // SAFETY: connector was null-checked above; the dev field lives at its ABI offset 0.
...
pub(super) extern "C" fn drm_connector_init(dev: *mut c_void, connector: *mut c_void, funcs: *const c_void, connector_type: i32) -> i32 {
    ...
    // SAFETY: the devices lookup just above confirms dev names a live mode_config
    // record, so config (derived from the same dev) has this ABI field populated.
    ...
    // SAFETY: connector was null-checked on entry and DRM_CONNECTOR_BASE_OFF stays
    // inside the caller-owned ABI-sized connector allocation drm_connector_init fills in.
    ...
    // SAFETY: name/layout are the allocation kms_name just returned; on this
    // failure path it has not yet been stored anywhere else, so this is its sole free.
    if object_result != 0 { unsafe { dealloc(name as *mut u8, layout); } return object_result; }
    let mut devices = DEVICES.lock();
    // SAFETY: same unstored name/layout allocation as above; this lookup-miss path
    // is still the only place that could free or retain it.
    ...
    // SAFETY: rec above confirms dev still names a live mode_config record under
    // the held DEVICES lock, so config's connector-count field is populated.
    let index = unsafe { *(config.add(MODE_CONFIG_NUM_CONNECTOR_OFF).cast::<i32>()) };
    // SAFETY: name/layout remain the unstored kms_name allocation; this bounds-check
    // failure is still the only path that could take ownership of it.
...
pub(super) extern "C" fn drm_connector_cleanup(connector: *mut c_void) {
    ...
    // SAFETY: entry.modes/probed_modes are mode allocations exclusively owned by
    // this now-removed connector entry; each was created with mode::mode_layout().
    ...
    // SAFETY: connector remains a live, non-null caller-owned allocation; base stays
    // inside its ABI-sized layout, same construction used throughout drm_connector_init.
