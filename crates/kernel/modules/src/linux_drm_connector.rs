...
pub(super) struct ConnectorRecord { pub(super) ptr: usize, name: usize, layout: Layout, modes: Vec<(usize, Layout)> }
...
pub(super) extern "C" fn drm_connector_cleanup(connector: *mut c_void) {
    ...
    for (mode, layout) in entry.modes { unsafe { dealloc(mode as *mut u8, layout); } }
