...
/// Install scheduler wait hooks for VFS sleeping-rwsem contention. # C: O(1)
pub(crate) fn set_vfs_rwsem_wait_hooks(park: ParkHook, schedule: ScheduleHook, wake: WakeHook) {
...
impl<C: LockClass> VfsRwsem<C> {
    ...
    pub fn write(&self) -> VfsRwsemWriteGuard<'_, C> {
...
pub struct VfsRwsemReadGuard<'a, C: LockClass> { lock: &'a VfsRwsem<C> }
impl<C: LockClass> Drop for VfsRwsemReadGuard<'_, C> { fn drop(&mut self) { self.lock.read_unlock(); } }
impl<C: LockClass> core::ops::Deref for VfsRwsemReadGuard<'_, C> {
    type Target = ();
...
impl<C: LockClass> core::ops::DerefMut for VfsRwsemWriteGuard<'_, C> {
...
mod tests {
    ...
    fn reset() {
        ...
        *lock.lock().unwrap() = WaitState { parked: 0, wake: false };
        set_vfs_rwsem_wait_hooks(park_reader, schedule, wake);
    }
    ...
    fn contended_reader_parks_then_wakes() {
        ...
        reset();
        let sem = Arc::new(VfsRwsem::<sync::Inode>::new());
    ...
    #[test]
    fn caller_selects_the_canonical_lockdep_class() {
        fn rank<C: sync::LockClass>(_: &VfsRwsem<C>) -> u16 { C::rank() }
        let inode = VfsRwsem::<sync::Inode>::new();
        let umount = VfsRwsem::<sync::Superblock>::new();
        assert_eq!(rank(&inode), sync::Inode::rank());
        assert_eq!(rank(&umount), sync::Superblock::rank());
        assert_ne!(rank(&inode), rank(&umount));
    }
}
