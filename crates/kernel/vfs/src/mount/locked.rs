...
/// against unmount/move. Skips mounts queued on an expire list (Linux's
/// `list_empty(&p->mnt_expire)` test — an auto-expiring submount must stay
/// reapable). # C: O(N_ns_mounts)
pub(super) fn lock_mnt_ns(ns: u64) {
    let root = root_mount_id(ns);
    for m in mounts_in_ns(ns) {
        let opts = m.flags();
        // "Don't allow unprivileged users to change mount flags" — atime is
        // frozen whatever it is; the rest only when currently protective.
        ...
        if Some(m.mnt_id) != root && !expiry::on_any_expire_list(m.mnt_id) { add |= MNT_LOCKED; }
