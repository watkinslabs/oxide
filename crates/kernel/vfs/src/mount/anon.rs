...
{
    // The anonymous namespace is owned by the caller's user namespace, as the
    // reference's `alloc_mnt_ns(current->nsproxy->mnt_ns->user_ns, true)` is.
    ...
    // The mount OWNS its anonymous namespace: nothing else refers to it, so
    // without this the namespace dies here and reaps the mount with it.
