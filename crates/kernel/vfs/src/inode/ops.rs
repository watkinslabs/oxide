...
impl Inode {
    ...
    /// `i_op->mkdir` with the VFS negative-dentry proof already established.
    /// # C: backend-dependent
    pub fn mkdir_unchecked(&self, name: &str, mode: u32, ctx: &CreateCtx) -> KResult<InodeRef> { self.i_op.mkdir_unchecked(self, name, mode, ctx) }
