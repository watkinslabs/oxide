...
/// and install it as `sb->s_root`. # C: O(1)
pub fn d_make_root(inode: InodeRef, sb: &Arc<SuperBlock>) -> Arc<Dentry> {
    let root = Dentry::new_root_in_sb(inode, sb);
...
pub fn d_add(parent: &Arc<Dentry>, name: &str, inode: InodeRef) -> Arc<Dentry> {
    let child = Dentry::new_child(parent, name, Some(inode));
...
/// a stale positive dentry isn't reused after unlink/rmdir/rename.
/// # C: O(log N_children)
