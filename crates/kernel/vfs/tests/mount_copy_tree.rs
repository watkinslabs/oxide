...
fn guard(ns: u64) -> MutexGuard<'static, ()> {
    ...
    TEST_NS.store(ns, Ordering::Release);
...
fn copy_tree_shared_peers() {
    let _g = guard(0xC07_03);
    common::register("/", facfs(0x1)).expect("root");
    common::register("/pa", facfs(0xA)).expect("pa");
    ...
    common::register("/pb", facfs(0xB)).expect("pb");
    common::join_peer_group("/pb", parent_pg);            // peer of pa

    common::register("/pa/x", facfs(0x11)).expect("under pa");
...
fn copy_tree_slave_no_backprop() {
    ...
    common::register("/qs/x/y", facfs(0x22)).expect("under slave copy");
...
fn copy_tree_unbindable_excluded() {
    let _g = guard(0xC07_05);
    common::register("/", facfs(0x1)).expect("root");
    common::register("/usrc", facfs(0xA)).expect("usrc");
    common::register("/usrc/keep", facfs(0xA1)).expect("keep");
    common::register("/usrc/skip", facfs(0xA2)).expect("skip");
...
fn copy_tree_dmounted_refcount() {
    let _g = guard(0xC07_06);
    common::register("/", facfs(0x1)).expect("root");
    common::register("/rsrc", facfs(0xA)).expect("rsrc");
    common::register("/rsrc/a", facfs(0xA1)).expect("a");
    ...
    assert!(common::mount_at_path_exact("/rdst/a").is_none(), "clone detached from the target parent");
