...
use vfs::inode::Inode;
use vfs::{Dentry, File, FileType, InodeBuilder, InodeRef, KResult, OpenFlags,
...
/// `inode.set_size` tracks the high-water length (so `O_APPEND` sees real
/// growth via `inode.size()`).
struct MemData { data: Mutex<Vec<u8>> }

struct MemFileOps;
impl vfs::FileOps for MemFileOps {
...
fn mk_memfile(initial: &[u8]) -> InodeRef {
    InodeBuilder::new(0xA9, mk_mode(FileType::Regular, 0o644), default_inode_ops(), Arc::new(MemFileOps))
...
fn append_file(ino: &InodeRef, mode: OpenFlags) -> Arc<File> {
...
fn append_writes_at_eof_not_at_cursor() {
    let ino = mk_memfile(b"HEAD");                 // size = 4
    ...
    assert_eq!(mem_bytes(&ino), b"HEADTAIL", "append must not overwrite the file head");
...
fn repeated_appends_keep_extending() {
    ...
    f.write(b"cc").unwrap();
    assert_eq!(mem_bytes(&ino), b"aabbcc", "each append starts at the current size");
    assert_eq!(f.pos(), 6);
}
...
fn rdwr_append_writes_at_eof_reads_at_cursor() {
    ...
    f.set_pos(1);
    f.write(b"XY").unwrap();
    assert_eq!(mem_bytes(&ino), b"0123XY", "append ignores the rewound cursor");
}
