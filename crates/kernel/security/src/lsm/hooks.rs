...
pub const LSM_ID_CAPABILITY: u64 = lsm_framework::uapi::LSM_ID_CAPABILITY;
...
pub type TaskSetNiceHook = fn(&sched::Task, &sched::Task, i32) -> Result<(), i64>;
pub type TaskSetSchedulerHook = fn(&sched::Task, &sched::Task) -> Result<(), i64>;
...
static TASK_HOOKS: Spinlock<task_dispatch::TaskHooks, LockClass> =
    Spinlock::new(task_dispatch::TaskHooks::new());

fn position(module: u64) -> Option<u16> {
    lsm_framework::registry::position(module)
}

fn hook_identity(module: u64) -> Option<(lsm_framework::LsmId, u16)> {
    let name = lsm_framework::uapi::id_name(module)?;
    Some((lsm_framework::LsmId { name, id: module }, position(module)?))
}
...
/// Register one task-nice provider. # C: O(providers)
pub fn register_task_setnice_for(module: u64, hook: TaskSetNiceHook) {
    let Some((lsm, at)) = hook_identity(module) else { return };
    TASK_HOOKS.lock().register_setnice(lsm, at, hook);
}

/// Register one task-scheduler provider. # C: O(providers)
pub fn register_task_setscheduler_for(module: u64, hook: TaskSetSchedulerHook) {
    let Some((lsm, at)) = hook_identity(module) else { return };
    TASK_HOOKS.lock().register_setscheduler(lsm, at, hook);
}
...
/// Mediate a task nice-value change through the framework's module order.
/// # C: O(userns depth + providers)
pub fn task_setnice(caller: &sched::Task, target: &sched::Task, nice: i32) -> Result<(), i64> {
    task_dispatch::setnice(&TASK_HOOKS, caller, target, nice)
}

/// Mediate a task scheduler-policy or parameter change.
/// # C: O(userns depth + providers)
pub fn task_setscheduler(caller: &sched::Task, target: &sched::Task) -> Result<(), i64> {
    task_dispatch::setscheduler(&TASK_HOOKS, caller, target)
}
