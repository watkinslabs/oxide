...
pub fn map_create_check(a: &Attr, caps: Caps, unpriv_disabled: bool) -> Result<MapCreate, Errno> {
    ...
    // `ops = bpf_map_types[map_type]; if (!ops) return -EINVAL;` — the
    // only map type with a backing implementation here is HASH.
    if m.map_type != uapi::map_type::HASH { return Err(Errno::Einval); }
    htab_map_alloc_check(&m, caps)?;
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub struct ProgLoad { pub prog_type: u32, pub insn_cnt: u32, pub insns: u64, pub license: u64 }
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub struct LinkCreate { pub prog_fd: u32, pub target_fd: u32, pub attach_type: u32, pub target_btf_id: u32 }

/// `link_create()` — kernel/bpf/syscall.c: `CHECK_ATTR` first, then the
/// attach-type → prog-type map, then the per-type attacher. Only
/// `BPF_LSM_MAC` reaches an implementation here. # C: O(ATTR_SIZE)
pub fn link_create_check(a: &Attr) -> Result<LinkCreate, Errno> {
    ...
    let l = LinkCreate {
        ...
        attach_type: a.u32_at(o::ATTACH_TYPE), target_btf_id: a.u32_at(o::TARGET_BTF_ID),
    };
    if attach_type_to_prog_type(l.attach_type) == uapi::prog_type::UNSPEC { return Err(Errno::Einval); }
    if l.attach_type != uapi::attach_type::LSM_MAC { return Err(Errno::Einval); }
    if a.u32_at(o::FLAGS) != 0 || l.target_fd != 0 { return Err(Errno::Einval); }
    Ok(l)
}
