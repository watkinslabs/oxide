...
/// part of it. `perf` answers `perf_get_event()` and `event->prog` for
/// TASK_FD_QUERY: the perf subsystem sits above this crate, so both arrive
/// from the syscall shim rather than from a second copy of perf's state here.
/// # C: per command
