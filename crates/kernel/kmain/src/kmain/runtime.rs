...
fn init_runtime_subsystems() {
    ...
    // Software-counter sample emission (`perf_sw_event` -> PERF_RECORD_SAMPLE)
    // lives in `fs::perf`, above the counter sites that charge it; the hook is
    // how the fault path reaches it.
    fs::perf::emit::init();
