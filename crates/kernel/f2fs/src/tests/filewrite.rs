...
#[test]
fn writing_to_a_compressed_inode_goes_through_the_cluster_writer() {
    // The general path writes a block at a time, which a compressed file has
    // no such thing as: its blocks are one image per cluster. This is the
    // dispatch, so what it proves is that the bytes come back — not how.
    ...
    .unwrap();
    let data: Vec<u8> = (0..4 * BLKSIZE).map(|i| ((i / 64) % 11) as u8).collect();
    assert_eq!(v.write_file(ino, 0, &data).unwrap(), data.len());
    let v = remount(v);
    assert_eq!(whole(&v, ino), data);
    // It really went through the cluster writer: the head slot carries the
    // sentinel, which the block-at-a-time path never writes.
    let inode = v.read_inode(ino).unwrap();
    let g = v.geometry(&inode).unwrap();
    let addrs = v.cluster_addrs(&inode, ino, &g, 0).unwrap();
    assert_eq!(addrs[0], crate::uapi::COMPRESS_ADDR);
}
