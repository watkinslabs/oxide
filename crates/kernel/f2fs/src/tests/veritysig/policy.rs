...
/// Serialise the tests that touch the machine-wide verity state.
///
/// The keyring and the require knob are process-wide by design — that is the
/// whole point of them — so two tests driving them at once would see each
/// other. Every other test pins its own policy and needs no lock.
struct VerityTestLockClass;
impl sync::LockClass for VerityTestLockClass {
    fn rank() -> u16 { 34 }
    fn name() -> &'static str { "VerityTestLockClass" }
}
static VERITY_TEST_LOCK: sync::Spinlock<(), VerityTestLockClass> = sync::Spinlock::new(());
fn global_verity_lock() -> sync::Guard<'static, (), VerityTestLockClass> {
    VERITY_TEST_LOCK.lock()
}
...
fn a_signed_file_and_an_empty_keyring_is_refused_rather_than_waved_through() {
    ...
    let p = Policy::trusting_nothing();
    assert!(p.store_is_empty());
...
fn a_certificate_that_is_not_one_does_not_join_the_keyring() {
    let mut p = Policy::trusting_nothing();
    assert_eq!(p.trust(b"not a certificate"), Err(VerityError::MalformedSignature));
    assert!(p.store_is_empty());
}

#[test]
fn the_machines_knob_is_what_a_mount_that_states_nothing_answers_with() {
    // The require flag is ONE machine-wide boolean in the reference, reached
    // through `/proc/sys/fs/verity/require_signatures`. A mount that states
    // nothing of its own follows it, and follows it at VERIFY time — a mount
    // that snapshotted it would keep answering with the value the machine had
    // when it was mounted.
    //
    // Serialised against every other test that touches the global, and it
    // restores what it found: the store and the knob are process-wide.
    let _g = global_verity_lock();
    let was = vfs::verity_keys::require_signatures();
    let p = Policy::new();

    vfs::verity_keys::set_require_signatures(false);
    assert!(!p.require(), "the mount did not follow the machine");
    assert_eq!(signature::verify(&p, HASH_ALG_SHA256, &digest(), b""), Ok(()),
               "an unsigned file was refused where the machine allows it");

    vfs::verity_keys::set_require_signatures(true);
    assert!(p.require(), "the mount snapshotted the knob instead of reading it");
    assert_eq!(signature::verify(&p, HASH_ALG_SHA256, &digest(), b""),
               Err(VerityError::SignatureRequired));

    vfs::verity_keys::set_require_signatures(was);
}

#[test]
fn a_mount_that_states_its_own_answer_ignores_the_machines() {
    // What keeps a test that pins its own policy safe beside one that does
    // not: an explicit answer is never overridden by the global.
    let _g = global_verity_lock();
    let was = vfs::verity_keys::require_signatures();
    let mut p = Policy::new();
    p.set_require(false);
    vfs::verity_keys::set_require_signatures(true);
    assert!(!p.require(), "an explicit answer was overridden by the machine's");
    vfs::verity_keys::set_require_signatures(was);
}
