...
fn a_budget_larger_than_both_caches_still_only_takes_a_quarter_each() {
    ...
    assert_eq!(b.age + b.read, 500_000, "half the budget stayed with the id cache");
}
...
fn a_budget_too_small_to_divide_takes_nothing_from_the_extent_caches() {
    ...
    for nr in 0..4 {
        assert_eq!(split(nr), Budget { age: 0, read: 0 }, "nr = {nr}");
    }
    assert_eq!(split(4), Budget { age: 1, read: 1 });
}
...
fn both_extent_caches_count_toward_what_could_be_freed() {
    assert_eq!(reclaimable(7, 11, 0), 18);
}
...
fn free_node_ids_count_only_above_the_working_set() {
    assert_eq!(reclaimable(0, 0, MAX_FREE_NIDS), 0);
    assert_eq!(reclaimable(0, 0, MAX_FREE_NIDS - 1), 0);
    assert_eq!(reclaimable(0, 0, MAX_FREE_NIDS + 9), 9);
}

#[test]
fn a_mount_holding_nothing_offers_nothing() {
    assert_eq!(reclaimable(0, 0, 0), 0);
}
