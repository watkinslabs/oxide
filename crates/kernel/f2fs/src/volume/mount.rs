...
impl<S: SectorSource> Volume<S> {
    ...
    pub fn mount_devices(
        ...
        let mut vol = Self::mount_devices_prepared(source, opts, want_write, reports)?;
        vol.finish_mount()?;
        Ok(vol)
    }


    /// Replay the on-disk state after the complete mount record exists.
    #[inline(never)]
    fn finish_mount(&mut self) -> Result<(), Errno> {
        ...
        // covered.
        crate::fault::apply(&self.fault, &self.opts.fault);
        ...
        // number.
        let mut outcome = self.recover_orphans();
        if outcome.is_ok() {
            self.begin_recovery();
            ...
            outcome = self.recover_at_mount().map(|r| {
                crate::volume::recover::report::emit(
                    crate::volume::recover::report::announce_for(r));
            });
            self.finish_recovery(outcome.is_ok());
        }
        self.end_repair_write();
        outcome
    }
