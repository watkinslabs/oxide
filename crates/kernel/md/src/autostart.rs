...
mod tests {
    ...
    fn boot_hook_assembles_raid_marked_partitions_and_holds_parents() {
        ...
        assert_eq!(stopped.buffer, vec![0x44; 512]);
        assert!(block::registry::by_name(&name).is_none(), "final stop unpublishes the MD node");
        assert!(!crate::is_md_device(dev_t));
        assert_eq!(crate::array_info(dev_t), None);
        assert_eq!(crate::stop_array(dev_t), Err(BlockError::Enxio), "a stopped array has no live control target");
        let mut after_stop = BlockRequest::new_flush();
        assert_eq!(array.dev.submit_sync(&mut after_stop), Err(BlockError::Eio));
        assert!(block::registry::close_disk(&array), "the control opener remains releasable after unpublish");
