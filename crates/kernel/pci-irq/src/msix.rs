...
use crate::{BarMapping, Binding, Mode, single_binding};
...
impl MsixGroup {
    ...
    pub fn bind(&mut self, entry: MsixEntry, action: arch_irq::DeviceAction,
        ...
        if !self.admits(entry) { return None; }
        #[cfg(target_arch = "x86_64")]
    ...
    /// Allocate and program one device-relative MSI-X vector with binding-owned
    /// hard-IRQ context, matching a driver's per-queue instance ownership.
    /// # C: O(N_vectors)
    pub fn bind_context(&mut self, entry: MsixEntry, action: arch_irq::DeviceAction,
        handler: fn(usize), arg: usize) -> Option<u32> {
        if !self.admits(entry) { return None; }
        #[cfg(target_arch = "x86_64")]
        { bind_msix_context_with(&hal_x86_64::pci::EcamPci::from_published()?, self, entry, action, handler, arg) }
        #[cfg(target_arch = "aarch64")]
        { bind_msix_context_with(&hal_aarch64::pci::EcamPci::from_published()?, self, entry, action, handler, arg) }
        #[cfg(not(any(target_arch = "x86_64", target_arch = "aarch64")))]
        { let _ = (entry, action, handler, arg); None }
    }

    fn admits(&self, entry: MsixEntry) -> bool {
        entry.bar == self.table_bar && entry.vector < self.table_size
            && !self.vectors.iter().any(|vector| vector.entry_va == entry.entry_va)
    }
...
fn bind_msix_context_with<R: pci::ConfigSpaceReader>(_: &R, group: &mut MsixGroup,
    entry: MsixEntry, action: arch_irq::DeviceAction, handler: fn(usize), arg: usize) -> Option<u32> {
    let message = arch_irq::alloc_pci_msi(group.bdf, entry.vector as u32)?;
...
pub(crate) fn request_msix<R: pci::ConfigSpaceReader>(r: &R, bdf: pci::Bdf, msi_cap: Option<u8>,
...
pub(crate) fn msix_entry_va(cap: pci::MsixCap, table: BarMapping) -> Option<u64> {
...
pub(crate) fn write_msix_mask(entry_va: u64) {
...
mod tests {
    ...
    fn cap(bar: u8, off: u32) -> pci::MsixCap {
        pci::MsixCap { enabled: false, function_mask: false, table_size: 1, table_bir: bar, table_offset: off, pba_bir: 0, pba_offset: 0 }
    }
