...
pub const fn xterm_256_rgb(idx: u32) -> u32 {
    rgb(xterm_256(idx))
}

/// One 6x6x6 cube axis step to an 8-bit channel. # C: O(1).
const fn cube_level(x: u8) -> u8 { if x == 0 { 0 } else { 55 + 40 * x } }

/// Resolve an SGR 256-color index to [r,g,b] per xterm (see `xterm_256_rgb`).
/// # C: O(1).
pub const fn xterm_256(idx: u32) -> [u8; 3] {
    if idx < 16 {
    ...
    let idx = if idx > 255 { 255 } else { idx };
