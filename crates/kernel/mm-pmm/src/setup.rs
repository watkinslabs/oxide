...
// `page_lock` owns the blocking per-page lock slow path and bounded wait table.
// `rmap` owns the tagged PageMeta mapping owner.
...
use crate::{BITMAP_SLOTS, Error as PmmError, PAGEBLOCK_TYPE_SLOT, PageBacking, PcpStorage, Pmm, UsableRegion, PCP_BITMAP_SLOT};
...
pub use boot_init::{HhdmBacking, MAX_REGIONS, SetupError, direct_map_base, init_from_boot_info, pmm_static, usable_regions};
pub use fwmap::{firmware_regions, FirmwareRegion, MAX_FIRMWARE_REGIONS};
pub use frame_alloc::{alloc_one_frame, alloc_one_frame_below, alloc_object_frame, alloc_movable_object_frame, alloc_raw_frame, alloc_raw_frame_below, frame_ptr, migrate_movable_object_frame, release_movable_object_frame, release_object_frame};
