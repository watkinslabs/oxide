...
fn assert_quota_rollback_dirty_error(err: ext4::MountError) {
    assert!(matches!(err, ext4::MountError::Quota(VfsError::Eio)));
}

#[test]
fn inline_append_inode_failure_quota_rollback_dirty_failure_surfaces_quota_error() {
    common::boot_hosted_pmm();
    let (m, sb) = mount_result(seeded_quota_disk()).expect("rw mount with hidden quota");
    let qid = Kqid::project(0);
    let inode = m.state().create_at(b"/inline-rollback-dirty-fail.txt", 0o644).expect("create file");
    ...
    let err = m.state().mount.append_block(ino, &vec![0x51; bs]).expect_err("rollback quota dirty fails");

    assert_quota_rollback_dirty_error(err);
    ...
    assert_eq!(after_q.dqb_curspace, before_q.dqb_curspace + bs as u64);
}

#[test]
fn external_insert_inode_failure_quota_rollback_dirty_failure_preserves_tree() {
    common::boot_hosted_pmm();
    let (m, sb) = mount_result(seeded_quota_disk()).expect("rw mount with hidden quota");
    let qid = Kqid::project(0);
    let inode = m.state().create_at(b"/external-rollback-dirty-fail.txt", 0o644).expect("create file");
    let ino = inode.ino() as u32;
    let bs = m.state().mount.sb.block_size as u64;

    for lb in [0u64, 2, 4, 6, 8] {
        m.state().mount.write_at(ino, lb * bs, &[0x61]).expect("seed external tree");
    }
    let before_free = m.state().mount.state_free_blocks();
    let before_raw = m.state().mount.read_inode(ino).expect("raw before");
    ...
    m.state().mount.fail_next_inode_write_for_tests();
    m.state().mount.fail_quota_mark_dirty_after_for_tests(1);
    let err = m.state().mount.append_block(ino, &vec![0x62; bs as usize]).expect_err("rollback quota dirty fails");
    ...
    let after_q = vfs::quota_getquota(&sb, qid).expect("quota after");
    assert_eq!(after_q.dqb_curspace, before_q.dqb_curspace + bs);
}
