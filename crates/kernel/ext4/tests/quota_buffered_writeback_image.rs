...
fn mount_result(disk: Arc<dyn BlockDevice>) -> vfs::KResult<(Arc<ext4::rootfs::Ext4Mount>, Arc<SuperBlock>)> {
    ...
    let root = fs.root();
    let sb = common::realize_sb_result(fs, root, 0xE471_B6F0, String::from("ext4"))?;
    Ok((m, sb))
}
...
#[test]
fn buffered_writeback_charges_project_quota_on_success() {
    common::boot_hosted_pmm();
    let (m, sb) = mount_result(seeded_quota_disk()).expect("rw mount with hidden quota");
    let qid = Kqid::project(0);
    let inode = m.state().create_at(b"/buffered-writeback-success.txt", 0o644).expect("create file");
    let ino = inode.ino() as u32;
    let bs = m.state().mount.sb.block_size as usize;
    let file = m.state().wrap_file(ino).expect("wrap file");

    assert_eq!(file.write(0, &vec![0x41; bs]).expect("buffered write"), bs);
    file.i_mapping().expect("mapping").writeback().expect("writeback");

    let raw = m.state().mount.read_inode(ino).expect("raw after writeback");
    assert_eq!(raw.size, bs as u64);
    assert_eq!(quota_space_matches_i_blocks(&m, &sb, ino, qid), raw.i_blocks as u64 * 512);
}

#[test]
fn buffered_writeback_edquot_leaves_disk_and_quota_unchanged() {
    common::boot_hosted_pmm();
    let (m, sb) = mount_result(seeded_quota_disk()).expect("rw mount with hidden quota");
    let qid = Kqid::project(0);
    let inode = m.state().create_at(b"/buffered-writeback-edquot.txt", 0o644).expect("create file");
    let ino = inode.ino() as u32;
    let bs = m.state().mount.sb.block_size as usize;
    ...
    vfs::quota_enable_limits(&sb, vfs::QuotaType::Project).expect("enable limits");

    let file = m.state().wrap_file(ino).expect("wrap file");
    assert_eq!(file.write(0, &vec![0x42; bs]).expect("buffered write"), bs);
    assert_eq!(file.i_mapping().expect("mapping").writeback(), Err(()));
...
#[test]
fn buffered_writeback_data_failure_rolls_project_quota_back_and_redirties() {
    common::boot_hosted_pmm();
    let (m, sb) = mount_result(seeded_quota_disk()).expect("rw mount with hidden quota");
    let qid = Kqid::project(0);
    let inode = m.state().create_at(b"/buffered-writeback-data-fail.txt", 0o644).expect("create file");
    let ino = inode.ino() as u32;
    let bs = m.state().mount.sb.block_size as usize;
    let before_free = m.state().mount.state_free_blocks();
    let before_raw = m.state().mount.read_inode(ino).expect("raw before");
    ...
    assert_eq!(file.write(0, &vec![0x43; bs]).expect("buffered write"), bs);
    m.state().mount.fail_next_data_write_for_tests();
    assert_eq!(file.i_mapping().expect("mapping").writeback(), Err(()));
    let failed_raw = m.state().mount.read_inode(ino).expect("raw after failed writeback");
    assert_eq!(m.state().mount.state_free_blocks(), before_free);
    assert_eq!(failed_raw.i_blocks, before_raw.i_blocks);
    assert_eq!(failed_raw.size, before_raw.size);
    assert_eq!(vfs::quota_getquota(&sb, qid).expect("quota after failure").dqb_curspace, before_q.dqb_curspace);

    file.i_mapping().expect("mapping retry").writeback().expect("retry writeback");
    assert!(m.state().mount.state_free_blocks() < before_free);
    assert!(quota_space_matches_i_blocks(&m, &sb, ino, qid) > before_q.dqb_curspace);
}

#[test]
fn buffered_writeback_quota_rollback_dirty_failure_surfaces_quota_error() {
    common::boot_hosted_pmm();
    let (m, sb) = mount_result(seeded_quota_disk()).expect("rw mount with hidden quota");
    let qid = Kqid::project(0);
    let inode = m.state().create_at(b"/buffered-writeback-rollback-dirty.txt", 0o644).expect("create file");
    let ino = inode.ino() as u32;
    let bs = m.state().mount.sb.block_size as usize;
    let before_free = m.state().mount.state_free_blocks();
    let before_raw = m.state().mount.read_inode(ino).expect("raw before");
    ...
    assert_eq!(file.write(0, &vec![0x44; bs]).expect("buffered write"), bs);
    m.state().mount.fail_next_data_write_for_tests();
    ...
    assert_eq!(after_raw.i_blocks, before_raw.i_blocks);
    assert_eq!(after_raw.size, before_raw.size);
    assert_eq!(vfs::quota_getquota(&sb, qid).expect("quota after").dqb_curspace,
        before_q.dqb_curspace + bs as u64);
}
