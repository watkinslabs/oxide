...
impl Mount {
    ...
    pub fn adjust_nlink(&self, ino: u32, delta: i32) -> Result<u16, MountError> {
        self.run_journaled(|m| m.adjust_nlink_in_transaction(ino, delta))
    }

    /// Adjust a link count inside an already-open journal transaction. # C: O(1) I/O
    pub(crate) fn adjust_nlink_in_transaction(&self, ino: u32, delta: i32) -> Result<u16, MountError> {
