...
/// The OWNER_DIED cmpxchg retry is unbounded upstream. It converges in
/// practice because each pass re-reads the word and either the owner check
/// stops matching or OWNER_DIED lands, and the racing writer is userspace
/// code for a thread that is already dying.
///
...
/// Offsets in `struct robust_list_head` — ABI-fixed field layout the kernel
/// reads directly out of user memory.
const HEAD_LIST_NEXT: u64 = 0;
const HEAD_FUTEX_OFFSET: u64 = 8;
const HEAD_LIST_OP_PENDING: u64 = 16;

/// On thread death, walk the user `robust_list_head` this thread registered
/// via `set_robust_list` and, for every robust mutex it still owns, mark
...
pub fn exit_robust_list(head_uaddr: u64, owner_tid: u32) {
    ...
    while cur.addr != head_uaddr && limit > 0 {
        // The NEXT entry is fetched BEFORE handling this one — the death
        // handler can fault or userspace can recycle the entry, so the link
        ...
        if cur.addr != pending.addr {
            ...
            // A failed handler aborts the walk.
            if handle_futex_death(uaddr, owner_tid, cur.pi(), DeathSite::List).is_err() { return; }
        }
        // The deferred fetch error is only acted on after the current entry
        // has been handled.
...
/// Read a `robust_list` pointer and split bit 0 off it. Bit 0 is
/// `FUTEX_ROBUST_MOD_PI`, the tag glibc sets on a PI robust mutex — it must be
/// masked off before treating the value as an address, not rejected. A prior
/// version of this code demanded every fetched pointer be 8-aligned and bailed
/// otherwise, so a single PI-tagged entry ABORTED THE WHOLE WALK and every
/// robust mutex after it stayed owned by a dead thread.
/// # C: O(1)
    ...
    -> Result<(), ()>
{
    ...
    loop {
        ...
        match death_verdict(uval & FUTEX_TID_MASK, owner_tid, pi, site) {
            ...
            // — setting OWNER_DIED here would create inconsistent state for
            // userspace's owner-died handling.
            DeathAction::WakeOnly => { wake_one(futex_uaddr); return Ok(()); }
            // Not ours — skip, but keep walking.
            DeathAction::Skip => return Ok(()),
            DeathAction::SetOwnerDied => {}
        }
        if uval & FUTEX_OWNER_DIED != 0 {
            // Already flagged. Still re-wake for the rare but possible case of
            // recursive thread-death, but the word already holds the value
            // the cmpxchg would write.
        ...
        if nval == uval {
            ...
            // would bypass the ownership handoff.
            if !pi && uval & FUTEX_WAITERS != 0 { wake_one(futex_uaddr); }
            return Ok(());
        }
        // userspace moved the word under us — re-read.
        tries += 1;
