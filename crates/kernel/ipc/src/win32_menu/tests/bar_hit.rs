...
fn metrics() -> BarMetrics { BarMetrics { char_width: CHAR_WIDTH, char_height: CHAR_HEIGHT, bar_height: BAR_HEIGHT } }
...
fn a_point_over_a_bar_item_names_that_item() {
    ...
    for position in 0..3u32 {
        let rect = menus.bar_item_rect(menu, position as usize, window, CHAR_WIDTH, CHAR_HEIGHT, BAR_HEIGHT).unwrap();
...
fn the_band_right_of_the_last_item_is_the_bars_own_background() {
    let (menus, menu, window) = bar();
    let last = menus.bar_item_rect(menu, 2, window, CHAR_WIDTH, CHAR_HEIGHT, BAR_HEIGHT).unwrap();
...
fn a_point_below_the_band_or_outside_the_window_names_nothing() {
    let (menus, menu, window) = bar();
    let bar_rect = menus.bar_rect(menu, window, CHAR_WIDTH, CHAR_HEIGHT, BAR_HEIGHT).unwrap();
...
fn the_hit_test_names_the_item_the_drawing_plan_highlights() {
    ...
    let drawn = menus.bar_item_rect(menu, 1, origin, CHAR_WIDTH, CHAR_HEIGHT, BAR_HEIGHT).unwrap();
