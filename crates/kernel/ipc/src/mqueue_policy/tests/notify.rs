...
fn the_three_linux_notify_modes_are_all_accepted() {
    // The pre-audit implementation rejected SIGEV_NONE with EINVAL; Linux
    // registers it (`ipc/mqueue.c:1346-1348`).
...
fn signal_zero_is_accepted_and_delivers_nothing() {
    // `do_mq_notify` accepts it (`valid_signal(0)`), `__do_notify` skips the
    // send (`mqueue.c:793-795`).
...
fn a_null_notification_only_clears_the_callers_own_registration() {
    // `mqueue.c:1336-1341`: a foreign deregister is a silent success that must
