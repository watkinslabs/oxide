...
impl GdiManager {
    ...
    pub fn acquire_dc_lease(&mut self, mut request: DcLeaseRequest) -> Result<u32, GdiError> {
        ...
        let (handle, index) = if let Some(index) = reuse { (self.dcs[index].0, index) } else {
            ...
            self.dcs.push((handle, DeviceContext { width: request.width, height: request.height, attr, font: Some(DEFAULT_DC_FONT_HANDLE), brush: None, dc_brush_color: 0xffffff, pen: super::DEFAULT_DC_PEN_HANDLE, dc_pen_color: 0, text: TextAttributes::default(), clip: None, meta_clip: None, paths: Default::default(), paint_clip: None, pixels: Vec::new(), lease: None, pending_output:Default::default(), saved: Vec::new(), palette: None, bitmap: None, memory: false, justification: (0, 0) }));
