...
mod pattern;
pub use pattern::{SharedDcColors, Fill, fill, hatch_rows, HS_DIAGCROSS, HS_API_MAX, OPAQUE};
...
pub enum BrushStyle { Solid(u32), Hollow, Pattern, Hatch { style: u32, color: u32 } }
...
impl GdiManager {
    ...
    /// A hatch style past the last drawn one still names a brush inside the
    /// interface range: it becomes a solid brush of the requested colour.
    /// Anything beyond that range names no brush at all. # C: O(1)
    pub fn create_hatch_brush(&mut self, style: u32, color: u32) -> Result<u32, GdiError> {
        if style > HS_DIAGCROSS {
            if style >= HS_API_MAX { return Err(GdiError::InvalidDimensions); }
            return self.create_solid_brush(color);
        }
        self.create_brush(BrushStyle::Hatch { style, color: color & RGB_MASK })
    }

    /// The brush a device context paints with, defaulting to the stock white
    /// brush the reference selects into a new context. # C: O(DCs)
    pub fn selected_brush_handle(&self, dc: u32) -> Result<u32, GdiError> {
        let state = &self.dcs.iter().find(|(id, _)| *id == dc).ok_or(GdiError::NoSuchObject)?.1;
        state.ensure_active()?;
        Ok(state.brush.unwrap_or(self.stock_object(WHITE_BRUSH).ok_or(GdiError::NoSuchObject)?.handle))
    }
    ...
    fn pat_blt_color(&mut self, dc: u32, x: i32, y: i32, width: i32, height: i32, rop: u32, shared: Option<SharedDcColors>) -> Result<(), GdiError> {
        ...
        if super::blt::rop_uses_source(rop) { return Err(GdiError::InvalidDimensions); }
        let state = &self.dcs.iter().find(|(id, _)| *id == dc).ok_or(GdiError::NoSuchObject)?.1;
        state.ensure_active()?;
        let colors = shared.unwrap_or(SharedDcColors { brush: state.dc_brush_color, text: state.text.foreground,
            background: state.text.background, background_mode: state.text.background_mode });
        ...
        let (left, right) = super::blt::signed_span(x, width, clip.left, clip.right);
        let (top, bottom) = super::blt::signed_span(y, height, clip.top, clip.bottom);
        for row in top..bottom { for col in left..right {
            let Some(cell) = fill.color(col, row) else { continue; };
            target.update(col, row, |old| super::blt::rop3(table, cell, 0, old));
        } }
        Ok(())
    }
}
