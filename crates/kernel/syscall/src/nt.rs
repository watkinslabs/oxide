...
/// Opaque identity exported as Wine's `__wine_unixlib_handle`.
pub const WINE_UNIXLIB_HANDLE: u64 = 0x4f58_4944_455f_554e;
...
pub enum NtService {
    ...
    RtlGetSystemTimePrecise = 405,
    RtlGetThreadErrorMode = 406,
    RtlGetThreadPreferredUILanguages = 407,
    RtlGetUserPreferredUILanguages = 408,
    RtlGetVersion = 409,
    RtlIdentifierAuthoritySid = 410,
    RtlIdnToAscii = 411,
    RtlIdnToNameprepUnicode = 412,
    RtlIdnToUnicode = 413,
    RtlImpersonateSelf = 414,
    RtlInitBarrier = 415,
    RtlInitCodePageTable = 416,
    RtlInitializeExtendedContext2 = 417,
    RtlInitializeSid = 418,
    RtlIsDosDeviceNameU = 419,
    RtlIsNormalizedString = 420,
    RtlIsProcessorFeaturePresent = 421,
    RtlLengthRequiredSid = 422,
    RtlLengthSid = 423,
    RtlLocalTimeToSystemTime = 424,
    RtlLocateExtendedFeature = 425,
    RtlMapGenericMask = 426,
    RtlNewSecurityObject = 427,
    RtlNewSecurityObjectEx = 428,
    RtlNewSecurityObjectWithMultipleInheritance = 429,
    ...
    RtlSetHeapInformation = 455,
    RtlSetProcessPreferredUILanguages = 456,
    ...
    RtlWow64GetProcessMachines = 479,
    RtlWow64GetThreadContext = 480,
    RtlWow64SetThreadContext = 481,
    RtlZombifyActivationContext = 482,
    TpAllocCleanupGroup = 483,
    TpAllocIoCompletion = 484,
    TpAllocPool = 485,
    TpAllocTimer = 486,
    TpAllocWait = 487,
    TpAllocWork = 488,
    TpCallbackMayRunLong = 489,
    TpQueryPoolStackInformation = 490,
    TpSetPoolStackInformation = 491,
    TpSimpleTryPost = 492,
    Strnicmp = 493,
    Vsnwprintf = 494,
    Isalnum = 495,
    ...
    GetParent = 522,
    ShowWindow = 523,
    InvalidateWindow = 524,
    BeginWindowPaint = 525,
    EndWindowPaint = 526,
    FillGdiRect = 527,
    BlitGdiSurface = 528,
    ...
    SetFocusWindow = 532,
    InjectKey = 533,
    /// Resolve one Wine relay thunk to its original PE export.
    RelayCall = 534,
    ...
    /// Dispatch one Wine Unix-library call `(handle, code, args)`.
    WineUnixCall = 538,
    /// Arm one process-owned Win32 window timer.
    SetWindowTimer = 539,
    /// Remove one process-owned Win32 window timer.
    KillWindowTimer = 540,
    /// Test the current thread's alertable NT APC state.
    NtTestAlert = 541,
    /// Continue execution from a validated Windows context record.
    NtContinue = 542,
    /// Keep a named NT object in its namespace after its handles close.
    NtMakePermanentObject = 543,
    RtlDeNormalizeProcessParams = 545,
    /// Query the primary DRM driver's native Vulkan admission contract.
...
pub enum NtWindowCall {
    ...
    PostQuit { code: i32 },
    SetFocus { hwnd: u64 },
    InjectKey { key: u16, pressed: u32, repeat: u32 },
    SetTimer { hwnd: u64, id: u64, timeout_ms: u32, proc: u64 },
    KillTimer { hwnd: u64, id: u64 },
}
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum NtGdiCall {
...
pub enum NtObjectCall {
    ...
    CreateTimer { handle: UserPtr<u32>, desired_access: u32, attributes: u64, timer_type: u32 },
    ...
    QueryToken { token: u32, class: u32, info: Option<UserPtr<u8>>, length: u32, return_length: Option<UserPtr<u32>> },
...
pub fn decode(service: u32, args: SyscallArgs) -> Option<NtCall> {
    ...
    if service == 405 { return Some(NtCall { service: NtService::RtlGetSystemTimePrecise, args }); }
    if service == 406 { return Some(NtCall { service: NtService::RtlGetThreadErrorMode, args }); }
    if service == 407 { return Some(NtCall { service: NtService::RtlGetThreadPreferredUILanguages, args }); }
    if service == 408 { return Some(NtCall { service: NtService::RtlGetUserPreferredUILanguages, args }); }
    if service == 409 { return Some(NtCall { service: NtService::RtlGetVersion, args }); }
    if service == 410 { return Some(NtCall { service: NtService::RtlIdentifierAuthoritySid, args }); }
    if service == 411 { return Some(NtCall { service: NtService::RtlIdnToAscii, args }); }
    if service == 412 { return Some(NtCall { service: NtService::RtlIdnToNameprepUnicode, args }); }
    if service == 413 { return Some(NtCall { service: NtService::RtlIdnToUnicode, args }); }
    if service == 414 { return Some(NtCall { service: NtService::RtlImpersonateSelf, args }); }
    if service == 415 { return Some(NtCall { service: NtService::RtlInitBarrier, args }); }
    if service == 416 { return Some(NtCall { service: NtService::RtlInitCodePageTable, args }); }
    if service == 417 { return Some(NtCall { service: NtService::RtlInitializeExtendedContext2, args }); }
    if service == 418 { return Some(NtCall { service: NtService::RtlInitializeSid, args }); }
    if service == 419 { return Some(NtCall { service: NtService::RtlIsDosDeviceNameU, args }); }
    if service == 420 { return Some(NtCall { service: NtService::RtlIsNormalizedString, args }); }
    if service == 421 { return Some(NtCall { service: NtService::RtlIsProcessorFeaturePresent, args }); }
    if service == 422 { return Some(NtCall { service: NtService::RtlLengthRequiredSid, args }); }
    if service == 423 { return Some(NtCall { service: NtService::RtlLengthSid, args }); }
    if service == 424 { return Some(NtCall { service: NtService::RtlLocalTimeToSystemTime, args }); }
    if service == 425 { return Some(NtCall { service: NtService::RtlLocateExtendedFeature, args }); }
    if service == 426 { return Some(NtCall { service: NtService::RtlMapGenericMask, args }); }
    if service == 427 { return Some(NtCall { service: NtService::RtlNewSecurityObject, args }); }
    if service == 428 { return Some(NtCall { service: NtService::RtlNewSecurityObjectEx, args }); }
    if service == 429 { return Some(NtCall { service: NtService::RtlNewSecurityObjectWithMultipleInheritance, args }); }
    ...
    if service == 436 { return Some(NtCall { service: NtService::RtlQueryEnvironmentVariableU, args }); }
    ...
    if service == 441 { return Some(NtCall { service: NtService::RtlRaiseException, args }); }
    if service == 442 { return Some(NtCall { service: NtService::RtlRaiseStatus, args }); }
    ...
    if service == 455 { return Some(NtCall { service: NtService::RtlSetHeapInformation, args }); }
    if service == 456 { return Some(NtCall { service: NtService::RtlSetProcessPreferredUILanguages, args }); }
    ...
    if service == 479 { return Some(NtCall { service: NtService::RtlWow64GetProcessMachines, args }); }
    ...
    if service == 484 { return Some(NtCall { service: NtService::TpAllocIoCompletion, args }); }
    ...
    if service == 489 { return Some(NtCall { service: NtService::TpCallbackMayRunLong, args }); }
    if service == 490 { return Some(NtCall { service: NtService::TpQueryPoolStackInformation, args }); }
    ...
    if service == 498 { return Some(NtCall { service: NtService::Memcmp, args }); }
    if service == 499 { return Some(NtCall { service: NtService::Strcmp, args }); }
    if service == 500 { return Some(NtCall { service: NtService::Strncmp, args }); }
    if service == 501 { return Some(NtCall { service: NtService::Strtol, args }); }
    if service == 502 { return Some(NtCall { service: NtService::Towupper, args }); }
    if service == 503 { return Some(NtCall { service: NtService::Wcscspn, args }); }
    if service == 504 { return Some(NtCall { service: NtService::Wcsnlen, args }); }
    if service == 505 { return Some(NtCall { service: NtService::Wcspbrk, args }); }
    if service == 506 { return Some(NtCall { service: NtService::Wcsspn, args }); }
    if service == 507 { return Some(NtCall { service: NtService::Wcsstr, args }); }
    ...
    if service == 524 { return Some(NtCall { service: NtService::InvalidateWindow, args }); }
    if service == 525 { return Some(NtCall { service: NtService::BeginWindowPaint, args }); }
    ...
    if service == 547 { return Some(NtCall { service: NtService::QueryVulkanCapability, args }); }
    if service == 531 { return Some(NtCall { service: NtService::PostQuitMessage, args }); }
    if service == 532 { return Some(NtCall { service: NtService::SetFocusWindow, args }); }
    ...
    if service == 538 { return Some(NtCall { service: NtService::WineUnixCall, args }); }
    ...
    if service == 543 { return Some(NtCall { service: NtService::NtMakePermanentObject, args }); }
    if service == 545 { return Some(NtCall { service: NtService::RtlDeNormalizeProcessParams, args }); }
    ...
    if service == 539 { return Some(NtCall { service: NtService::SetWindowTimer, args }); }
...
pub fn decode_memory(call: NtCall) -> Result<NtMemoryCall, Errno> {
    ...
    match call.service {
        ...
        NtService::RtlQueryEnvironmentVariableU => Err(Errno::Enosys),
        ...
        NtService::RtlRaiseException => Err(Errno::Enosys),
        NtService::RtlRaiseStatus => Err(Errno::Enosys),
        ...
        NtService::RtlSetHeapInformation => Err(Errno::Enosys),
        NtService::RtlSetProcessPreferredUILanguages => Err(Errno::Enosys),
        ...
        NtService::RtlWow64EnableFsRedirection | NtService::RtlWow64EnableFsRedirectionEx | NtService::RtlWow64GetProcessMachines | NtService::RtlWow64GetThreadContext | NtService::RtlWow64SetThreadContext | NtService::RtlZombifyActivationContext | NtService::TpAllocCleanupGroup | NtService::TpAllocIoCompletion | NtService::TpAllocPool | NtService::TpAllocTimer | NtService::TpAllocWait | NtService::TpAllocWork | NtService::TpCallbackMayRunLong | NtService::TpQueryPoolStackInformation | NtService::TpSetPoolStackInformation | NtService::TpSimpleTryPost | NtService::Strnicmp | NtService::Vsnwprintf | NtService::Isalnum | NtService::Iswalnum | NtService::Isxdigit | NtService::Memcmp | NtService::Strcmp | NtService::Strncmp | NtService::Strtol | NtService::Towupper | NtService::Wcscspn | NtService::Wcsnlen | NtService::Wcspbrk | NtService::Wcsspn | NtService::Wcsstr | NtService::Wcstol | NtService::LdrGetDllHandle | NtService::RtlFindExportedRoutineByName => Err(Errno::Enosys),
        ...
        NtService::RtlGetEnabledExtendedFeatures | NtService::RtlGetExePath | NtService::RtlGetExtendedContextLength2 | NtService::RtlGetExtendedFeaturesMask | NtService::RtlGetFullPathNameU | NtService::RtlGetGroupSecurityDescriptor | NtService::RtlGetLocaleFileMappingAddress | NtService::RtlGetNativeSystemInformation | NtService::RtlGetOwnerSecurityDescriptor | NtService::RtlGetProductInfo | NtService::RtlGetProcessPreferredUILanguages | NtService::RtlGetSaclSecurityDescriptor | NtService::RtlGetSearchPath | NtService::RtlGetSystemPreferredUILanguages | NtService::RtlGetSystemTimePrecise | NtService::RtlGetThreadErrorMode | NtService::RtlGetThreadPreferredUILanguages | NtService::RtlGetUserPreferredUILanguages | NtService::RtlGetVersion | NtService::RtlIdnToAscii | NtService::RtlIdnToNameprepUnicode | NtService::RtlIdnToUnicode | NtService::RtlIdentifierAuthoritySid | NtService::RtlImpersonateSelf | NtService::RtlInitBarrier | NtService::RtlInitCodePageTable | NtService::RtlInitializeExtendedContext2 | NtService::RtlInitializeSid | NtService::RtlIsDosDeviceNameU | NtService::RtlIsNormalizedString | NtService::RtlIsProcessorFeaturePresent | NtService::RtlLengthRequiredSid | NtService::RtlLengthSid | NtService::RtlLocalTimeToSystemTime | NtService::RtlLocateExtendedFeature | NtService::RtlMapGenericMask | NtService::RtlNewSecurityObject | NtService::RtlNewSecurityObjectEx | NtService::RtlNewSecurityObjectWithMultipleInheritance | NtService::RtlNormalizeProcessParams | NtService::RtlNormalizeString | NtService::RtlOpenCurrentUser => Err(Errno::Enosys),
...
pub fn decode_window(call: NtCall) -> Result<NtWindowCall, Errno> {
    ...
    match call.service {
        ...
        NtService::SetFocusWindow => Ok(NtWindowCall::SetFocus { hwnd: a.a0 }),
        NtService::InjectKey => Ok(NtWindowCall::InjectKey { key: a.a0 as u16, pressed: a.a1 as u32, repeat: a.a2 as u32 }),
        NtService::SetWindowTimer => Ok(NtWindowCall::SetTimer { hwnd: a.a0, id: a.a1, timeout_ms: a.a2 as u32, proc: a.a3 }),
        NtService::KillWindowTimer => Ok(NtWindowCall::KillTimer { hwnd: a.a0, id: a.a1 }),
        _ => Err(Errno::Enosys),
    }
}
...
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub enum NtVulkanCall {
    QueryCapability { info: UserPtr<NtVulkanCapability>, length: u32 },
}

#[repr(C)]
#[derive(Copy, Clone, Debug, Eq, PartialEq)]
pub struct NtVulkanCapability {
    pub version: u32,
    pub flags: u32,
    pub max_width: u32,
    pub max_height: u32,
...
pub fn decode_vulkan(call: NtCall) -> Result<NtVulkanCall, Errno> {
    if call.service != NtService::QueryVulkanCapability { return Err(Errno::Enosys); }
    Ok(NtVulkanCall::QueryCapability { info: UserPtr::new(call.args.a0)?, length: call.args.a1 as u32 })
}
...
pub fn decode_object(call: NtCall) -> Result<NtObjectCall, Errno> {
    ...
    match call.service {
        ...
        NtService::CreateTimer => Ok(NtObjectCall::CreateTimer { handle: UserPtr::new(a.a0)?, desired_access: a.a1 as u32, attributes: a.a2, timer_type: a.a3 as u32 }),
        ...
        NtService::QueryToken => Ok(NtObjectCall::QueryToken { token: a.a0 as u32, class: a.a1 as u32, info: optional_ptr(a.a2)?, length: a.a3 as u32, return_length: optional_ptr(a.a4)? }),
