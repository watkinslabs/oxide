...
pub type SysEnterFn = fn(u32, &crate::SyscallArgs);
/// `sys_exit` hook: receives the syscall nr + the return value. # type
pub type SysExitFn = fn(u32, i64, &crate::SyscallArgs);
...
#[inline]
pub fn fire_sys_enter(nr: u32, args: &crate::SyscallArgs) {
    let raw = SYS_ENTER_HOOK.load(Ordering::Acquire);
    if raw.is_null() { return; }
    // SAFETY: raw was installed via `install_sys_enter_hook` with the
    // documented `fn(u32, &SyscallArgs)` signature; non-null implies a live fn pointer.
...
#[inline]
pub fn fire_sys_exit(nr: u32, ret: i64, args: &crate::SyscallArgs) {
    let raw = SYS_EXIT_HOOK.load(Ordering::Acquire);
    if raw.is_null() { return; }
    // SAFETY: raw was installed via `install_sys_exit_hook` with the
    // documented `fn(u32, i64, &SyscallArgs)` signature; non-null implies a live fn pointer.
