...
mod tests {
    ...
    /// ftruncate. Pre-B37 confused these. F123: also fixes the
    /// destination — arm 43 was landing on x86 179 (quotactl), not
    /// x86 137 (statfs).
    #[test]
    ...
    /// F123: dhcpcd-aarch64 wedged because arm 261 (prlimit64) fell
    /// through to x86 261 (futimesat). musl's getrlimit hit ENOSYS
    /// and silently stalled the client. Pin so this never regresses.
    #[test]
    ...
    /// F123: epoll family on arm. Without explicit mappings, arm
    /// 20/21/22 fell through to x86 20 (writev), 21 (access),
    /// 22 (pipe) — every dhcpcd epoll call would silently scramble
    /// fds. Pin all three.
    #[test]
    ...
    /// F123: inotify family — arm 26/27/28 must not fall through to
    /// x86 msync/mincore/madvise.
    #[test]
    ...
    /// F123: sysinfo (arm 179) was routed to x86 39 (getpid) with a
    /// comment claiming "no x86 nr in our table" — incorrect; x86
    /// sysinfo lives at slot 99 and IS implemented.
    #[test]
    ...
    /// F123: renameat2/seccomp/getrandom on arm-generic must land on
    /// the same-named x86 slot. renameat2 in particular is a high-risk
    /// pass-through — arm 276 → x86 276 (vmsplice) would corrupt the
    /// rename path.
    #[test]
    ...
    /// F123: SysV shm (arm 194-197) — without mapping, arm 194
    /// fell through to x86 194 (listxattr).
    #[test]
    ...
    /// F123: getrlimit/setrlimit (arm 163/164) must not fall through
    /// to x86 163 (acct) / 164 (settimeofday).
    #[test]
    ...
    /// F123: post-rebase syscalls (424+) use the SAME nr on both
    /// arches, but pin a few so a future numbering drift catches.
    #[test]
