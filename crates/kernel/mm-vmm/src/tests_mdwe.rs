...
fn mprotect_commits_linux_prefix_and_applies_read_implies_exec_per_vma() {
    ...
    let partial = mm.mprotect_user(first, 2 * PAGE, VmaProt::READ, true, &mut |v: &crate::Vma| v.pkey).unwrap();
    ...
    mixed.mmap_with_may(Some(second), PAGE, VmaProt::READ, VmaProt::READ,
                        private(), VmaBacking::Anonymous, true).unwrap();
    let outcome = mixed.mprotect_user(first, 2 * PAGE, VmaProt::READ, true, &mut |v: &crate::Vma| v.pkey).unwrap();
