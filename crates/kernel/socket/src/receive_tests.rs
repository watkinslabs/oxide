...
// Every test here exercises the process-global AF_UNIX in-flight/GC state, so
// each takes the crate's one claim on it. The claim is not a convention: the
// publication path checks it, so a test that forgets fails on its first run.
...
fn discard_queued_cycle(mode: Discard) {
    let _serial = crate::test_support::scm_graph();
