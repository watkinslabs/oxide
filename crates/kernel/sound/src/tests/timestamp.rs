...
fn timestamp_type_selects_the_status_clock_for_playback_and_capture() {
    ...
    assert_eq!(pcm::handle(owner, 0, uapi::PCM_TSTAMP, (&mut invalid as *mut u32) as u64),
        test_err(syscall::errno::Errno::Einval));
    assert_eq!(capture::handle(owner, 0, uapi::PCM_TTSTAMP, 0),
    ...
    assert_eq!(pcm::handle(owner, 0, uapi::PCM_SW_PARAMS, sw.as_mut_ptr() as u64), 0);
    assert_eq!(capture::handle(owner, 0, uapi::PCM_SW_PARAMS, sw.as_mut_ptr() as u64), 0);

    let mut playback_kind = uapi::TSTAMP_TYPE_MONOTONIC_RAW;
    let mut capture_kind = uapi::TSTAMP_TYPE_REALTIME;
    assert_eq!(pcm::handle(owner, 0, uapi::PCM_TSTAMP,
                          (&mut playback_kind as *mut u32) as u64), 0);
    assert_eq!(capture::handle(owner, 0, uapi::PCM_TTSTAMP,
    ...
    assert_eq!(pcm::handle(owner, 0, uapi::PCM_STATUS, open_status.as_mut_ptr() as u64), 0);
    assert_eq!(get_u64(&open_status, uapi::ST_TSTAMP_SEC), 0);
    assert_eq!(pcm::handle(owner, 0, uapi::PCM_PREPARE, 0), 0);
    assert_eq!(capture::handle(owner, 0, uapi::PCM_PREPARE, 0), 0);
    assert_eq!(pcm::handle(owner, 0, uapi::PCM_START, 0), 0);
    assert_eq!(capture::handle(owner, 0, uapi::PCM_START, 0), 0);
    ...
    assert_eq!(pcm::handle(owner, 0, uapi::PCM_STATUS, playback.as_mut_ptr() as u64), 0);
    assert_eq!(capture::handle(owner, 0, uapi::PCM_STATUS, captured.as_mut_ptr() as u64), 0);
    ...
    assert_eq!(pcm::handle(owner, 0, uapi::PCM_SYNC_PTR, sync.as_mut_ptr() as u64), 0);
