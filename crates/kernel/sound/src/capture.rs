...
// refinement) and driving drv-virtio-snd's capture ops (cap_hw_params /
...
fn caps() -> (u64, u64, u8, u8) {
    drv_virtio_snd::cap_caps().unwrap_or((1 << 5, 1 << 6, 1, 2))
}
...
fn refine(b: &UserBuf, commit: bool) -> i64 {
    ...
    if commit {
        if !drv_virtio_snd::cap_hw_params(rate_hz_to_enum(r.rate), fmt_alsa_to_virtio(r.format),
...
fn readi(arg: u64) -> i64 {
    ...
    while done < bytes {
        ...
        let got = drv_virtio_snd::pcm_recv(&mut staged[..chunk]);
        if got == 0 { break; }
...
pub fn read_bytes(buf: &mut [u8]) -> usize {
    ...
    let n = drv_virtio_snd::pcm_recv(buf);
    if n > 0 {
