...
/// have been published. The PCI vector is enabled only after interface
/// publication, so the final hardware mask remains clear here. # C: O(1)
pub fn start_plan(tx_config: u32, rx_dma: u64, tx_dma: u64) -> Option<[Op; 12]> {
    ...
    Some([
        ...
        Op::Read32(regs::TX_CONFIG), Op::Write16(regs::INTR_MASK, 0),
    ])
}
...
mod tests {
    ...
    fn rings_and_filters_precede_dma_enable_and_interrupts() {
        ...
        assert_eq!(plan[11], Op::Write16(regs::INTR_MASK, 0));
