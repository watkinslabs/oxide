...
pub struct CommandTransport { page: DmaPage, ring: Spinlock<CommandRing, DriverLockClass> }

impl CommandTransport {
    /// Take ownership of the page already named by CRCR. # C: O(1)
    pub fn new(page: DmaPage) -> Option<Self> { Some(Self { ring: Spinlock::new(CommandRing::new(page.dma())?), page }) }
    /// Submit exactly one command after writing and synchronizing its TRB. # C: O(1)
    pub fn submit(&self, mmio: &Mmio, trb: Trb) -> Option<u64> {
        let mut ring = self.ring.lock();
        let (pa, _) = ring.push(trb);
        let index = pa.checked_sub(self.page.dma())?.checked_div(TRB_BYTES as u64)? as usize;
        let trb = ring.trb(index)?;
