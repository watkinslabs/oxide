...
/// # C: O(1)
pub fn geometry(bar_bytes: u64, caplength: u8, hcsparams1: u32, dboff: u32, rtsoff: u32) -> Option<Geometry> {
    ...
    { return None; }
    Some(Geometry { operational, runtime, doorbells, max_slots, max_interrupters, max_ports })
}
...
mod tests {
    ...
    fn capability_geometry_preserves_hardware_offsets_and_limits() {
        ...
        let g = geometry(0x4000, 0x40, hcs, 0x2000, 0x1000).unwrap();
        assert_eq!((g.operational, g.runtime, g.doorbells, g.max_slots), (0x40, 0x1000, 0x2000, 64));
    ...
    fn malformed_or_out_of_aperture_controller_is_rejected() {
        let hcs = 1 | (1 << 8) | (1 << 24);
        assert!(geometry(0x1000, 0x1f, hcs, 0x200, 0x400).is_none());
        assert!(geometry(0x1000, 0x20, hcs, 0x200, 0x1000).is_none());
        assert!(geometry(0x1000, 0x20, hcs, 0x1000, 0x400).is_none());
        assert!(geometry(0x1000, 0x20, 0, 0x200, 0x400).is_none());
    }
}
