...
impl Console {
    ...
    fn blit_glyph(&mut self, codepoint: u32) {
        // Map the codepoint to a glyph index through the active font's
        // unicode table (`conv_uni_to_pc`); unmapped → the font's '?'
        // fallback. Same font source as the VT-layer renderer (vcrender) —
        // one console font, no ASCII-only shortcut.
