...
/// Built-in 8×16 console font: IBM VGA `default8x16` glyphs for ASCII
/// 0x20..0x7e (the classic kbd `default8x16.psfu`, public domain).
/// Indexed `BUILTIN_FONT[(cp-0x20)*16 + row]`; MSB = leftmost pixel.
#[rustfmt::skip]
const BUILTIN_FONT: &[u8; BUILTIN_FONT_LEN] = &[
...
mod tests {
    ...
    fn glyph_blit_writes_pixels() {
        ...
        c.bg = [0, 0, 0];
        c.put(b"X");
        // Real 8×16 font: the 'X' glyph's first 8×16 cell must contain
        // both foreground (red) and background pixels — i.e. an actual
        // glyph shape, not the old all-or-nothing placeholder.
        let (mut fg, mut bg) = (0u32, 0u32);
        for py in 0..16usize {
            for px in 0..8usize {
                let off = (py * 64 + px) * 4;
                if c.fb[off + 2] == 0xff { fg += 1; } else { bg += 1; }
            }
        }
        assert!(fg > 0, "glyph wrote no foreground pixels");
        assert!(bg > 0, "glyph wrote no background pixels (solid block?)");
    }
}
