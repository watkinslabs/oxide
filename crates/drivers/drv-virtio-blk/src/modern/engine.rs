...
impl BlkState {
    ...
    pub(super) fn do_request(&self, h: u64, type_: u32, sector: u64, data: &mut [u8],
        ...
        let target = {
            ...
            // `inflight` is held. The descriptor and avail-ring stores must be
            // clean before the index is published: Linux's virtio_wmb() gives
            // the device a complete chain before it is allowed to consume it.
