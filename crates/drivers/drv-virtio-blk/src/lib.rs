...
//! in `pci_boot::virtio_drv` negotiates VIRTIO_F_VERSION_1, programs
//! queue 0, reaches DRIVER_OK, and hands the persistent ring addresses
//! + device-cfg here via `modern::init_blk`, which builds a `BlkState`
//! request engine and registers a `BlockDevice` so ext4 can mount a
//! real disk.
//!
