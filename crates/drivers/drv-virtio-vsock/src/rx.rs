...
use super::{Ctx, CTX, RX_RING_BUFS};
...
pub(crate) fn prepost_all(device_key: u32) {
    let mut g = CTX.lock();
    let ctx = match g.iter_mut().find(|ctx| ctx.device_key == device_key) { Some(c) => c, None => return };
    let h = ctx.hhdm;
...
pub(crate) fn drain() -> usize {
    ...
    let mut pkts: alloc::vec::Vec<(VsockHdr, alloc::vec::Vec<u8>)> = alloc::vec::Vec::new();
    {
        let mut g = CTX.lock();
        for ctx in g.iter_mut() {
            drain_one(ctx, &mut pkts);
        }
    }

    let count = pkts.len();
    for (hdr, payload) in pkts { net::vsock::deliver_rx(&hdr, &payload); }
    count
}

fn drain_one(ctx: &mut Ctx, pkts: &mut alloc::vec::Vec<(VsockHdr, alloc::vec::Vec<u8>)>) {
