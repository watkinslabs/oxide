...
fn clear_card_state_releases_bound_scanout_resource() {
    ...
    crate::node::set_scanout_ops(3, crate::node::ScanoutOps {
        ...
        present: test_present, set_cursor: Some(test_set_cursor), move_cursor: Some(test_move_cursor),
...
fn mmap_pin_survives_card_remove_until_unpin() {
    ...
    t.insert_buf(DumbBuf { card_id: 0, handle: 1, resource_id: 0, blob_mem: 0, owner_token: 0, pa: 0x10_0000, size: 4096,
...
fn closefb_defers_retirement_until_scanout_replaces_it() {
    let mut t = DumbTables::new();
    t.insert_buf(DumbBuf { card_id: 0, handle: 7, resource_id: 0, blob_mem: 0, owner_token: 11, pa: 0x10_0000, size: 4096,
