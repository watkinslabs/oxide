...
pub fn read_header(src: &[u8]) -> Result<Header> {
    ...
    if dict_id_len != 0 {
        let mut id = 0u32;
        for i in 0..dict_id_len { id |= (src[at + i] as u32) << (8 * i); }
        at += dict_id_len;
        if id != 0 { return Err(Error::DictionaryRequired); }
    }
...
/// # C: O(1)
pub fn write_header(content_size: u64, checksum: bool, out: &mut Vec<u8>) {
    ...
    let mut fhd = FHD_SINGLE_SEGMENT | (fcs_code << FHD_FRAME_CONTENT_SIZE_SHIFT);
...
mod tests {
    ...
    fn every_content_size_width_round_trips() {
        ...
        for size in [0u64, 1, 255, 256, 257, 65791, 65792, 100_000, 5_000_000_000] {
            ...
            write_header(size, false, &mut buf);
    ...
    fn the_checksum_flag_survives_the_round_trip() {
        let mut buf = alloc::vec::Vec::new();
        write_header(4096, true, &mut buf);
    ...
    #[test]
    fn a_frame_naming_a_dictionary_is_refused() {
        // zram never writes one, and silently ignoring the id would decode
        // wrong content rather than failing.
        let mut buf = alloc::vec::Vec::new();
        buf.extend_from_slice(&MAGIC.to_le_bytes());
        buf.push(FHD_SINGLE_SEGMENT | 1);
        buf.push(0x42);
        buf.push(10);
        assert_eq!(read_header(&buf).unwrap_err(), Error::DictionaryRequired);
    }
