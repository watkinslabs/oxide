...
/// Maximum LZO1X-1 stream size for `bytes` input bytes. # C: O(1)
pub const fn worst_size(bytes: usize) -> usize { bytes + bytes / 16 + 64 + 3 }
...
pub fn compress(src: &[u8], dst: &mut [u8], rle: bool) -> Option<usize> {
    ...
    compress_with(src, dst, rle, &mut workspace)
}

/// Reusable direct-match workspace for allocation-free repeated compression.
pub struct Workspace { dict: Box<[u32]> }

impl Workspace {
    /// Allocate the exact LZO1X-1 dictionary. # C: O(D_SIZE)
    pub fn new() -> Self { Self { dict: vec![0u32; D_SIZE].into_boxed_slice() } }
}

impl Default for Workspace { fn default() -> Self { Self::new() } }

/// Encode through one caller-owned dictionary, clearing it between streams.
/// # C: O(src bytes + D_SIZE)
pub fn compress_with(src: &[u8], dst: &mut [u8], rle: bool,
    workspace: &mut Workspace) -> Option<usize>
{
    compress_impl(src, dst, rle, workspace, None)
}

fn compress_impl(src: &[u8], dst: &mut [u8], rle: bool, workspace: &mut Workspace,
    mut probes: Option<&mut usize>) -> Option<usize>
{
    workspace.dict.fill(0);
    let n = src.len();
    ...
    if n > TAIL_RESERVE {
        let (i, p) = body(src, &mut out, rle, max_off, &mut workspace.dict, &mut probes)?;
        ii = i;
        patch_back = p;
    }
...
#[cfg(test)]
/// Encode while counting direct-dictionary probes. # C: O(src bytes + D_SIZE)
pub(crate) fn compress_counted(src: &[u8], dst: &mut [u8], rle: bool,
    workspace: &mut Workspace) -> Option<(usize, usize)>
{
    let mut probes = 0;
    let written = compress_impl(src, dst, rle, workspace, Some(&mut probes))?;
    Some((written, probes))
}
...
/// # C: O(src bytes)
fn body(src: &[u8], out: &mut Out, rle: bool, max_off: usize,
    dict: &mut [u32], probes: &mut Option<&mut usize>) -> Option<(usize, usize)>
{
    let n = src.len();
    ...
    loop {
        ...
        loop {
            ...
            if dv == 0 && rle {
                ...
                if let Some(count) = probes.as_deref_mut() { *count += 1; }
