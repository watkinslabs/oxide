...
pub(crate) const MAX_RK: usize = BLOCK * 15;
...
/// # C: O(rounds)
pub(crate) fn expand(key: &[u8], out: &mut [u8], rounds: usize) {
    let nk = key.len() / 4;
...
/// Encrypt one block in place with an expanded schedule. # C: O(rounds)
pub(crate) fn encrypt(rk: &[u8], rounds: usize, b: &mut [u8; BLOCK]) {
...
/// Decrypt one block in place with an expanded schedule. # C: O(rounds)
pub(crate) fn decrypt(rk: &[u8], rounds: usize, b: &mut [u8; BLOCK]) {
    add_round_key(b, &rk[rounds * BLOCK..(rounds + 1) * BLOCK]);
