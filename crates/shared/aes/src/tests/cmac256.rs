...
use crate::cmac::{Cmac, CMAC_LEN, CMAC_LEN_8};
use crate::gcm;
...
fn cmac_matches_vectors() {
    ...
    for (ks, pick) in [(K128, 0usize), (K256, 1usize)] {
        ...
        for (n, t128, t256) in CASES {
            ...
            Cmac::with_key(key.clone()).mac_into(&msg[..*n], &mut out);
            assert_hex(&out, want);
            assert_eq!(Cmac::with_key(key.clone()).mac(&msg[..*n]), out);
        }
    }
}
...
fn truncated_tag_is_prefix_of_full() {
    ...
    for ks in [K128, K256] {
        let key = AesKey::new(&hex(ks)).unwrap();
        for n in 0..=msg.len() {
            let full = Cmac::with_key(key.clone()).mac(&msg[..n]);
            let mut short = [0u8; CMAC_LEN_8];
            Cmac::with_key(key.clone()).mac_into(&msg[..n], &mut short);
...
fn one_bit_message_change_changes_tag() {
    ...
    for n in [0usize, 1, 15, 16, 17, 31, 32, 33] {
        ...
        let want = Cmac::with_key(key.clone()).mac(&base);
        for bit in 0..(n * 8) {
            let mut m = base.clone();
            m[bit / 8] ^= 1 << (bit % 8);
            assert_ne!(Cmac::with_key(key.clone()).mac(&m), want);
        }
        // Appending a byte must also change the tag, including from empty.
        let mut longer = base.clone();
        longer.push(0);
        assert_ne!(Cmac::with_key(key.clone()).mac(&longer), want);
    }
}
...
fn gmac_matches_vectors() {
    for (ks, want) in [(GMAC_K128, "346434fd51d5cd0c5887ec63e39b907a"),
        ...
        let key = AesKey::new(&hex(ks)).unwrap();
        let mut out = [0u8; 16];
        gcm::gmac(&key, &iv12(GMAC_IV), &hex(GMAC_AAD), &mut out);
        assert_hex(&out, want);
    }
}
...
fn gmac_one_byte_aad_change_changes_tag() {
    ...
    let mut want = [0u8; 16];
    gcm::gmac(&key, &iv, &aad, &mut want);
    for i in 0..aad.len() {
        let mut bad = aad.clone();
        bad[i] ^= 0x01;
        let mut got = [0u8; 16];
        gcm::gmac(&key, &iv, &bad, &mut got);
        assert_ne!(got, want);
    }
}
