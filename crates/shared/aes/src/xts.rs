// XTS-AES (IEEE 1619 / NIST SP 800-38E): the narrow-block tweakable mode a
// storage layer uses when every unit must encrypt independently and no space
// is available for an authentication tag.
//
...
pub enum XtsError {
    /// A key that is not two equal halves of a width AES accepts.
    BadKeyLength,
...
#[derive(Clone)]
pub struct Xts { data: AesKey, tweak: AesKey }
...
/// `a ^= b`. # C: O(1)
fn xor(a: &mut [u8], b: &[u8]) { for i in 0..a.len().min(b.len()) { a[i] ^= b[i]; } }

impl Xts {
    /// Split `key` into its data and tweak halves.
    ///
    /// 32 bytes is AES-128-XTS and 64 bytes is AES-256-XTS; the halves are
    /// what AES sees, so the name of the mode is the width of a HALF.
    /// # C: O(1)
    pub fn new(key: &[u8]) -> Result<Self, XtsError> {
        if key.len() != 32 && key.len() != 64 { return Err(XtsError::BadKeyLength); }
        let (d, t) = key.split_at(key.len() / 2);
        let data = AesKey::new(d).ok_or(XtsError::BadKeyLength)?;
        let tweak = AesKey::new(t).ok_or(XtsError::BadKeyLength)?;
        Ok(Self { data, tweak })
    }
