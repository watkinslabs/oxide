...
/// `a ^= b` over one block. # C: O(1)
fn xor(a: &mut [u8], b: &[u8]) { for i in 0..a.len().min(b.len()) { a[i] ^= b[i]; } }

/// Read one block out of a slice. # C: O(1)
fn blk(s: &[u8]) -> [u8; BLOCK_LEN] {
...
/// # C: O(len(buf))
pub fn encrypt(key: &AesKey, iv: &mut [u8; BLOCK_LEN], buf: &mut [u8]) -> Result<(), CbcError> {
    if buf.len() % BLOCK_LEN != 0 { return Err(CbcError::NotBlockAligned); }
    let mut chain = *iv;
...
/// that was consumed.
/// # C: O(len(buf))
pub fn decrypt(key: &AesKey, iv: &mut [u8; BLOCK_LEN], buf: &mut [u8]) -> Result<(), CbcError> {
    if buf.len() % BLOCK_LEN != 0 { return Err(CbcError::NotBlockAligned); }
    let mut chain = *iv;
    for c in buf.chunks_exact_mut(BLOCK_LEN) {
        let ct = blk(c);
...
/// one block.
/// # C: O(len(buf))
pub fn cts_encrypt(key: &AesKey, iv: &[u8; BLOCK_LEN], buf: &mut [u8]) -> Result<(), CbcError> {
    let n = buf.len();
...
/// # C: O(len(buf))
pub fn cts_decrypt(key: &AesKey, iv: &[u8; BLOCK_LEN], buf: &mut [u8]) -> Result<(), CbcError> {
    let n = buf.len();
    if n < BLOCK_LEN { return Err(CbcError::TooShort); }
    let mut chain = *iv;
