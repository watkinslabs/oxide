...
/// sha512crypt (Drepper 2007), bit-compatible with glibc's
/// libcrypt `$6$<salt>$<digest>` format. Verified against RFC
/// test vectors in tests below.
///
/// Salt may be at most 16 bytes; longer salts are truncated by
/// the caller before invocation. Default rounds = 5000; minimum
/// 1000, maximum 999_999_999. The returned string is the
/// 86-character digest portion only (no `$6$<salt>$` prefix).
/// # C: O(rounds × 64)
pub fn sha512crypt(password: &[u8], salt: &[u8], rounds: u32) -> String {
    ...
    // A_input = password || salt || (n-byte chunks of B) || (per-bit of n)
    let mut a_in = Sha512::new();
    a_in.update(password);
    a_in.update(salt);
    let mut k = n;
    while k >= 64 { a_in.update(&b); k -= 64; }
    ...
    // DP = SHA512(password * n) → P = first n bytes (extended)
    let mut dp_h = Sha512::new();
    for _ in 0..n { dp_h.update(password); }
    let dp = dp_h.finish();
    let p = extend(&dp, n);

    // DS = SHA512(salt * (16 + a[0])) → S = first s_len bytes
    let mut ds_h = Sha512::new();
    ...
    // Permuted base64 encoding (sha512 variant).
    encode_b64_sha512(&c)
}

/// Helper: produce `len` bytes by hashing `src` repeatedly.
/// Per Drepper §B.4: the per-block digest is concatenated; we
/// take a different shortcut here matching glibc — write the
/// digest mod-block until we reach `len`.
fn extend(src: &[u8; 64], len: usize) -> Vec<u8> {
    let mut out = Vec::with_capacity(len);
    let mut remain = len;
    while remain >= 64 { out.extend_from_slice(src); remain -= 64; }
    if remain > 0 { out.extend_from_slice(&src[..remain]); }
    out
}

/// crypt-base64 with the sha512crypt-specific byte-permutation.
/// Each 3-byte triple is reordered (low, mid, high) and emitted
/// little-endian into 4 base64 chars; final byte 63 is emitted
/// as a 2-char tail.
fn encode_b64_sha512(c: &[u8; 64]) -> String {
    const ALPH: &[u8; 64] = b"./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
    // glibc permutation order — 21 triples + 1 final.
...
mod tests {
    ...
    /// Drepper-2007 §B.4 published test vector. Validates glibc parity.
    #[test]
    fn sha512crypt_drepper_published_vector() {
        // From the Drepper spec, RFC-shape test #1:
        //   key = "Hello world!"
        //   salt = "saltstring"
        //   rounds = 5000 (default)
        // expected (the part after "$6$saltstring$"):
        //   svn8UoSVapNtMuq1ukKS4tPQd8iKwSMHWjl/O817G3uBnIFNjnQJuesI68u4OTLiBFdcbYEdFCoEOfaS35inz1
        let got = sha512crypt(b"Hello world!", b"saltstring", 5000);
        let want = "svn8UoSVapNtMuq1ukKS4tPQd8iKwSMHWjl/O817G3uBnIFNjnQJuesI68u4OTLiBFdcbYEdFCoEOfaS35inz1";
        assert_eq!(got, want, "drepper vector mismatch");
    }
}
