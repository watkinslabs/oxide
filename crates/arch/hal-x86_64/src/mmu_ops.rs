...
impl MmuOps for X86Mmu {
    ...
    unsafe fn move_leaf_4k_at(root_pa: u64, old: Va, new: Va)
        ...
        unsafe {
            hal::pt_walker::move_leaf_4k_at_root::<PtWalkerX86, _>(
    ...
    unsafe fn move_leaf_at(root_pa: u64, old: Va, new: Va)
        ...
        if hhdm == 0 { return Err(hal::pt_walker::WalkErr::AllocFailed); }
        unsafe {
            hal::pt_walker::move_leaf_at_root::<PtWalkerX86, _>(
                root_pa, old.0, new.0, hhdm, || alloc_frame(root_pa),
            )
        }
    }
    ...
    unsafe fn split_leaf_at(root_pa: u64, va: Va) -> Result<(), hal::pt_walker::WalkErr> {
        ...
        if hhdm == 0 { return Err(hal::pt_walker::WalkErr::AllocFailed); }
        unsafe {
            hal::pt_walker::split_leaf_at_root::<PtWalkerX86, _>(
