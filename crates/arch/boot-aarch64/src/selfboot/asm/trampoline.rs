// MMU trampoline: runs with the MMU off at whatever physical address the image
// was loaded at, builds the boot page tables, enables the MMU and jumps to the
// kernel's higher-half VMA. Table extents and descriptors come from
// `linear_map`, so the tables `tables.rs` reserves and the arithmetic that
// fills them here are the same constants.
...
1:
    ...
    /* l1_ident[0] = Device 1 GiB block @0 (GIC, PL011).
    ...
    mov     x2, #{device_bits}
    str     x2, [x1, #0]
    /* l1_ident[device slots .. end] = Device 1 GiB blocks. Covers the
       virt PCIe ECAM/MMIO windows at high phys (e.g. 0x40_1000_0000 =
       256 GiB) so HHDM-based PCIe enumeration doesn't fault. RAM ends
       below the first of these, so device attrs there are correct.   */
    mov     x2, #{device_first_slot}
    mov     x5, #{device_bits}          /* device-block attr bits      */
2:
    ...
    /* The RAM slots are built at PAGE granularity instead of 1 GiB
       blocks, so a single page can later be taken out of the kernel's
       view of RAM without re-granularising a live mapping — which this
       architecture only guarantees free of translation-conflict aborts
       when the implementation says so, and the CPU models we boot do
       not say so. Device space keeps its blocks: nothing is ever
       removed from it and paging it would cost orders of magnitude
       more static memory than paging RAM.
       l1_ident[RAM slots] -> _sb_l2_hhdm -> _sb_l3_hhdm -> RAM.
       Every count and descriptor below is the constant `linear_map`
       is tested against, so the reserved tables and the arithmetic
       that fills them cannot drift apart.                            */
    adrp    x7, _sb_l2_hhdm
    add     x7, x7, #:lo12:_sb_l2_hhdm
    adrp    x8, _sb_l3_hhdm
    add     x8, x8, #:lo12:_sb_l3_hhdm
    /* Bottom level: leaf j outputs RAM base + j*4 KiB, Normal-WB
       inner-shareable, access flag set — the attributes the blocks it
       replaces carried, differing only in descriptor kind.           */
    mov     x2, #0
    mov     x4, #{page_bits}
    mov     x6, #{ram_base}
    mov     x5, #{l3_entries}
14:
    ...
    cmp     x2, x5
    b.lt    14b
    /* L2 level: entry j -> the j-th bottom-level table. The tables are
       contiguous, so one flat index spans every RAM slot's L2 table.  */
    mov     x2, #0
    mov     x5, #{l3_tables}
15:
    lsl     x3, x2, #12
    add     x3, x3, x8
    orr     x3, x3, #{table_bits}
    str     x3, [x7, x2, lsl #3]
    add     x2, x2, #1
    cmp     x2, x5
    b.lt    15b
    /* l1_ident[RAM slots] -> the L2 tables.                          */
    mov     x2, #0
16:
    lsl     x3, x2, #12
    add     x3, x3, x7
    orr     x3, x3, #{table_bits}
    add     x4, x2, #{ram_first_slot}
    str     x3, [x1, x4, lsl #3]
    add     x2, x2, #1
    cmp     x2, #{ram_slots}
    b.lt    16b
...
5:
    ...
    .section .text
    "#,
    device_bits       = const crate::linear_map::DEVICE_BLOCK_BITS,
    page_bits         = const crate::linear_map::NORMAL_PAGE_BITS,
    table_bits        = const crate::linear_map::TABLE_BITS,
    ram_base          = const crate::linear_map::RAM_BASE_PA,
    ram_first_slot    = const crate::linear_map::RAM_FIRST_L1_SLOT,
    ram_slots         = const crate::linear_map::RAM_L1_SLOTS,
    device_first_slot = const crate::linear_map::DEVICE_FIRST_L1_SLOT,
    l1_entries        = const crate::linear_map::L1_ENTRIES,
    l3_tables         = const crate::linear_map::L3_TABLES,
    l3_entries        = const crate::linear_map::L3_ENTRIES,
);
