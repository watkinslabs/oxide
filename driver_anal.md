...
- Full Linux-grade driver architecture, including proper bus factoring,
  hotplug, fault injection, and multi-device coverage: about 49% complete.
...
- Virtio-snd no longer has PCI-transport-owned sound config harvest. The
  ...
  `DEVICE_CFG` resource before querying PCM stream info.
...
- Virtio child probing is model-driven and child resource handoff is more
  ...
  virtio-vsock, virtio-snd, and virtio-input config parsing have moved into
  their child drivers, but common virtio transport, feature negotiation,
  remaining config harvest, and child policy remain too
...
- Extract the rest of the real virtio bus/core split out of
  ...
  `DEVICE_CFG` window; virtio-blk, virtio-vsock, virtio-snd, and virtio-input
  config parsing have moved into their child drivers. Feature negotiation,
  queue programming, remaining config harvest, MSI-X setup, and failure release
  helpers still need to move behind a `VirtioPciTransport`/`VirtioProbeState`
  boundary.
