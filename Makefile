...
windows-nt-transition-test:
	...
	$(WARNING_RUN) $(CARGO) test --manifest-path userspace/probes/Cargo.toml -p windows-xinput --quiet
	$(WARNING_RUN) $(CARGO) test --manifest-path userspace/probes/Cargo.toml -p windows-winsock --quiet
...
# Native x86_64 PE handoff smoke. The xtask stages the declared Wine fixture
# and its complete runtime catalog into the disposable root image.
...
smoke-windows-notepad-x86:
	...
	SMOKE_REQUIRED_MARKERS='[WINDOWS-USER32] create-window|[WINDOWS-USER32] get-message|[WINDOWS-GDI] begin-paint|[WINDOWS-GDI] present' \
