...
| S4 | io controller (block-layer time accounting) + cpuset affinity (needs SMP multi-CPU runqueues) | **BLOCKED** — cpuset is meaningless until real SMP: AP bring-up + load-balance (`live/balance.rs`) is a one-shot boot smoke, production scheduling is single-CPU. io controller needs block-layer per-request time accounting (own subsystem). Revisit after a real periodic SMP balancer lands. NOT a systemd hard-blocker (pids/memory/cpu/freeze cover what systemd needs). |
