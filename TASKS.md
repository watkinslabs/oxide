...
| BLOCKER | openssl libcrypto.so load-time hang on aarch64 | **OPEN — HARD systemd-on-arm blocker** (found F345). libcrypto.so.3 (openssl 3.0.15) HANGS at LOAD on aarch64 under the oxide kernel — before `main`. Proven: a probe that only `printf("main reached")` + takes symbol addresses (NO openssl call) never reaches main on arm; ruled out libssl `SSL_CTX_new`, crypto-insn SIGILL (kernel sets `AT_HWCAP=0` in `crates/kernel/exec/src/stack.rs:136` so openssl uses pure-C SHA), and IFUNC (no IRELATIVE relocs). x86 loads + EVP works; libgcrypt (smaller .so, same SHA) works on arm. So it's ld-musl loading/relocating the ~4 MB libcrypto.so OR a constructor doing a blocking/looping syscall — arm-specific. openssl_probe is GATED OFF on arm in `assets/oxide-smokes.sh` (uname check) so arm boots; libs staged both arches. **Root-cause in D6 via qemu-mcp** (boot arm, interrupt at the hang, regs/PC: kernel-vs-userspace; trace ld-musl reloc loop / kernel fault-retry). Blocks systemd-resolved/journald openssl on arm. |
