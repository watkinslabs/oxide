...
| S4a | **real SMP multi-CPU scheduling** (the foundation cpuset needs — no longer skipped): periodic load balancer + multi-CPU boot verification | **IN PROGRESS** (SMP1, F325): AP bring-up already existed (Limine x86 / PSCI arm) + per-CPU runqueue array + `balance_once` (busiest→idlest migrate + resched IPI); it was only a one-shot boot smoke. Now `balance_once` runs **periodically** from the kthread tick (bounded loop/pass, converges a burst), and the **boot-smoke gate boots `SMP=2`** (Makefile `SMP` passthrough + `boot-smoke.sh OXIDE_SMP=2`) so AP-online + balancing is exercised on BOTH arches every push. NEXT: per-AP periodic timer (APs preempt on own tick, not just resched IPI); least-loaded initial placement. |
