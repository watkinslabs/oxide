...
| S4a-arm | arm SMP=2 bring-up (PSCI AP entry) | **BLOCKED — diagnosed** (F325): arm `-smp 2` wedges (340s, no login). Root cause: `bring_up_aps_arm` calls PSCI `cpu_on(entry_pa = oxide_ap_entry_arm as VA)`, but PSCI starts secondaries with MMU state unguaranteed → the AP faults fetching/executing at the upper-half VA (and `ldr [x0]` of a VA context ptr). FIX: identity-mapped AP trampoline (phys entry) that sets up the AP's MMU (TTBR + enable) then jumps to VA `ap_main`. arm gate stays `-smp 1` until then. Also: `balance_once`'s wake-IPI is `#[cfg(x86_64)]`-only — arm needs a GIC SGI resched IPI or migrated tasks never wake on the destination AP. |
