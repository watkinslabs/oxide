...
| S4b | cpuset affinity: per-task cpu-allowed mask + `sched_setaffinity`/`getaffinity`; picker + balancer honor it; cgroup `cpuset.cpus`/`cpuset.mems` apply it to members | not started (unblocked once S4a balancer is live) |
