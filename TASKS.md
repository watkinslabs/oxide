...
- **bash command substitution `$(...)` yields empty/partial — BASH-SPECIFIC, kernel proven correct** (B20). Symptom: `$(echo x)` empty (racy), `$(echo one; echo two)` → only `one`; backticks + process-substitution `<()` same; pipelines (`echo|cat`), subshells `()`, here-strings `<<<`, file redirects all WORK. Exhaustively ruled out the kernel: `/bin/cmdsubst_probe` reproduces both the single fork+dup2+exec+parent-read AND the nested `$(echo one; echo two)` subshell-holds-write-end structure in pure C — BOTH capture full output. Also verified pipe/dup2/CLOEXEC/F_DUPFD/wait-after-close-retention/nested-fork-exit all correct. So it is bash-internal: needs syscall-level tracing of bash (`$()` uses some fcntl/O_NONBLOCK+poll/signal path the C repro doesn't). NOT a kernel pipe bug. High value for distro scripts; resume by stracing bash (add strace to rootfs, or gdb via qemu MCP and break in command_substitute/read_comsub).
- **alarm()/SIGALRM does not wake a task blocked in `read()`** (B20). A `read()` on an empty pipe with an `alarm(1)` + SIGALRM handler never returns (the EINTR check added in 7fcc11fe only fires once something wakes the parked task; `sys_kill` wakes via `wake_if_sleeping`, but the timer/alarm path apparently doesn't post+wake for a task parked in read). Investigate `sched::live::tick_deadline` / itimer delivery to parked tasks.
