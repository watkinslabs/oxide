...
| S4b | cpuset affinity: per-task cpu-allowed mask + `sched_setaffinity`/`getaffinity`; picker + balancer honor it; cgroup `cpuset.cpus`/`cpuset.mems` apply it to members | **PARTIAL** (F328): `Task::cpus_allowed` AtomicU64 (default all-CPUs, inherited on fork); real `sched_setaffinity`/`getaffinity` (slots 203/204 — were stubs) in `syscalls/affinity.rs`, masked to online CPUs, ESRCH/EINVAL per Linux; `balance_once` honors the mask (won't migrate a task to a CPU outside it — puts it back + skips). Hosted test `cpus_allowed_defaults_to_any`. REMAINING: cgroup `cpuset.cpus`/`cpuset.mems` → parse cpulist → CPUSET_HOOK → set member `cpus_allowed` (the WEIGHT_HOOK pattern); enforce affinity at initial placement/wake too (today only the balancer checks). |
