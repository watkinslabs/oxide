...
| S4a-arm | arm SMP=2 bring-up (PSCI AP entry) | **PARTIAL** (B50): real root cause was NOT the MMU trampoline — it was the **PSCI conduit**. `cpu_on` used `smc #0`, but QEMU `virt` (default `secure=off`, no EL3) makes SMC at EL1 UNDEFINED → the BSP took a sync exception (gdb: ESR_EL1 EC=0 at `smc #0` in cpu_on) and looped in `oxide_default_vector_handler`. Fix: HVC conduit (`psci::hvc` / `conduit_call`). VERIFIED: `make SMP=2 qemu-arm` now prints `smp: aps_started=1` + reaches `oxide login:`. REMAINING before flipping the arm gate to `-smp 2`: arm `ap_main` is inert (no VBAR_EL1 / install_default_runqueue / GIC per-CPU enable / DAIF unmask — unlike x86's `oxide_ap_entry_x86`), so the AP comes online but can't run migrated tasks; and `balance_once`'s wake-IPI is `#[cfg(x86_64)]`-only (needs a GIC SGI). Do AP-participation + SGI next, then gate arm `-smp 2`. Conduit-detection from DTB/ACPI for real EL3 hardware also pending (HVC hardcoded for QEMU virt). |
