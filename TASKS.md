...
| R3 | real sysfs — `/sys` is a fake-constant devfs overlay, NOT a filesystem. `/sys/class/net/<if>/*` dynamic from netdev registry; `/sys/devices/system/cpu/{online,possible,present}` from real CPU count; register a SysfsFs backend. HIGH: udev + networkd device model | **partial** — SysfsFs FileSystem mounted at `/sys`; `/sys/class/net` + per-iface dirs dynamic from `net::sock::stack().ifaces` with full attribute set (address/mtu/operstate/type/flags/carrier/speed/duplex/ifindex/...) (PR #1363). REMAINING: `/sys/devices/system/cpu/{present,offline}`; `/sys/class/<name>` → `/sys/devices/...` symlinks (docs/19§2 invariant); uevent broadcast (K5) |
