...
/// Direction of an OperationRegion field access.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum RegionAccessDirection { Read, Write }

/// PCI function selected by a PCI configuration OperationRegion.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct PciAddress { pub segment: u16, pub bus: u8, pub device: u8, pub function: u8 }

/// Complete OperationRegion access retained until the address-space handler.
///
/// `offset` is byte-relative to `base`; `width` is in bits. `pci` is present
/// only for PCI configuration space.
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub struct RegionAccess {
    pub space: value::RegionSpace,
    pub base: u64,
    pub length: u64,
    pub offset: u64,
    pub width: u64,
    pub direction: RegionAccessDirection,
...
impl AmlContext {
    ...
    pub(crate) fn read_region(&self, region_handle: AmlHandle, offset: u64, width: u64) -> Result<u64, AmlError> {
        self.handler.access(self.region_access(region_handle, offset, width, RegionAccessDirection::Read)?, 0)
    }

    pub(crate) fn write_region(&self, region_handle: AmlHandle, offset: u64, width: u64, value: u64) -> Result<(), AmlError> {
        self.handler.access(self.region_access(region_handle, offset, width, RegionAccessDirection::Write)?, value).map(|_| ())
    }

    fn region_access(&self, region_handle: AmlHandle, offset_bits: u64, width: u64,
        direction: RegionAccessDirection) -> Result<RegionAccess, AmlError> {
        use core::convert::TryInto;
        use value::RegionSpace;

        if offset_bits % 8 != 0 { return Err(AmlError::FieldInvalidOffset); }
        let (space, base, length, parent_device) = match self.namespace.get(region_handle)? {
            AmlValue::OpRegion { region, offset, length, parent_device } => (*region, *offset, *length, parent_device),
            _ => return Err(AmlError::FieldRegionIsNotOpRegion),
        };
        let offset = offset_bits / 8;
        let access_bytes = width.checked_add(7).ok_or(AmlError::FieldInvalidAccessSize)? / 8;
        if !matches!(width, 8 | 16 | 32 | 64) || offset.checked_add(access_bytes).is_none_or(|end| end > length) {
            return Err(AmlError::FieldInvalidAccessSize);
        }
        let pci = match space {
            RegionSpace::SystemMemory => None,
            RegionSpace::SystemIo if width <= 32 => None,
            RegionSpace::PciConfig if width <= 32 => {
                let parent = parent_device.as_ref().ok_or(AmlError::FieldRegionMissingParent)?;
                let segment = match self.namespace.search(&AmlName::from_str("_SEG").unwrap(), parent) {
                    Ok((_, handle)) => self.namespace.get(handle)?.as_integer(self)?.try_into().map_err(|_| AmlError::FieldInvalidAddress)?,
                    Err(AmlError::ValueDoesNotExist(_)) => 0,
                    Err(err) => return Err(err),
                };
                let bus = match self.namespace.search(&AmlName::from_str("_BBN").unwrap(), parent) {
                    Ok((_, handle)) => self.namespace.get(handle)?.as_integer(self)?.try_into().map_err(|_| AmlError::FieldInvalidAddress)?,
                    Err(AmlError::ValueDoesNotExist(_)) => 0,
                    Err(err) => return Err(err),
                };
                let (_, handle) = self.namespace.search(&AmlName::from_str("_ADR").unwrap(), parent)?;
                let adr = self.namespace.get(handle)?.as_integer(self)?;
                Some(PciAddress { segment, bus, device: (adr >> 16) as u8, function: adr as u8 })
            }
            RegionSpace::SystemIo | RegionSpace::PciConfig => return Err(AmlError::FieldInvalidAccessSize),
            _ => return Err(AmlError::UnsupportedOperationRegionSpace(space)),
        };
        Ok(RegionAccess { space, base, length, offset, width, direction, pci })
    }
...
pub enum AmlError {
    ...
    FieldRegionMissingParent,
    UnsupportedOperationRegionSpace(value::RegionSpace),
    RegionAccessUnavailable,
...
mod tests {
    ...
    struct CaptureHandler { seen: Arc<Spinlock<Option<(RegionAccess, u64)>>> }

    impl Handler for CaptureHandler {
        fn access(&self, access: RegionAccess, value: u64) -> Result<u64, AmlError> {
            *self.seen.lock() = Some((access, value));
            Ok(0x5a)
        }
    }

    fn context_with_region(space: value::RegionSpace, base: u64, length: u64) -> (AmlContext, AmlHandle, Arc<Spinlock<Option<(RegionAccess, u64)>>>) {
        let seen = Arc::new(Spinlock::new(None));
        let mut context = AmlContext::new(Box::new(CaptureHandler { seen: seen.clone() }), DebugVerbosity::None);
        let name = AmlName::from_str("\\RGN").unwrap();
        context.namespace.add_value(name.clone(), AmlValue::OpRegion { region: space, offset: base, length, parent_device: None }).unwrap();
        let region = context.namespace.get_handle(&name).unwrap();
        (context, region, seen)
    }
    ...
    #[test]
    fn field_read_retains_operation_region_context() {
        let (context, region, seen) = context_with_region(value::RegionSpace::SystemMemory, 0x4000, 8);
        assert_eq!(context.read_region(region, 16, 32), Ok(0x5a));
        assert_eq!(*seen.lock(), Some((RegionAccess {
            space: value::RegionSpace::SystemMemory, base: 0x4000, length: 8, offset: 2, width: 32,
            direction: RegionAccessDirection::Read, pci: None,
        }, 0)));
    }

    #[test]
    fn operation_region_rejects_unaligned_and_out_of_bounds_accesses() {
        let (context, region, seen) = context_with_region(value::RegionSpace::SystemMemory, 0, 2);
        assert_eq!(context.read_region(region, 1, 8), Err(AmlError::FieldInvalidOffset));
        assert_eq!(context.read_region(region, 8, 16), Err(AmlError::FieldInvalidAccessSize));
        assert_eq!(*seen.lock(), None);
    }

    #[test]
    fn pci_operation_region_retains_the_function_identity() {
        let seen = Arc::new(Spinlock::new(None));
        let mut context = AmlContext::new(Box::new(CaptureHandler { seen: seen.clone() }), DebugVerbosity::None);
        let device = AmlName::from_str("\\PCI0").unwrap();
        context.namespace.add_level(device.clone(), LevelType::Device).unwrap();
        context.namespace.add_value(AmlName::from_str("\\PCI0._SEG").unwrap(), AmlValue::Integer(3)).unwrap();
        context.namespace.add_value(AmlName::from_str("\\PCI0._BBN").unwrap(), AmlValue::Integer(7)).unwrap();
        context.namespace.add_value(AmlName::from_str("\\PCI0._ADR").unwrap(), AmlValue::Integer(0x0012_0005)).unwrap();
        let region_name = AmlName::from_str("\\PCI0.RGN").unwrap();
        context.namespace.add_value(region_name.clone(), AmlValue::OpRegion {
            region: value::RegionSpace::PciConfig, offset: 0x40, length: 0x100, parent_device: Some(device),
        }).unwrap();
        let region = context.namespace.get_handle(&region_name).unwrap();
        assert_eq!(context.write_region(region, 24, 8, 0xaa), Ok(()));
        assert_eq!(*seen.lock(), Some((RegionAccess {
            space: value::RegionSpace::PciConfig, base: 0x40, length: 0x100, offset: 3, width: 8,
            direction: RegionAccessDirection::Write,
            pci: Some(PciAddress { segment: 3, bus: 7, device: 0x12, function: 5 }),
        }, 0xaa)));
    }
}
