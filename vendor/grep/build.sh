#!/usr/bin/sh
# GNU grep 3.11 build recipe — static-musl /usr/bin/grep.
...
if [ ! -d "$SRC" ]; then
  echo "missing $SRC — run tools/fetch-grep.sh first" >&2
  exit 1
fi

HDRS_X86=/tmp/musl-hdrs-grep
mkdir -p "$HDRS_X86"
...
HDRS_ARM=/tmp/musl-hdrs-grep-arm
mkdir -p "$HDRS_ARM"
...
build_one() {
  ...
  cp "$SRC/src/grep" "grep-$suffix"
  strip "grep-$suffix" 2>/dev/null || true
  echo "  → grep-$suffix  ($(stat -c %s "grep-$suffix") bytes)"
}

build_one "x86_64"  "musl-gcc"  "-isystem $HDRS_X86" "x86_64"
build_one "aarch64" "$CROSS_CC" "-isystem $HDRS_ARM" "aarch64"

echo "OK — built grep for {x86_64, aarch64}"
