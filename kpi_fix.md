...
| GAP | P0 | TBD-F-kpi-symbol-audit | Build `tools/kpi-audit`: ingest `nm -u driver.ko`, classify unresolved symbols against implemented KPI exports, and emit a ledger-ready missing-symbol table. This is first because real driver symbol demand should order every later shim. |
| GAP | P0 | TBD-F-kpi-uapi-header-surface | Add a generated/minimal Linux KPI header surface for module builds: version macros, config/autoconf values, compiler attributes, section annotations, module license/init/exit/export macros, errno/types, `container_of`, list/hlist/rbtree/xarray/idr/ida/bitmap/bitops declarations. |
| GAP | P0 | TBD-F-kpi-module-loader-lifecycle | Finish module lifecycle: syscall entry for `init_module`/`finit_module`/`delete_module`, executable/W^X module memory, call `module_init`, call `module_exit`, registry state, refcounts, taint flags, unload denial while referenced. |
